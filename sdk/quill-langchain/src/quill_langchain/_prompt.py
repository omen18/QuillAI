"""Build a Quill-aware system prompt for LangChain/LangGraph agents.

The workflow distilled here mirrors the Quill CLI's `quill-usage` skill —
...
``quill_store_query``, the workflow drops the persistence step rather than
...
_INTRO = (
    "You use Quill Engine as the semantic layer for data querying. SQL "
...
_STEP_RECALL = """1. Recall similar past NL→SQL pairs:
   `quill_recall_queries(question="<user's question>", limit=3)`
...
_STEP_FETCH = """2. Fetch schema and business context:
   `quill_fetch_context(question="<user's question>")`
   Optionally narrow scope with `model="<name>"` or
   `item_type="model" | "column" | "relationship" | "view"`."""

_STEP_LIST_MODELS_FALLBACK = """1. If you don't already know the available models, call `quill_list_models()`
   to enumerate them."""

_STEP_COMPOSE = (
    "{n}. Compose SQL targeting Quill model names — NEVER raw database tables."
)

_STEP_DRY_PLAN = """{n}. (Complex queries only) Verify with `quill_dry_plan(sql="...")` before
...
_STEP_QUERY = """{n}. Execute: `quill_query(sql="...", limit=100)`. Raise the limit only when
   you genuinely need more rows."""

_STEP_STORE = """{n}. Persist the NL→SQL pair: `quill_store_query(nl="<user's original \
...
def _build_workflow_section(tool_names: set[str]) -> str:
    ...
    has_recall = "quill_recall_queries" in tool_names
    has_fetch = "quill_fetch_context" in tool_names
    has_store = "quill_store_query" in tool_names
    has_dry_plan = "quill_dry_plan" in tool_names
    has_list_models = "quill_list_models" in tool_names
...
def _error_recovery_section(*, has_fetch_context: bool, has_list_models: bool) -> str:
    if has_fetch_context:
        find_name_hint = (
            'use `quill_fetch_context(question="<bad name>", '
            'item_type="model")` (or `item_type="column"`) to find the '
            "correct one."
        )
    elif has_list_models:
        find_name_hint = (
            "use `quill_list_models()` to enumerate models and inspect their columns."
        )
    else:
...
def _things_to_avoid_section(tool_names: set[str]) -> str:
    bullets = []
    if "quill_fetch_context" in tool_names:
        bullets.append(
            "- Don't guess model or column names — call `quill_fetch_context` first."
        )
    elif "quill_list_models" in tool_names:
        bullets.append(
            "- Don't guess model or column names — call `quill_list_models()` "
            "first when in doubt."
        )
    if "quill_recall_queries" in tool_names:
        bullets.append(
            '- Don\'t skip `quill_recall_queries` on questions that seem "simple" — '
            "past pairs are often the most accurate template."
        )
    if "quill_store_query" in tool_names:
        bullets.append(
...
def build_system_prompt(toolkit: QuillToolkit, *, tools: Iterable | None = None) -> str:
    ...
    ``quill_store_query``, the workflow's persistence step is dropped too.
    """
    ...
    sections.append(
        _error_recovery_section(
            has_fetch_context="quill_fetch_context" in tool_names,
            has_list_models="quill_list_models" in tool_names,
        )
    )
