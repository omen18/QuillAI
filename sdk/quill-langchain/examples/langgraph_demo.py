"""LangGraph demo: build a Quill-aware ReAct agent from primitives.
...
Prereqs match ``langchain_demo.py``: a CLI-prepared Quill project, OPENAI_API_KEY,
...
def build_app(toolkit: QuillToolkit, model_name: str = "gpt-4o"):
    """Compile a ReAct graph that uses Quill tools."""
    ...
    def agent_node(state: AgentState) -> dict:
        """Call the model. Inject the Quill system prompt only on the first turn."""
...
def main() -> None:
    ...
    if not project_path:
        sys.exit(
            ...
            "  PROJECT_PATH=/Users/you/my-quill-project python examples/langgraph_demo.py"
        )
