"""End-to-end LangChain agent demo using quill-langchain.

Shows the minimum viable flow:
  1. Build a toolkit from a CLI-prepared Quill project.
  2. Pass its tools and system prompt straight into a LangChain agent.
  3. Ask a data question and let the agent decide which Quill tools to call.

The agent picks up the full Quill workflow from ``toolkit.system_prompt()``:
...
``.quill/memory/`` exists; otherwise the agent runs with the 3 runtime
tools only.

Prerequisites
=============
  - A CLI-prepared Quill project. Either follow the README quickstart, or
...
=====
    ...
    export PROJECT_PATH=/path/to/your-quill-project
...
try:
    ...
    sys.exit(
        ...
        "It is declared as a dependency of quill-langchain, but if you used an\n"
...
def main() -> None:
    ...
    if not project_path:
        sys.exit(
            ...
            "  PROJECT_PATH=/Users/you/my-quill-project python examples/langchain_demo.py"
        )
    ...
    #    quill_project.yml + target/mdl.json must exist, profile must resolve,
    ...
    # 3) Run and print the conversation. The agent decides which Quill tools
