...
def project_with_memory(duckdb_project):
    """Augment the duckdb_project fixture with an indexed .quill/memory dir."""
    memory_dir = duckdb_project / ".quill" / "memory"
...
def test_fetch_context_runs_against_real_lancedb(project_with_memory):
    toolkit = QuillToolkit.from_project(project_with_memory)
    fetch = next(t for t in toolkit.get_tools() if t.name == "quill_fetch_context")
...
def test_store_then_recall_round_trip(project_with_memory):
    toolkit = QuillToolkit.from_project(project_with_memory)
    store = next(t for t in toolkit.get_tools() if t.name == "quill_store_query")
    recall = next(t for t in toolkit.get_tools() if t.name == "quill_recall_queries")
