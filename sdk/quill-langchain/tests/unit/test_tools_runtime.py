"""Tests for runtime LLM-facing tools: quill_query, quill_dry_plan, quill_list_models."""
...
def test_get_tools_returns_three_runtime_tools_when_memory_disabled(
    ...
    assert names == ["quill_dry_plan", "quill_list_models", "quill_query"]


def test_quill_query_success_envelope(tmp_project, fake_active_profile):
    ...
    toolkit = QuillToolkit.from_project(tmp_project)
    tool = _get_tool(toolkit, "quill_query")

    with patch("quill_langchain._toolkit.QuillEngine", return_value=fake_engine):
...
def test_quill_query_rejects_limit_above_hard_cap(tmp_project, fake_active_profile):
    ...
    # Don't even need to patch QuillEngine — validation should fire before
...
def test_quill_query_rejects_zero_or_negative_limit(tmp_project, fake_active_profile):
...
def test_quill_query_error_envelope_on_quill_error(tmp_project, fake_active_profile):
    fake_engine = MagicMock(name="engine")
    fake_engine.query.side_effect = QuillError(
...
def test_quill_dry_plan_returns_sql_code_block(tmp_project, fake_active_profile):
    fake_engine = MagicMock(name="engine")
    ...
    toolkit = QuillToolkit.from_project(tmp_project)
    tool = _get_tool(toolkit, "quill_dry_plan")
...
def test_quill_list_models_returns_markdown_table(tmp_project, fake_active_profile):
    manifest = {
        "models": [
            {
                "name": "orders",
    ...
    toolkit = QuillToolkit.from_project(tmp_project)
    tool = _get_tool(toolkit, "quill_list_models")
