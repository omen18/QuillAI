"""Tests for QuillToolkit runtime API: query, dry_plan, dry_run."""
...
def test_query_invokes_quill_engine_with_resolved_manifest(
    tmp_project, fake_active_profile
):
    """toolkit.query reads the manifest fresh and delegates to QuillEngine.query."""
    ...
    with patch(
        "quill_langchain._toolkit.QuillEngine", return_value=fake_engine
    ) as engine_ctor:
...
def test_connector_is_reused_across_query_calls(tmp_project, fake_active_profile):
    ...
    with patch("quill_langchain._toolkit.QuillEngine", side_effect=make_engine):
...
def test_manifest_is_read_through_on_every_call(
    ...
    with patch(
        ...
        # Simulate `quill context build` updating the file.
