...
from quill_langchain.exceptions import MemoryNotEnabledError
...
def test_local_lancedb_provider_is_enabled(tmp_path):
    ...
    provider = LocalLanceDBMemoryProvider(memory_path=tmp_path / ".quill" / "memory")
    assert provider.enabled is True


def test_local_lancedb_provider_open_constructs_memory_store(tmp_path):
    """open() lazily constructs a quill.memory.MemoryStore at the given path."""
    memory_path = tmp_path / ".quill" / "memory"
    provider = LocalLanceDBMemoryProvider(memory_path=memory_path)

    fake_store = MagicMock(name="MemoryStore")
    with patch(
        "quill_langchain._providers.memory.MemoryStore",
