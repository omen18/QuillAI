...
Verifies that:
- Each tool registered by QuillToolkit.toolset() has the expected name,
...
def test_six_tools_when_memory_enabled_and_write_allowed(
    ...
    with patch("quill_pydantic._providers.memory.MemoryStore", return_value=fake_store):
        toolkit = QuillToolkit.from_project(tmp_project)
        ts = toolkit.toolset()
...
def mock_query_table():
    ...
    with patch.object(QuillToolkit, "query", return_value=table) as m:
        yield m


def test_test_model_drives_quill_list_models_call(
    tmp_project, fake_active_profile, mock_query_table
):
    """TestModel auto-fills any tool the agent could call. With
    call_tools=['quill_list_models'], the agent invokes it once."""
    toolkit = QuillToolkit.from_project(tmp_project)
    agent = Agent(
        TestModel(call_tools=["quill_list_models"]),
...
def test_model_retry_flow_surfaces_through_agent(tmp_project, fake_active_profile):
    """A QuillError raised inside a tool becomes a ModelRetry that the agent
    ...
    from quill.model.error import ErrorCode, ErrorPhase, QuillError  # noqa: PLC0415

    toolkit = QuillToolkit.from_project(tmp_project)
    with patch.object(
        QuillToolkit,
        "query",
        side_effect=QuillError(
        ...
        agent = Agent(
            TestModel(call_tools=["quill_query"]),
            toolsets=[toolkit.toolset()],
        )
        # TestModel sends bad-args by default — combined with our
        # QuillError-side-effect, this should produce a ModelRetry. The
