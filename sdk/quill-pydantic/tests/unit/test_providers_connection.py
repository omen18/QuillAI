...
def test_explicit_profile_kwarg_resolves_first(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "quill_pydantic._providers.connection.list_profiles",
        lambda: fake_profiles,
    )
    monkeypatch.setattr(
        "quill_pydantic._providers.connection.get_active_profile",
...
def test_project_config_profile_field_resolves_second(monkeypatch, tmp_path):
    """Layer 2: quill_project.yml's `profile:` field used when no explicit kwarg."""
    fake_profiles = {
    ...
    (tmp_path / "quill_project.yml").write_text("profile: from_project\n")
...
def test_unknown_profile_name_raises(monkeypatch, tmp_path):
    ...
    with pytest.raises(QuillToolkitInitError, match="profile.*not found"):
