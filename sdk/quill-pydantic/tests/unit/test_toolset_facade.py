"""Unit tests for QuillToolkit.toolset() — the Pydantic AI adapter facade."""
...
def test_toolset_returns_three_runtime_tools_when_memory_disabled(
    ...
    assert sorted(names) == ["quill_dry_plan", "quill_list_models", "quill_query"]
...
def test_toolset_includes_memory_tools_when_memory_dir_exists(
    ...
    """When .quill/memory/ exists, the toolset gains the 3 memory tools."""
...
def test_toolset_include_memory_write_ignored_when_memory_disabled(
    ...
    toolkit = QuillToolkit.from_project(tmp_project)  # no .quill/memory
