...
def test_query_invokes_quill_engine_with_resolved_manifest(
    ...
    with patch(
        "quill_pydantic._toolkit.QuillEngine", return_value=fake_engine
    ) as engine_ctor:
...
def test_connector_is_reused_across_query_calls(tmp_project, fake_active_profile):
    ...
    with patch("quill_pydantic._toolkit.QuillEngine", side_effect=make_engine):
...
def test_dry_plan_delegates_to_engine(tmp_project, fake_active_profile):
    ...
    with patch("quill_pydantic._toolkit.QuillEngine", return_value=fake_engine):
