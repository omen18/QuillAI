...
def test_project_mdl_source_raises_on_missing_target(tmp_path):
    """A missing target/mdl.json raises QuillToolkitInitError when loading."""
    source = ProjectMDLSource(project_path=tmp_path)
...
def test_project_mdl_source_normalizes_malformed_json_to_init_error(tmp_path):
    """Malformed mdl.json must surface as QuillToolkitInitError, not raw JSONDecodeError,
    ...
    with pytest.raises(QuillToolkitInitError, match="not valid JSON"):
        source.load_manifest()
