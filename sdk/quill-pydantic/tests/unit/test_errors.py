"""Tests for QuillError → ModelRetry mapping.

Mirrors quill-langchain's test_envelope.py but adapts to Pydantic AI's
...
from quill_pydantic._errors import (
    METADATA_CAP_BYTES,
    redact_secrets,
    should_propagate,
    to_model_retry,
...
def test_should_propagate_returns_true_for_infra_codes(code):
    exc = QuillError(error_code=code, message="boom")
...
def test_to_model_retry_no_phase_still_builds_a_message():
    ...
    exc = QuillError(error_code=ErrorCode.GENERIC_USER_ERROR, message="something off")
    retry = to_model_retry(exc)
...
def test_to_model_retry_redacts_secrets_in_metadata():
    ...
    from quill_pydantic._errors import _build_message  # noqa: PLC0415
