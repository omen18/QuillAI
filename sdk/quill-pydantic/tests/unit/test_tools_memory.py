...
- QuillError → ModelRetry mapping fires
- quill_store_query uses retries=0 (write failures don't retry)
"""
...
from quill_pydantic._tools_memory import build_memory_toolset
...
def test_build_memory_toolset_drops_store_when_include_write_false():
    ...
    assert sorted(names) == ["quill_fetch_context", "quill_recall_queries"]
...
def test_fetch_context_returns_typed_full_payload():
    ...
    ts = build_memory_toolset(toolkit, include_write=True, takes_ctx=False)
    fn = _get_tool(ts, "quill_fetch_context")
...
def test_fetch_context_wraps_quill_error_as_model_retry():
    toolkit = _mock_toolkit()
    toolkit.memory.fetch.side_effect = QuillError(
...
def test_recall_returns_typed_pair_list():
    ...
    ts = build_memory_toolset(toolkit, include_write=True, takes_ctx=False)
    fn = _get_tool(ts, "quill_recall_queries")
...
def test_store_returns_success_string():
    toolkit = _mock_toolkit()
    ts = build_memory_toolset(toolkit, include_write=True, takes_ctx=False)
    fn = _get_tool(ts, "quill_store_query")
...
def test_store_query_retries_zero():
    ...
    toolkit = _mock_toolkit()
    ts = build_memory_toolset(toolkit, include_write=True, takes_ctx=False)
    entry = _entry(ts, "quill_store_query")
