...
def test_instructions_includes_workflow_section(tmp_project, fake_active_profile):
    ...
    # Workflow rule about Quill model names being preferred over raw tables.
...
def test_instructions_lists_enabled_tools_in_summary(tmp_project, fake_active_profile):
    toolkit = QuillToolkit.from_project(tmp_project)
    prompt = toolkit.instructions()
    assert "quill_query" in prompt
    assert "quill_dry_plan" in prompt
    assert "quill_list_models" in prompt
...
def test_instructions_omits_memory_tools_when_disabled(
    ...
    toolkit = QuillToolkit.from_project(tmp_project)
    prompt = toolkit.instructions()
    assert "quill_fetch_context" not in prompt
    assert "quill_recall_queries" not in prompt
    assert "quill_store_query" not in prompt
...
def test_instructions_includes_memory_tools_when_enabled(
    ...
    assert "quill_fetch_context" in prompt
    assert "quill_recall_queries" in prompt
    assert "quill_store_query" in prompt
...
def test_memory_workflow_uses_strong_default_language(tmp_project, fake_active_profile):
    ...
    project = tmp_project
    (project / ".quill" / "memory").mkdir(parents=True)
    fake_store = MagicMock(name="MemoryStore")
...
def test_instructions_respects_include_memory_write_false(
    ...
    """When the caller passes a tool list with `quill_store_query` filtered out,
