"""Unit tests for the 3 runtime tools (quill_query, quill_dry_plan, quill_list_models).
...
- Error path: QuillError → ModelRetry mapping fires
- Propagate-class QuillError re-raises instead of becoming ModelRetry
- `limit` clamping for quill_query
"""
...
from quill_pydantic._tools import MAX_QUERY_ROWS, build_runtime_toolset


def _mock_toolkit(*, manifest=None, query_table=None, dry_plan_sql="EXPANDED SQL"):
    """Build a MagicMock that quacks like a QuillToolkit for tool tests."""
    tk = MagicMock(name="QuillToolkit")
...
def test_build_runtime_toolset_registers_three_tools():
    ...
    assert sorted(tool_names) == ["quill_dry_plan", "quill_list_models", "quill_query"]
...
def test_quill_query_returns_typed_result():
    toolkit = _mock_toolkit()
    ts = build_runtime_toolset(toolkit, takes_ctx=False)
    fn = _get_tool(ts, "quill_query")

    result = fn(sql="SELECT * FROM orders", limit=100)

    assert isinstance(result, QuillQueryResult)
    assert result.columns == ["id", "name"]
    assert result.row_count == 2
    assert result.truncated is False


def test_quill_query_rejects_limit_below_one():
    toolkit = _mock_toolkit()
    ts = build_runtime_toolset(toolkit, takes_ctx=False)
    fn = _get_tool(ts, "quill_query")
...
def test_quill_query_rejects_limit_above_max():
    toolkit = _mock_toolkit()
    ts = build_runtime_toolset(toolkit, takes_ctx=False)
    fn = _get_tool(ts, "quill_query")
...
def test_quill_query_wraps_quill_error_as_model_retry():
    toolkit = _mock_toolkit()
    toolkit.query.side_effect = QuillError(
...
def test_quill_query_propagates_infra_errors():
    """GET_CONNECTION_ERROR is in propagate set — re-raises as QuillError, not ModelRetry."""
    toolkit = _mock_toolkit()
    toolkit.query.side_effect = QuillError(
    ...
    with pytest.raises(QuillError, match="cannot connect"):
        fn(sql="SELECT 1", limit=10)


def test_quill_query_truncated_when_engine_returns_more_than_limit():
...
def test_quill_dry_plan_returns_string():
    toolkit = _mock_toolkit(dry_plan_sql="WITH a AS (SELECT 1) SELECT * FROM a")
    ts = build_runtime_toolset(toolkit, takes_ctx=False)
    fn = _get_tool(ts, "quill_dry_plan")
...
def test_quill_dry_plan_wraps_error_as_model_retry():
    toolkit = _mock_toolkit()
    toolkit.dry_plan.side_effect = QuillError(
        error_code=ErrorCode.INVALID_SQL,
...
def test_quill_list_models_returns_typed_summaries():
    manifest = {
        "models": [
            {
                "name": "orders",
    ...
    ts = build_runtime_toolset(toolkit, takes_ctx=False)
    fn = _get_tool(ts, "quill_list_models")
...
def test_quill_list_models_empty_manifest():
...
def test_quill_list_models_wraps_error_as_model_retry():
    toolkit = _mock_toolkit()
    toolkit._mdl_source.load_manifest.side_effect = QuillError(
