...
return shapes verbatim (see core/quill/src/quill/memory/store.py). Tests
...
from quill_pydantic._models import (
    FetchContextResult,
    ModelSummary,
    RecalledPair,
    QuillQueryResult,
...
def test_quill_query_result_round_trip():
    payload = QuillQueryResult(
        columns=["id", "name"],
    ...
    restored = QuillQueryResult.model_validate(dumped)
    assert restored == payload


def test_quill_query_result_rejects_negative_row_count():
    with pytest.raises(ValidationError, match="row_count"):
        QuillQueryResult(columns=[], rows=[], row_count=-1, truncated=False)


def test_quill_query_result_row_count_must_equal_len_rows():
...
def test_quill_query_result_truncated_flag_marks_limit_overflow():
    payload = QuillQueryResult(
