...
```bash
quill profile add my_project --datasource duckdb   # or mysql, postgres, ...
quill context init
...
`quill-pydantic` exposes datasource and memory extras that pass through to the
matching `quill` extras, so you only have to install once:

```bash
# Match the datasource your quill_project.yml uses (DuckDB needs no extra):
pip install "quill-pydantic[mysql]"
pip install "quill-pydantic[postgres,memory]"
pip install "quill-pydantic[bigquery,memory]"
...
pip install "quill-pydantic[all,memory]"
```

If `quill` is already installed (e.g. you use the CLI), the bare
`pip install quill-pydantic` is enough — your existing extras carry over.
...
- **6 LLM-facing tools** (3 runtime + 3 memory when `.quill/memory/` exists):
  - `quill_query` — execute SQL through Quill's semantic layer, returns
    a `QuillQueryResult` (typed Pydantic model)
  - `quill_dry_plan` — plan SQL without execution; verifies it targets MDL
    models correctly
  - `quill_list_models` — list project models with column counts and
    descriptions
  - `quill_fetch_context` — retrieve schema/business context for a question
  - `quill_recall_queries` — surface similar past NL→SQL pairs as few-shot
    examples
...
DuckDB files) propagate as `QuillError` for outer code to handle.
...
`include_memory_write=False` removes `quill_store_query` from the toolset
while keeping `quill_fetch_context` and `quill_recall_queries`. Use this for
shared / curated memory stores.

`takes_ctx=True` exposes `ctx: RunContext` as the first parameter of every
tool. Use this when mixing quill tools with your own `deps_type=`-typed
...
- **Sync direct API only.** `aquery` / `adry_plan` etc. are not provided —
  ...
  underlying `QuillEngine` is sync I/O so an async wrapper would be fake-async
  with no real concurrency benefit. Revisit when Core ships an async-native
  engine.
- **One toolkit per agent.** If you need to query multiple Quill projects,
  build separate toolkits + agents and federate in Python.
- **Memory is auto-detected** from `.quill/memory/` and there is no kwarg to
  override. To enable, run `quill memory index`; to disable, delete the
  directory.
- **No hot reload mechanism.** `target/mdl.json` is re-read on every tool
  call so `quill context build` updates are picked up automatically. Profile
  changes require constructing a new toolkit.
- **Don't run `quill memory index` while an agent is using the same project.**
