"""Memory tools (quill_fetch_context, quill_recall_queries, quill_store_query).

Three tools registered into a FunctionToolset when the project has a
``.quill/memory/`` directory. ``include_write=False`` keeps the two
read-only tools and drops ``quill_store_query`` — useful for shared /
curated memory stores.

``quill_store_query`` is registered with ``retries=0``: write failures
...
from quill_pydantic._models import FetchContextResult, RecalledPair
...
def _register_fetch_context(
    ...
    if takes_ctx:
        ...
        def quill_fetch_context(
            ...
            Call this BEFORE writing SQL so you query the correct Quill
...
def _register_recall_queries(
    ...
    if takes_ctx:
        ...
        @toolset.tool_plain(retries=2)
        def quill_recall_queries(question: str, limit: int = 3) -> list[RecalledPair]:
            """Recall up to *limit* past NL→SQL pairs similar to *question*.
...
def _register_store_query(
    ...
    if takes_ctx:
        ...
        def quill_store_query(
            ...
            Call this AFTER quill_query succeeds and the result was
...
def _run_recall(toolkit: QuillToolkit, question: str, limit: int) -> list[RecalledPair]:
    try:
