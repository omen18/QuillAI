...
from quill_pydantic._providers.connection import ProfileConnectionProvider
from quill_pydantic._providers.mdl_source import ProjectMDLSource
from quill_pydantic._providers.memory import (
    LocalLanceDBMemoryProvider,
    NoopMemoryProvider,
)
from quill_pydantic.exceptions import QuillToolkitInitError
...
class QuillToolkit:
    """Adapter that exposes an existing Quill project as Pydantic AI tools."""
    ...
    def memory(self):
        if not hasattr(self, "_memory_api"):
            from quill_pydantic._memory_api import _MemoryAPI  # noqa: PLC0415
    ...
    def toolset(
        ...
        *,
        ...
        ``.quill/memory/`` directory in the project).
        ``include_memory_write=False`` removes ``quill_store_query`` while
        keeping the read-only memory tools (``quill_fetch_context``,
        ``quill_recall_queries``). When memory is disabled,
        ...
        its first parameter. Use this when mixing quill tools with other
        ``deps_type=...`` tools in the same agent; the context object is
        ignored internally (toolkit captures its own state).
        """
        from quill_pydantic._tools import build_runtime_toolset  # noqa: PLC0415

        ts = build_runtime_toolset(self, takes_ctx=takes_ctx)
        if self._memory.enabled:
            from quill_pydantic._tools_memory import (  # noqa: PLC0415
    ...
    def instructions(self, *, toolset: object | None = None) -> str:
        """Return a Quill-aware instructions string suitable for Pydantic AI.

        Composition mirrors the quill-langchain SDK:
        ...
        """
        from quill_pydantic._instructions import build_instructions  # noqa: PLC0415
