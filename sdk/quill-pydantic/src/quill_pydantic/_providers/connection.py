"""Connection providers resolve a Quill profile to (datasource, connection_info).

Layer order (highest priority first):
  1. ``explicit_profile=`` kwarg passed to ``QuillToolkit.from_project``
  2. ``profile:`` field in the project's ``quill_project.yml``
  3. The user's globally active profile (``quill profile switch ...``)
...
from quill.context import load_project_config
from quill.profile import (  # noqa: F401  re-exported for monkeypatching in tests
...
class ProfileConnectionProvider:
    ...
    def _resolve_profile(self, explicit: str | None) -> dict[str, Any]:
        ...
        # Layer 2: profile name from quill_project.yml
        ...
        if not active:
            raise QuillToolkitInitError(
                "no active Quill profile found. "
                "Run `quill profile add` and `quill profile switch` first."
            )
        return dict(active)
    ...
    def _lookup_named_profile(self, name: str) -> dict[str, Any]:
        ...
        if name not in profiles:
            raise QuillToolkitInitError(
                f"profile {name!r} not found in ~/.quill/profiles.yml. "
