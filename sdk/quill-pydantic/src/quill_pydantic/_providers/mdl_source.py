...
prepared Quill project directory. Each ``load_manifest()`` call re-reads the
file from disk so that ``quill context build`` updates by an external CLI run
    ...
    def load_manifest(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise QuillToolkitInitError(
                ...
                "Re-run `quill context build` to regenerate it."
            ) from exc
