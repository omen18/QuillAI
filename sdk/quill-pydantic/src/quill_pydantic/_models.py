...
``core/quill/src/quill/memory/store.py`` ``get_context`` and
...
class QuillQueryResult(BaseModel):
    """Result of ``quill_query``: columns, rows, and overflow flag.
    ...
    def _row_count_matches_rows(self) -> QuillQueryResult:
...
class ModelSummary(BaseModel):
    """Per-model entry returned by ``quill_list_models``.
...
class FetchContextResult(BaseModel):
    """Result of ``quill_fetch_context``, matching Core's get_context shape.
...
class RecalledPair(BaseModel):
    """One NL→SQL pair returned by ``quill_recall_queries``.
