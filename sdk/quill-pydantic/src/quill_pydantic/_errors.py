"""QuillError → ModelRetry mapping for Pydantic AI tools.
...
Quill errors split into two classes:
...
def should_propagate(exc: QuillError) -> bool:
    """Whether this QuillError should bubble past the agent instead of
...
def _build_message(exc: QuillError) -> str:
    ...
    if phase == ErrorPhase.SQL_PARSING:
        ...
        framing = (
            ...
            "Verify the model name with quill_list_models and retry."
        )
...
def to_model_retry(exc: QuillError) -> ModelRetry:
    """Convert a retry-class QuillError into a ModelRetry the LLM can act on.
