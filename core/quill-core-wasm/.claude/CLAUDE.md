...
Quill Engine compiled to WebAssembly for browser-native analytics. Runs SQL queries on Parquet/CSV/JSON data through the MDL semantic layer entirely client-side, powered by DataFusion.

## Why a Separate Crate?

Uses **upstream DataFusion** (crates.io v53), not the upstream fork. The WASM version executes queries directly via DataFusion — no SQL unparser or dialect transpilation needed. Kept outside the `quill-core/` workspace to avoid dependency conflicts.

Shared code: `quill-core-base` (manifest types, no DataFusion dependency) and `quill-core` semantic layer (MDL analysis rules).
...
Browser JS
  ...
  ├── loadMDL(mdl_json, source)    → AnalyzedQuillMDL + table resolution
...
- `src/lib.rs` — Single-file crate with `QuillEngine` struct:
  ...
  - `load_mdl()` → Parse manifest, analyze with quill-core, register tables by mode
  - `query()` → Apply MDL analyzer rules → DataFusion execute → JSON string
- `sdk/src/index.ts` — TypeScript wrapper (`QuillEngine` class) for npm package
- `sdk/src/quill_core_wasm.d.ts` — Hand-maintained type stubs for wasm-bindgen output
...
- **quill-core** (path: `../quill-core/core`) — semantic layer, `default-features = false`
- **quill-core-base** (path: `../quill-core-base`) — shared manifest types
...
## npm Package (quill-core-wasm)

`package.json` defines the npm package. TypeScript SDK wraps the raw wasm-bindgen API:
- `QuillEngine.init(options?)` — load WASM binary, create engine
