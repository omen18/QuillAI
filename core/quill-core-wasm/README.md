...
`QuillEngine.init()` defaults to fetching the WASM binary via `import.meta.url`, which in Node resolves to a `file://` URL. Node's `undici` fetch does not support `file://` and `init()` will throw. Pass the binary directly as a `BufferSource`:
...
const buf = readFileSync(
  'node_modules/@quill/quill-core-wasm/dist/quill_core_wasm_bg.wasm'
);
const engine = await QuillEngine.init({
...
the examples import directly from `pkg/quill_core_wasm.js`.
...
|--------|------|-------------|
| `wasmUrl` | `string \| URL \| BufferSource` | WASM binary source. Defaults to sibling `quill_core_wasm_bg.wasm` via `import.meta.url`. |
...
- In Node, pass `wasmUrl: BufferSource` to `QuillEngine.init()` — Node's fetch can't load `file://` URLs.
