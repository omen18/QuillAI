/**
 * Integration tests for the quill-core-wasm TypeScript wrapper.
 *
...
const wasmBytes = readFileSync(resolve(distDir, "quill_core_wasm_bg.wasm"));

/** Dynamic import of the built SDK (must exist in dist/) */
const { QuillEngine } = await import(resolve(distDir, "index.js"));
...
describe("QuillEngine.init", () => {
  it("creates an engine instance from WASM bytes", async () => {
    const engine = await QuillEngine.init({ wasmUrl: wasmBytes });
  ...
  it("creates multiple independent engine instances", async () => {
    const engine1 = await QuillEngine.init({ wasmUrl: wasmBytes });
    const engine2 = await QuillEngine.init({ wasmUrl: wasmBytes });
