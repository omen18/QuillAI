//! # quill-core-wasm
//!
//! Quill Engine compiled to WebAssembly for browser-native analytics.
//!
//! This crate provides a WASM-compatible version of the Quill Engine that runs
//! entirely in the browser. It uses **upstream DataFusion** (not the upstream fork)
...
//! - **M3**: quill-core semantic layer (MDL plan rewriting)
...
pub struct QuillEngine {
    ...
    analyzed_mdl: Option<std::sync::Arc<quill_core::mdl::AnalyzedQuillMDL>>,
...
#[wasm_bindgen]
impl QuillEngine {
    /// Initialize a new QuillEngine instance.
    ///
    ...
    /// comparisons match `create_quill_ctx` on the native side.
    #[wasm_bindgen(constructor)]
    pub fn new() -> Result<QuillEngine, JsError> {
    ...
    ///
    /// Parses the MDL JSON, builds the semantic layer (AnalyzedQuillMDL),
    /// and reconfigures the SessionContext with Quill analyzer rules in
    ...
    /// (typically `quill.public`), so queries can reference models without a
    /// catalog prefix.
    #[wasm_bindgen(js_name = loadMDL)]
    pub async fn load_mdl(&mut self, mdl_json: &str, source: &str) -> Result<(), JsError> {
        use std::collections::HashMap;
        use std::sync::Arc;
        use quill_core::mdl::context::{apply_quill_on_ctx, Mode};
        use quill_core::mdl::AnalyzedQuillMDL;
        use quill_core_base::mdl::manifest::Manifest;
        ...
        let analyzed_mdl: Arc<AnalyzedQuillMDL> = if is_url_source(source) {
        ...
        // Clone the Arc so `apply_quill_on_ctx` can take ownership while
        // we keep a handle on `self` for cubeQuery/listCubes access.
        let new_ctx = apply_quill_on_ctx(
            &self.ctx,
    ...
    async fn load_mdl_url_mode(
        &mut self,
        manifest: &quill_core_base::mdl::manifest::Manifest,
        source: &str,
    ) -> Result<std::sync::Arc<quill_core::mdl::AnalyzedQuillMDL>, JsError> {
        ...
        for (name, table_ref, table) in staged {
            ...
            // `QuillMDL::get_table` finds it during plan analysis.
            register_tables.insert(table_ref, table);
        }

        AnalyzedQuillMDL::analyze_with_tables(manifest.clone(), register_tables)
            .map(Arc::new)
    ...
    async fn load_mdl_fallback(
        ...
        if use_url_tables {
            ...
            AnalyzedQuillMDL::analyze_with_url_tables(manifest, &self.ctx)
                .await
                .map(Arc::new)
            ...
            AnalyzedQuillMDL::analyze_with_tables(manifest, register_tables)
                .map(Arc::new)
                .map_err(|e| JsError::new(&format!("Failed to analyze MDL: {e}")))
        }
    }
    ...
    /// by the Python binding), translates it to SQL via quill-core, and
    ...
    pub async fn cube_query(&self, cube_query_json: &str) -> Result<String, JsError> {
        ...
        let quill_mdl = analyzed.quill_mdl();
        let manifest = &quill_mdl.manifest;

        let query: quill_core::mdl::CubeQuery = serde_json::from_str(cube_query_json)
            .map_err(|e| JsError::new(&format!("Invalid CubeQuery JSON: {e}")))?;

        let sql = quill_core::mdl::cube_query_to_sql(&query, manifest)
...
impl Default for QuillEngine {
    fn default() -> Self {
        Self::new().expect("Failed to create default QuillEngine")
    }
}
...
mod tests {
    ...
    async fn test_basic_query() {
        let engine = QuillEngine::new().unwrap();
    ...
    fn minimal_mdl(model_name: &str, physical_table: &str) -> String {
        ...
        // `catalog`/`schema` default to `quill`/`public` so loadMDL will align
    ...
    async fn test_bare_model_name_query() {
        // Proves §2.5.2: bare model names resolve without a `quill.public.`
        // prefix after loadMDL aligns the default catalog/schema.
        let mut engine = QuillEngine::new().unwrap();
        engine
            .register_json(
                "customers",
                r#"[
        ...
        // Bare model name — no `quill.public.` prefix.
        let result = engine
