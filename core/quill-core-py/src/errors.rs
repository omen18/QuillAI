...
use thiserror::Error;
use quill_core::DataFusionError;
use quill_core::QuillError;
use quill_core_base::mdl::ParsedDataSourceError;
    ...
    fn from(err: DataFusionError) -> Self {
        if let DataFusionError::Context(_, ee) = &err {
            if let DataFusionError::External(we) = ee.as_ref() {
                if let Some(we) = we.downcast_ref::<QuillError>() {
...
impl From<quill_core::parser::ParserError> for CoreError {
    fn from(err: quill_core::parser::ParserError) -> Self {
