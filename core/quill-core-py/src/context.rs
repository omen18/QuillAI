...
use quill_core::array::{AsArray, GenericByteArray};
use quill_core::ast::{
    visit_statements_mut, Expr, LimitClause, Statement, Value, ValueWithSpan,
};
use quill_core::datatypes::GenericStringType;
use quill_core::dialect::GenericDialect;
use quill_core::ipc::writer::StreamWriter;
use quill_core::mdl::context::apply_quill_on_ctx;
use quill_core::mdl::function::{
    ByPassAggregateUDF, ByPassScalarUDF, ByPassWindowFunction, FunctionType,
    RemoteFunction,
};
use quill_core::{
    mdl, AggregateUDF, AnalyzedQuillMDL, CsvReadOptions, ParquetReadOptions, ScalarUDF,
    SessionConfig, WindowUDF,
};
use quill_core_base::mdl::DataSource;

/// The Python wrapper for the Quill Core session context.
...
pub struct PySessionContext {
    ...
    base_ctx: quill_core::SessionContext,
    ctx: quill_core::SessionContext,
    exec_ctx: quill_core::SessionContext,
    mdl: Arc<AnalyzedQuillMDL>,
    ...
    /// (`apply_quill_on_ctx` -> `register_table_with_mdl`), while `query`,
...
impl Default for PySessionContext {
    fn default() -> Self {
        let ctx = quill_core::SessionContext::new();
        Self {
            base_ctx: ctx.clone(),
            ctx: ctx.clone(),
            exec_ctx: ctx,
            mdl: Arc::new(AnalyzedQuillMDL::default()),
...
impl PySessionContext {
    ...
    pub fn new(
        ...
        let Some(mdl_base64) = mdl_base64 else {
            ...
            let ctx = quill_core::mdl::create_quill_ctx(Some(config), data_source.as_ref());
...

        {
            ...
            match AnalyzedQuillMDL::analyze(
                manifest,
                ...
                Ok(analyzed_mdl) => {
                    ...
                    let unparser_ctx = py
                        .detach(|| {
                            runtime.block_on(apply_quill_on_ctx(
                                &ctx,
    ...
    /// Transform the given Quill SQL to the equivalent Planned SQL.
    ...
    pub fn pushdown_limit(&self, sql: &str, limit: Option<usize>) -> PyResult<String> {
        ...
        let mut statements =
            quill_core::parser::Parser::parse_sql(&GenericDialect {}, sql)
                .map_err(CoreError::from)?;
        if statements.len() != 1 {
        ...
        py.detach(|| {
            ...
            let (batches, schema) = self
                ...
                .block_on(async {
                    ...
                    Ok::<_, quill_core::DataFusionError>((batches, schema))
                })
                .map_err(CoreError::from)?;
    ...
    pub fn dry_run(&self, py: Python<'_>, sql: &str) -> PyResult<String> {
        ...
        let result = py
            ...
            .map_err(CoreError::from)?;
        Ok(quill_core::util::pretty::pretty_format_batches(&result)
            .map_err(|e| CoreError::new(&e.to_string()))?
            .to_string())
    }
