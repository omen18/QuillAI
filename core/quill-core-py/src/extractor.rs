...
use std::sync::Arc;
use quill_core::ast::{visit_relations, ObjectName};
use quill_core::dialect::GenericDialect;
use quill_core::mdl::manifest::{Cube, Model, Relationship, View};
use quill_core::mdl::QuillMDL;
use quill_core::parser::Parser;
use quill_core_base::mdl::Manifest;
...
fn resolve_used_table_names(mdl: &QuillMDL, sql: &str) -> Result<Vec<String>, CoreError> {
    let mut config = quill_core::SessionConfig::new();
    config.options_mut().sql_parser.enable_ident_normalization = false;
    let ctx_state = quill_core::SessionContext::new_with_config(config).state();
    ctx_state
...
/// used by quill-core so session-property placeholders (`@session_id`) are
/// tolerated. Tables qualified with a non-matching catalog/schema are ignored,
/// mirroring `resolve_used_table_names`.
fn resolve_condition_models(mdl: &QuillMDL, condition: &str) -> Vec<String> {
...
fn matched_table_name(mdl: &QuillMDL, name: &ObjectName) -> Option<String> {
...
fn extract_models(mdl: &QuillMDL, used_datasets: &[String]) -> Vec<Arc<Model>> {
    ...
    while let Some(dataset_name) = stack.pop() {
        if let Some(model) = mdl.get_model(&dataset_name) {
            ...
            // otherwise the RLAC subquery analysis in quill-core fails.
...
/// dropping orphaned cubes is therefore safe and avoids quill-core failing MDL
...
mod tests {
    ...
    use quill_core::mdl::manifest::{DataSource, JoinType};
    use quill_core_base::mdl::builder::{
    ...
    pub fn mdl_base64() -> String {
        ...
        // model) so quill-core does not fail MDL analysis with "baseObject is not
