...
use quill_core::mdl::context::{apply_quill_on_ctx, Mode, SessionPropertiesRef};
use quill_core::mdl::manifest::JoinType;
use quill_core::mdl::{create_quill_ctx, AnalyzedQuillMDL};
...
impl TestContext {
    pub fn new(ctx: SessionContext, analyzed_quill_mdl: Arc<AnalyzedQuillMDL>) -> Self {
        Self {
            ctx,
            analyzed_quill_mdl,
            test_dir: None,
    ...
    pub async fn try_new_for_test_file(relative_path: &Path) -> Option<Self> {
        ...
        let ctx = create_quill_ctx(Some(config), None);
        ...
        match file_name {
            ...
            _ => {
                ...
                let mdl = Arc::new(AnalyzedQuillMDL::default());
    ...
    pub fn analyzed_quill_mdl(&self) -> &Arc<AnalyzedQuillMDL> {
...
async fn register_ecommerce_mdl(
    ctx: &SessionContext,
) -> Result<(SessionContext, Arc<AnalyzedQuillMDL>)> {
    let manifest = ManifestBuilder::new()
        ...
        .view(
            ViewBuilder::new("Customer_view")
                .statement(r#"select * from quill.public."Customers""#)
                .build(),
        )
        .view(ViewBuilder::new("Revenue_orders").statement(r#"select "Order_id", sum("Price") from quill.public."Order_items" group by "Order_id""#).build())
        .view(
            ViewBuilder::new("Revenue_orders_alias")
                .statement(r#"select "Order_id" as "Order_id", sum("Price") as "Totalprice" from quill.public."Order_items" group by "Order_id""#)
                .build())
        .build();
    ...
    let analyzed_mdl = Arc::new(AnalyzedQuillMDL::analyze_with_tables(
        manifest,
        register_tables,
    )?);
    let ctx = apply_quill_on_ctx(
