...
use crate::mdl::{AnalyzedQuillMDL, ColumnReference, Dataset, SessionStateRef};
...
/// Because the number of the CompoundIdentifier elements length would be greater than 3 in Quill Core,
...
pub fn create_quill_calculated_field_expr(
    ...
    let Some(required_fields) = analyzed_quill_mdl
        .lineage
        .required_fields_map
        .get(&qualified_col)
    else {
    ...
    let Some(schema) = models
        .into_iter()
        .map(|m| analyzed_quill_mdl.quill_mdl().get_model(&m))
        .filter(|m| m.is_some())
...
pub(crate) fn create_remote_expr_for_model(
    ...
    let schema = dataset.to_remote_schema(
        Some(analyzed_quill_mdl.quill_mdl().get_register_tables()),
...
/// Create the Logical Expr for the remote column.
pub(crate) fn create_quill_expr_for_model(
    expr: &str,
    model: Arc<Model>,
    session_state: SessionStateRef,
) -> Result<Expr> {
...
pub fn to_field(column: &quill_core_base::mdl::Column) -> Result<Field> {
...
pub fn to_remote_field(
    column: &quill_core_base::mdl::Column,
    session_state: SessionStateRef,
) -> Result<Vec<Field>> {
...
mod tests {
    ...
    #[test]
    fn test_create_quill_expr() -> Result<()> {
        ...
        let column_rf = analyzed_mdl
            ...
            .get(&from_qualified_name(
                &analyzed_mdl.quill_mdl,
                "orders",
                "customer_name",
            ))
            .unwrap();
        let expr = super::create_quill_calculated_field_expr(
            column_rf.clone(),
            analyzed_mdl.clone(),
            ctx.state_ref(),
        )?;
    ...
    #[test]
    fn test_create_quill_expr_non_relationship() -> Result<()> {
        let test_data: PathBuf =
            [env!("CARGO_MANIFEST_DIR"), "tests", "data", "mdl.json"]
                .iter()
                .collect();
    ...
    #[test]
    fn test_create_quill_expr_for_model() -> Result<()> {
        let test_data: PathBuf =
            [env!("CARGO_MANIFEST_DIR"), "tests", "data", "mdl.json"]
                .iter()
                .collect();
        ...
        let ctx = SessionContext::new();
        let model = analyzed_mdl.quill_mdl().get_model("customer").unwrap();
        let expr = super::create_quill_expr_for_model(
            "c_name",
            Arc::clone(&model),
            ctx.state_ref(),
        )?;
