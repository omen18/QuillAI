...
use dialect::QuillDialect;
use log::{debug, info, warn};
use manifest::Relationship;
use parking_lot::RwLock;
use std::hash::Hash;
use std::{collections::HashMap, sync::Arc};
use quill_core_base::mdl::DataSource;

pub mod builder {
    pub use quill_core_base::mdl::builder::*;
}
pub mod context;
pub(crate) mod cube;
...
pub mod lineage;
pub mod manifest {
    pub use quill_core_base::mdl::manifest::*;
}
...
pub struct AnalyzedQuillMDL {
    pub quill_mdl: Arc<QuillMDL>,
    pub lineage: Arc<lineage::Lineage>,
}

impl Hash for AnalyzedQuillMDL {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.quill_mdl.hash(state);
    }
}

impl Default for AnalyzedQuillMDL {
    fn default() -> Self {
        let manifest = ManifestBuilder::default().build();
        let quill_mdl = QuillMDL::new(manifest);
        let lineage = lineage::Lineage::new(&quill_mdl).unwrap();
        AnalyzedQuillMDL {
            quill_mdl: Arc::new(quill_mdl),
            lineage: Arc::new(lineage),
        }
    }
}

impl AnalyzedQuillMDL {
    pub fn analyze(
        manifest: Manifest,
        properties: SessionPropertiesRef,
        mode: Mode,
    ) -> Result<Self> {
        let quill_mdl = Arc::new(QuillMDL::infer_and_register_remote_table(
            manifest, properties, mode,
        )?);
        lineage::validate_cubes(&quill_mdl)?;
        let lineage = Arc::new(lineage::Lineage::new(&quill_mdl)?);
        Ok(AnalyzedQuillMDL { quill_mdl, lineage })
    }
    ...
    pub fn analyze_with_tables(
        ...
        let mut quill_mdl = QuillMDL::new(manifest);
        for (name, table) in register_tables {
            quill_mdl.register_table(name, table);
        }
        lineage::validate_cubes(&quill_mdl)?;
        let lineage = lineage::Lineage::new(&quill_mdl)?;
        Ok(AnalyzedQuillMDL {
            quill_mdl: Arc::new(quill_mdl),
            lineage: Arc::new(lineage),
        })
    ...
    pub async fn analyze_with_url_tables(
        ...
        if let Some(data_source) = quill_mdl.data_source() {
            match data_source {
        ...
        let state = ctx.state();
        for model in quill_mdl.models().to_vec() {
            ...
            quill_mdl.register_table(table_reference.to_string(), Arc::new(table));
        }
    ...
    pub fn quill_mdl(&self) -> Arc<QuillMDL> {
        Arc::clone(&self.quill_mdl)
    }
...
impl QuillMDL {
    ...
    pub fn new_ref(manifest: Manifest) -> Arc<Self> {
        Arc::new(QuillMDL::new(manifest))
    }

    /// Create a QuillMDL from a manifest and register the table reference of the model as a remote table.
    ...
    pub fn infer_and_register_remote_table(
        ...
        use quill_core_base::mdl::ModelSource;

        let mut mdl = QuillMDL::new(manifest);
        let sources: Vec<_> =
            mdl.models()
                .iter()
                .map(|model| match model.source() {
                    ModelSource::TableReference => {
                        ...
                        let datasource = QuillDataSource::new_with_schema(schema);
    ...
    fn infer_source_column(column: &Column) -> Result<Option<Field>> {
        ...
        if let Some(expression) = column.expression() {
            let ExprWithAlias { expr, alias } = QuillMDL::sql_to_expr(expression)?;
...
pub fn transform_sql(
    analyzed_mdl: Arc<AnalyzedQuillMDL>,
    ...
    runtime.block_on(transform_sql_with_ctx(
        &create_quill_ctx(None, analyzed_mdl.quill_mdl().data_source().as_ref()),
        analyzed_mdl,
        remote_functions,
        Arc::new(properties),
        sql,
...
pub async fn transform_sql_with_ctx(
    ...
    info!("quill-core received SQL: {sql}");
    ...
    let plan = match ctx.state().create_logical_plan(sql).await {
        ...
        Err(e) => {
            ...
            match permission_analyze(
                analyzed_mdl.quill_mdl().manifest.clone(),
                sql,
                remote_functions,
                properties,
            )
    ...
    debug!("quill-core original plan:\n {plan}");
    let analyzed = ctx.state().optimize(&plan)?;
    debug!("quill-core final planned:\n {analyzed}");

    let data_source = analyzed_mdl.quill_mdl().data_source().unwrap_or_default();
    let quill_dialect = QuillDialect::new(&data_source);
    let unparser = Unparser::new(&quill_dialect)
        .with_pretty(true)
    ...
    match unparser.plan_to_sql(&analyzed) {
        Ok(sql) => {
            ...
            let replaced = sql
                .to_string()
                .replace(analyzed_mdl.quill_mdl().catalog_schema_prefix(), "");
            info!("quill-core planned SQL: {replaced}");
            Ok(replaced)
        }
        Err(e) => Err(e),
    }
...
/// it will return an column not found error because the column won't be registered in the [QuillDataSource].
...
async fn permission_analyze(
    ...
    let ctx = create_quill_ctx(None, manifest.data_source.as_ref());
    ...
    let ctx = apply_quill_on_ctx(&ctx, analyzed_mdl, properties, Mode::PermissionAnalyze)
        .await?;
    ...
    debug!("quill-core start to anlayze:\n {plan}");
        ...
        Err(e) => {
            if let DataFusionError::Context(_, ee) = &e {
                if let DataFusionError::External(we) = ee.as_ref() {
                    if we.downcast_ref::<QuillError>().is_some() {
                        return Err(e);
                    }
                }
            }
...
/// Analyze the decision point. It's same as the /v1/analysis/sql API in quill engine
pub fn decision_point_analyze(_quill_mdl: Arc<QuillMDL>, _sql: &str) {}
...
mod test {
    ...
    use crate::mdl::context::{apply_quill_on_ctx, Mode, SessionPropertiesRef};
    use crate::mdl::function::RemoteFunction;
    use crate::mdl::manifest::DataSource::MySQL;
    use crate::mdl::manifest::Manifest;
    use crate::mdl::{self, create_quill_ctx, transform_sql_with_ctx, AnalyzedQuillMDL};
    ...
    async fn test_remote_function() -> Result<()> {
        ...
        let ctx = create_quill_ctx(None, analyzed_mdl.quill_mdl().data_source().as_ref());
    ...
    async fn test_unicode_remote_column_name() -> Result<()> {
        let ctx = create_quill_ctx(None, None);
        ...
        let sql = r#"select * from quill.test.artist"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
            Arc::new(HashMap::new()),
            sql,
        ...
        let sql = r#"select group from quill.test.artist"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
        ...
        let sql = r#"select subscribe_plus from quill.test.artist"#;
        let actual = mdl::transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_invalid_infer_remote_table() -> Result<()> {
        ...
        let sql = r#"select name_append from quill.test.artist"#;
        let _ = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
        ...
        let sql = r#"select lower_name from quill.test.artist"#;
        let _ = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_query_hidden_column() -> Result<()> {
        ...
        let sql = r#"select 串接名字 from quill.test.artist"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
        ...
        let sql = r#"select "名字" from quill.test.artist"#;
        let _ = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
            ...
            .await.map_err(|e| {
                assert_snapshot!(
                    e.to_string(),
                    @"Schema error: No field named \"名字\". Valid fields are quill.test.artist.\"串接名字\"."
                )
            });
        Ok(())
    }
    ...
    async fn test_disable_simplify_expression() -> Result<()> {
        ...
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::new(AnalyzedQuillMDL::default()),
            &[],
            Arc::new(HashMap::new()),
            sql,
        )
    ...
    async fn test_disable_decorrelate_predicate_subquery() -> Result<()> {
        ...
        let sql = r#"select * from quill.test.artist where 名字 in (SELECT 名字 FROM quill.test.artist)"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_infer_timestamp_column() -> Result<()> {
        ...
        let sql = r#"select current_date > "出道時間" from quill.test.artist"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_disable_count_wildcard_rule() -> Result<()> {
        ...
        let analyzed_mdl = Arc::new(AnalyzedQuillMDL::default());
    ...
    async fn test_mysql_style_interval() -> Result<()> {
        let ctx = create_quill_ctx(None, Some(&DataSource::MySQL));
        let analyzed_mdl = Arc::new(AnalyzedQuillMDL::default());
    ...
    async fn test_eval_timestamp_with_session_timezone() -> Result<()> {
        ...
        headers.insert("x-quill-timezone".to_string(), Some("+08:00".to_string()));
    ...
    async fn test_disable_pushdown_filter() -> Result<()> {
        ...
        let sql = r#"select count(*) from quill.test.artist where cast(cast_timestamptz as timestamp) > timestamp '2011-01-01 21:00:00'"#;
        let actual = transform_sql_with_ctx(
            &create_quill_ctx(None, None),
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_register_timestamptz() -> Result<()> {
        ...
        let analyzed_mdl =
            Arc::new(AnalyzedQuillMDL::analyze_with_tables(manifest, registers)?);
        ...
        .await?;
        let sql = r#"select arrow_typeof(timestamp_col), arrow_typeof(timestamptz_col) from quill.test.timestamp_table limit 1"#;
    ...
    async fn test_coercion_timestamptz() -> Result<()> {
        ...
        for timezone_type in [
            ...
            let sql = r#"select timestamp_col = timestamptz_col from quill.test.timestamp_table"#;
            let actual = transform_sql_with_ctx(
                &create_quill_ctx(None, None),
                Arc::clone(&analyzed_mdl),
                &[],
            ...
            let sql = r#"select timestamptz_col > cast('2011-01-01 18:00:00' as TIMESTAMP WITH TIME ZONE) from quill.test.timestamp_table"#;
            let actual = transform_sql_with_ctx(
                &create_quill_ctx(None, None),
                Arc::clone(&analyzed_mdl),
                &[],
            ...
            let sql = r#"select timestamptz_col > '2011-01-01 18:00:00' from quill.test.timestamp_table"#;
            let actual = transform_sql_with_ctx(
                &create_quill_ctx(None, None),
                Arc::clone(&analyzed_mdl),
                &[],
            ...
            let sql = r#"select timestamp_col > cast('2011-01-01 18:00:00' as TIMESTAMP WITH TIME ZONE) from quill.test.timestamp_table"#;
            let actual = transform_sql_with_ctx(
                &create_quill_ctx(None, None),
                Arc::clone(&analyzed_mdl),
                &[],
    ...
    async fn test_list() -> Result<()> {
        ...
        let sql = "select list_col[1] from quill.test.list_table";
        let actual = transform_sql_with_ctx(
            &ctx,
            Arc::clone(&analyzed_mdl),
            &[],
    ...
    async fn test_union_all_local_runtime() -> Result<()> {
        // Reproduce quill-core-wasm bug: UNION ALL traps in LocalRuntime mode
        ...
        let ctx = create_quill_ctx(None, None);
        let analyzed = Arc::new(AnalyzedQuillMDL::default());
        let ctx = apply_quill_on_ctx(
            &ctx,
            Arc::clone(&analyzed),
            Arc::new(HashMap::new()),
    ...
    async fn test_dialect_specific_function_rewrite() -> Result<()> {
        ...
        let mdl = Arc::new(AnalyzedQuillMDL::analyze(
            manifest,
            Arc::new(HashMap::default()),
            Mode::Unparse,
        )?);
    ...
    async fn test_multiple_relationship_calculated_columns() -> Result<()> {
        ...
        // quill-core's plan analyzer uses deep recursion and can exceed the
    ...
    async fn test_clac_with_required_properties() -> Result<()> {
        ...
        match AnalyzedQuillMDL::analyze(manifest.clone(), headers.clone(), Mode::Unparse) {
            Err(e) => {
                assert_snapshot!(
                    e.to_string(),
    ...
    async fn test_extract_roundtrip_bigquery() -> Result<()> {
        let ctx = create_quill_ctx(None, Some(&DataSource::BigQuery));
        let manifest = ManifestBuilder::new()
            .catalog("quill")
            .schema("test")
            .model(
    ...
    async fn test_ambiguous_table_name() -> Result<()> {
        ...
        .await
        {
            ...
            Err(e) => assert_snapshot!(
                e.to_string(),
                @"Error during planning: table 'quill.test.CUSTOMER' not found"
            ),
        }
    ...
    async fn test_trim_function_bigquery_and_mssql() -> Result<()> {
        ...
        let ctx = create_quill_ctx(None, Some(&DataSource::MSSQL));
        let manifest = ManifestBuilder::new()
            .catalog("quill")
            .schema("test")
            .model(
    ...
    async fn test_snowflake_unnest() -> Result<()> {
        let ctx = create_quill_ctx(None, Some(&DataSource::Snowflake));
        let manifest = ManifestBuilder::new()
            .catalog("quill")
            .schema("test")
            .model(
    ...
    async fn test_analyze_with_url_tables_rejects_non_file_datasource() {
        ...
        let result = AnalyzedQuillMDL::analyze_with_url_tables(manifest, &ctx).await;
        match result {
            Err(e) => assert!(
    ...
    async fn test_analyze_with_url_tables_allows_no_datasource() {
        ...
        let dir = std::env::temp_dir().join("quill_test_url_tables_no_ds");
        ...
        assert!(analyzed.quill_mdl().get_model("test").is_some());
    ...
    async fn test_analyze_with_url_tables_local_file_datasource() {
        ...
        let dir = std::env::temp_dir().join("quill_test_url_tables_local");
        let _ = std::fs::create_dir_all(&dir);
        ...
        let ctx = datafusion::prelude::SessionContext::new();
        let analyzed = AnalyzedQuillMDL::analyze_with_url_tables(manifest, &ctx)
            .await
            .expect("analyze_with_url_tables should succeed for LocalFile");

        // Verify the model exists and the table is registered
        assert!(analyzed.quill_mdl().get_model("orders").is_some());
        assert!(
            analyzed.quill_mdl().register_tables.contains_key(&url),
    ...
    /// Contract tests for `apply_quill_on_ctx`'s catalog isolation: each call
    ...
    mod catalog_race {
        ...
        /// `apply_quill_on_ctx` leaves the base context's sentinel catalog
        ...
        async fn apply_does_not_mutate_base_ctx_catalog_list() -> Result<()> {
            let catalog_name = "quill_race_sentinel";
            ...
            assert!(
                ...
                "apply_quill_on_ctx replaced/mutated the base ctx's catalog \
        ...
        /// runtime, call `apply_quill_on_ctx` against one shared base context
        ...
        fn apply_is_isolated_under_concurrent_same_context_calls() -> Result<()> {
            ...
            const SHARED_CATALOG: &str = "quill_stress_shared";

            let base_ctx = Arc::new(create_quill_ctx(None, None));
        ...
        async fn top_level_catalog_snapshot_isolated_from_later_base_changes(
            ...
            let manifest = single_model_manifest("quill_layer1", "marker_col");
            let analyzed_mdl = Arc::new(AnalyzedQuillMDL::analyze(
                manifest,
                Arc::new(HashMap::default()),
                Mode::Unparse,
        ...
        async fn table_added_to_pre_existing_catalog_after_apply_is_visible_from_derived_ctx(
            ...
            let manifest = single_model_manifest("quill_layer2", "marker_col");
            let analyzed_mdl = Arc::new(AnalyzedQuillMDL::analyze(
                manifest,
                Arc::new(HashMap::default()),
                Mode::Unparse,
