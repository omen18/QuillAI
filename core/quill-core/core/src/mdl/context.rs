...
use crate::logical_plan::analyze::expand_view::ExpandQuillViewRule;
...
use crate::logical_plan::optimize::type_coercion::TypeCoercion as QuillTypeCoercion;
use crate::logical_plan::utils::create_schema;
use crate::mdl::manifest::Model;
use crate::mdl::type_planner::QuillTypePlanner;
...
/// Apply Quill Rules to the context for sql generation.
///
...
pub async fn apply_quill_on_ctx(
    ctx: &SessionContext,
    ...
    let mut config = ctx
        ...
        .with_default_catalog_and_schema(
            analyzed_mdl.quill_mdl.catalog(),
            analyzed_mdl.quill_mdl.schema(),
        )
    ...
    let type_planner = Arc::new(QuillTypePlanner::default());
    ...
    if !Arc::ptr_eq(new_state.catalog_list(), &private_catalog_list) {
        return internal_err!(
            "apply_quill_on_ctx: final SessionState's catalog_list is not the \
...
/// Execution mode for Quill engine.
#[derive(Debug)]
pub enum Mode {
...
fn analyze_rule_for_local_runtime(
    ...
    vec![
        ...
        Arc::new(ExpandQuillViewRule::new(
            Arc::clone(&analyzed_mdl),
            Arc::clone(&session_state_ref),
        )),
...
fn analyze_rule_for_unparsing(
    ...
    vec![
        ...
        // Use QuillTypeCoercion for the unparsing purpose
        Arc::new(QuillTypeCoercion::new()),
    ]
}
...
pub async fn register_table_with_mdl(
    ...
    let quill_mdl = analyzed_mdl.quill_mdl();
    catalog.register_schema(&quill_mdl.manifest.schema, Arc::new(schema))?;
    ctx.register_catalog(&quill_mdl.manifest.catalog, Arc::new(catalog));

    for model in quill_mdl.manifest.models.iter() {
        let table = QuillDataSource::new(
            Arc::clone(model),
            &properties,
            Arc::clone(&analyzed_mdl),
            &mode,
        ...
        ctx.register_table(
            TableReference::full(quill_mdl.catalog(), quill_mdl.schema(), model.name()),
            Arc::new(table),
        )?;
    }
    for view in quill_mdl.manifest.views.iter() {
        let plan = ctx.state().create_logical_plan(&view.statement).await?;
        let view_table = ViewTable::new(plan, Some(view.statement.clone()));
        ctx.register_table(
            TableReference::full(quill_mdl.catalog(), quill_mdl.schema(), view.name()),
            Arc::new(view_table),
        )?;
    }
    Ok(())
...
#[derive(Debug)]
pub struct QuillDataSource {
    schema: SchemaRef,
}

impl QuillDataSource {
    pub fn new(
        model: Arc<Model>,
        properties: &SessionPropertiesRef,
        analyzed_mdl: Arc<AnalyzedQuillMDL>,
...
#[async_trait]
impl TableProvider for QuillDataSource {
    ...
    async fn scan(
        ...
        unreachable!("QuillDataSource should be replaced before physical planning")
    }
}
