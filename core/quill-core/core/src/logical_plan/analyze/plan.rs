...
use crate::mdl::utils::{
    create_remote_expr_for_model, create_quill_calculated_field_expr,
    create_quill_expr_for_model, is_dag, quoted,
};
use crate::mdl::Dataset;
use crate::mdl::{AnalyzedQuillMDL, ColumnReference, SessionStateRef};

use super::access_control::{build_filter_expression, collect_condition, validate_rule};

#[derive(Debug)]
pub(crate) enum QuillPlan {
...
impl QuillPlan {
    fn name(&self) -> &str {
        match self {
            QuillPlan::Calculation(node) => node.calculation.column.name(),
        }
    }

    fn as_ref(&self) -> Arc<dyn UserDefinedLogicalNode> {
        match self {
            QuillPlan::Calculation(calculation) => Arc::clone(calculation) as _,
        }
    }
}
...
impl ModelPlanNode {
    pub fn new(
        ...
        ModelPlanNodeBuilder::new(analyzed_quill_mdl, session_state, properties).build(
            model,
            required_fields,
            original_table_scan,
        )
...
/// analyzed_quill_mdl: The analyzed Quill MDL.
struct ModelPlanNodeBuilder {
    ...
    required_calculation: Vec<QuillPlan>,
...
impl ModelPlanNodeBuilder {
    ...
    fn build(
        ...
        let model_ref = TableReference::full(
            self.analyzed_quill_mdl.quill_mdl().catalog(),
            self.analyzed_quill_mdl.quill_mdl().schema(),
            model.name(),
        );
        ...
        for column in required_columns {
            ...
            let (is_valid, rule_name) = validate_clac_rule(
                ...
                &column,
                &self.properties,
                Some(Arc::clone(&self.analyzed_quill_mdl)),
            )?;
            if !is_valid {
                ...
                return Err(DataFusionError::External(Box::new(
                    QuillError::PermissionDenied(message),
                )));
            }
            ...
            if column.is_calculated {
                let expr = if column.expression.is_some() {
                    ...
                    let expr = create_quill_calculated_field_expr(
                        column_rf,
                        Arc::clone(&self.analyzed_quill_mdl),
    ...
    /// other Quill models — they remain unanalyzed here. [`ModelAnalyzeRule`] is responsible
    ...
    fn create_partial_calculation(
        ...
        if let Some(primary_key_column) = primary_key_column {
            let expr = create_quill_expr_for_model(
        ...
        Ok(QuillPlan::Calculation(Arc::new(CalculationPlanNode::new(
            column_rf,
            col_expr,
            partial_chain,
            Arc::clone(&self.session_state),
...
fn collect_partial_model_plan_for_calculation(
    ...
    let Some(set) = analyzed_quill_mdl
        .lineage()
        .required_fields_map
        .get(qualified_column)
    else {
    ...
    for c in set {
        ...
        let Some(ColumnReference { dataset, column }) =
            analyzed_quill_mdl.quill_mdl().get_column_reference(c)
        else {
    ...
    for c in set {
        ...
        if column.is_calculated {
            let expr_plan = if let Some(expression) = &column.expression {
                let Ok(expr) = create_quill_expr_for_model(
                    expression,
                    Arc::clone(&m),
                    Arc::clone(&session_state_ref),
                ) else {
...
fn get_remote_column_exp(
    ...
    let (is_valid, rule_name) = validate_clac_rule(
        ...
        column,
        &session_properties,
        Some(Arc::clone(&analyzed_quill_mdl)),
    )?;
    if !is_valid {
        let message = if let Some(rule_name) = rule_name {
            format!(
        ...
        for expr in required_exprs.iter() {
            ...
            if let Expr::Wildcard { qualifier, .. } = expr {
                let model = if let Some(model) = qualifier {
                    let Some(model) =
                        analyzed_quill_mdl.quill_mdl.get_model(&format!("{model}"))
                    else {
...
impl CalculationPlanNode {
    pub fn new(
        ...
        let dimensions = pk_columns
            .iter()
            .map(|pk_column| {
                Ok(create_quill_expr_for_model(
                    &pk_column.name,
                    Arc::clone(&model),
                    Arc::clone(&session_state_ref),
                )?
