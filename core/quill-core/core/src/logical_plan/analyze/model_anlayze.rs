...
use crate::mdl::{AnalyzedQuillMDL, Dataset, SessionStateRef};
...
/// 3. Remove the catalog and schema prefix of Quill for the column and refresh the schema. (top-down)
///
    ...
    fn analyze(&self, plan: LogicalPlan, _: &ConfigOptions) -> Result<LogicalPlan> {
        ...
        self.analyze_scope(plan, &mut scope_manager, root_scope_id)?
            ...
            .map_data(|plan| {
                ...
                > {
                    self.remove_quill_catalog_schema_prefix_and_refresh_schema(plan)
                })
                .data()
            })?
...
impl ModelAnalyzeRule {
    ...
    fn collect_required_column(
        ...
        match expr {
            ...
            // e.g. Column {
            //         relation: None,
            //         name: "min(quill.public.order_items_model.price)",
            //     },
    ...
    fn analyze_model_internal(
        ...
        match plan {
            LogicalPlan::SubqueryAlias(SubqueryAlias { input, alias, .. }) => {
                ...
                // to the ModelPlanNode before the SubqueryAlias. We should check the patten of Quill-generated model plan like:
    ...
    fn build_model_plan_node(
        &self,
        model: Arc<quill_core_base::mdl::Model>,
        ...
        self.analyze_scope(plan, &mut scope_manager, root_scope_id)?
            ...
            .map_data(|p| {
                p.transform_up_with_subqueries(&|p| -> Result<Transformed<LogicalPlan>> {
                    self.remove_quill_catalog_schema_prefix_and_refresh_schema(p)
                })
                .data()
            })?
    ...
    fn analyze_table_scan(
        ...
        if belong_to_mdl(
            &analyzed_quill_mdl.quill_mdl(),
            table_scan.table_name.clone(),
            Arc::clone(&session_state_ref),
        ) {
            let table_name = table_scan.table_name.table();
            if let Some(model) = analyzed_quill_mdl.quill_mdl.get_model(table_name) {
            ...
            Arc::unwrap_or_clone(Arc::clone(&input))
        {
            if let Some(model_node) = node.as_any().downcast_ref::<ModelPlanNode>() {
                    ...
                    .get_model(model_node.plan_name())
                {
                    ...
                    internal_err!(
                        "Model {} not found in the QuillMDL",
    ...
    /// Remove the catalog and schema prefix of Quill for the column and refresh the schema.
    /// The plan created by DataFusion is always with the Quill prefix for the column name.
    /// Something like "quill.public.order_items_model.price". However, the model plan will be rewritten to a subquery alias
    /// The catalog and schema are invalid for the subquery alias. We should remove the prefix and refresh the schema.
    fn remove_quill_catalog_schema_prefix_and_refresh_schema(
        &self,
        plan: LogicalPlan,
    ) -> Result<Transformed<LogicalPlan>> {
        match plan {
            LogicalPlan::SubqueryAlias(SubqueryAlias { input, alias, .. }) => {
                let subquery = self
                    .remove_quill_catalog_schema_prefix_and_refresh_schema(
            ...
            Expr::Column(Column { relation, name, .. }) => {
                if let Some(relation) = relation {
                    ...
                    let name = name.replace(
                        self.analyzed_quill_mdl.quill_mdl().catalog_schema_prefix(),
                        "",
                    );
