...
use quill_core_base::mdl::RowLevelAccessControl;
use quill_core_base::mdl::{Column, Model, SessionProperty};

use crate::{
    logical_plan::utils::{create_schema, from_qualified_name},
    mdl::{
        context::SessionPropertiesRef, type_planner::QuillTypePlanner, Dataset,
        SessionStateRef,
    },
    AnalyzedQuillMDL,
};
...
pub fn build_filter_expression(
    ...
    analyzed_mdl: Option<Arc<AnalyzedQuillMDL>>,
    model: Arc<Model>,
...
///
/// It resolves table references against the [`AnalyzedQuillMDL`]'s models so that
...
impl RlacContextProvider {
    pub fn new(
        ...
        // QuillTypePlanner is stateless; the session-attached instance is not exposed
        // through SessionState's public API on this DataFusion version, so we build a
        // fresh one to mirror what `apply_quill_on_ctx` registers.
        let type_planner: Option<Arc<dyn TypePlanner>> =
            Some(Arc::new(QuillTypePlanner::default()));
        Self {
            session_state,
            analyzed_mdl,
            options,
    ...
    fn model_table_source(&self, name: &TableReference) -> Option<Arc<dyn TableSource>> {
        let mdl = self.analyzed_mdl.as_ref()?.quill_mdl();
...
pub(crate) fn validate_clac_rule(
    ...
    if is_valid && column.is_calculated {
        if let Some(analyzed_mdl) = analyzed_mdl {
            let qualified_col =
                from_qualified_name(&analyzed_mdl.quill_mdl, model_name, column.name());
            ...
            for field in required_fields {
                ...
                let Some(ref_model) = analyzed_mdl.quill_mdl.get_model(model_name.table())
                else {
