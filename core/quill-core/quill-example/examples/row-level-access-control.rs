...
use quill_core::mdl::manifest::{JoinType, Manifest, SessionProperty};
use quill_core::mdl::{transform_sql_with_ctx, AnalyzedQuillMDL};

/// It's an example to show how to use quill engine to set up row level access control
...
async fn main() -> datafusion::common::Result<()> {
    ...
    ctx.register_csv(
        "documents",
        "quill-example/data/company/documents.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let documents = ctx
    ...
    ctx.register_csv(
        "tenants",
        "quill-example/data/company/tenants.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let tenants = ctx
    ...
    ctx.register_csv(
        "users",
        "quill-example/data/company/users.csv",
        CsvReadOptions::new(),
    )
    .await?;
    let users = ctx
    ...
    let analyzed_mdl =
        Arc::new(AnalyzedQuillMDL::analyze_with_tables(manifest, register)?);
    ...
    let sql = "select * from quill.test.documents";
    let sql =
