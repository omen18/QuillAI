...
use quill_core::mdl::manifest::{DataSource, JoinType, Manifest};
use quill_core::mdl::{create_quill_ctx, transform_sql_with_ctx, AnalyzedQuillMDL};
...
async fn main() -> datafusion::common::Result<()> {
    ...
    let sql = "select customer_state from quill.public.orders_model";
    println!("Original SQL: \n{sql}");
    let sql = transform_sql_with_ctx(
        &create_quill_ctx(None, Some(&DataSource::BigQuery)),
        analyzed_mdl,
        &[],
        HashMap::new().into(),
        sql,
