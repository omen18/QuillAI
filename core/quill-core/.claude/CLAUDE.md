...
Rust semantic engine for Quill Engine. Handles MDL analysis, query planning, logical plan optimization, and SQL generation via Apache DataFusion (upstream, crates.io v53). Published to crates.io as `quill-semantic-core` (lib name `quill_core`).
...
| `quill-example/` | Usage examples |

## Key Source Directories (`core/src/`)

- `mdl/` — Core MDL processing: `QuillMDL` (manifest + symbol table), `AnalyzedQuillMDL` (with lineage), function definitions (scalar/aggregate/window per dialect), type planning
...
- **quill-core-base**: Shared manifest types from `../quill-core-base`
