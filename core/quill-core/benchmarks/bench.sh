...
COMMAND=
BENCHMARK=all
QUILL_DIR=${QUILL_DIR:-$SCRIPT_DIR/..}
...
QUILL_DIR=/source/datafusion ./bench.sh run tpch
...
QUILL_DIR      directory to use (default $QUILL_DIR)
...
main() {
    ...
    case "$COMMAND" in
        run)
            ...
            BRANCH_NAME=$(cd "${QUILL_DIR}" && git rev-parse --abbrev-ref HEAD)
            ...
            echo "***************************"
            echo "Quill Benchmark Script"
            echo "COMMAND: ${COMMAND}"
            echo "BENCHMARK: ${BENCHMARK}"
            echo "QUILL_DIR: ${QUILL_DIR}"
            ...
            pushd "${QUILL_DIR}/benchmarks" > /dev/null
            mkdir -p "${RESULTS_DIR}"
            case "$BENCHMARK" in
                all)
                    run_tpch "1"
...
# Runs quill benchmark
run_quill() {
    RESULTS_FILE="${RESULTS_DIR}/quill.json"
    echo "RESULTS_FILE: ${RESULTS_FILE}"
    echo "Running quill benchmark..."
    $CARGO_COMMAND --bin quill -- benchmark -i 10 -o "${RESULTS_FILE}" ${QUERY_ARG}
}
