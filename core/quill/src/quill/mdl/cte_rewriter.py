...
all column references, then calls quill-core ``transform_sql`` once per model
...
to quill-core), and the models it references are emitted as model CTEs before
...
# Ensure the Quill dialect is registered with sqlglot on import.
import quill.mdl.quill_dialect as _quill_dialect  # noqa: F401
from quill.model.data_source import DataSource
from quill.model.error import ErrorCode, ErrorPhase, QuillError
from quill.policy import resolve_model_name

_SQLGLOT_DIALECT_MAP: dict[DataSource, str] = {
    DataSource.quill: "trino",
    DataSource.datafusion: "quill",
...
class CTERewriter:
    ...
    session_context:
        A ``quill_core.SessionContext`` used to expand per-model SQL.
    data_source:
    ...
    def __init__(
        ...
        *,
        ...
        # It is NOT expanded by quill-core — it becomes a CTE kept verbatim,
    ...
    def _raise_case_collision(self) -> None:
        ...
        Used on case-insensitive-column dialects, where Quill resolves column
                ...
                if low in seen:
                    raise QuillError(
                        ...
                        message=(
                            ...
                            f"only in case ('{seen[low]}' and '{col_name}'). Quill "
    ...
    def rewrite(self, sql: str) -> str:
        ...
        Models are expanded by quill-core. A referenced view is emitted as a
        ...
        if not used_columns and not view_refs:
            ...
            if self.fallback:
                quill_sql = self.session_context.transform_sql(sql)
                return sqlglot.transpile(quill_sql, read="quill", write=self.dialect)[0]
    ...
    def _collect_model_columns(
        ...
        be passed as-is to ``transform_sql`` so quill-core applies CLAC.
        ...
        # that quill-core controls column visibility (CLAC).
        ...
        # mixed-case model name (e.g. ``"QUILL_AI_CaseTest"``) folds to the
        ...
        #   so it matches the dialect-folded qualify schema. Quill resolves column
    ...
    def _resolve_column(
        ...
        a "column not found" from quill-core).
    ...
    def _alias_projection_to_user_quoting(
        ...
        quill-core projects manifest-case columns (e.g. ``"Year"``). We add an
        ...
        """
        # quill-core's single-model expansion is always a ``SELECT``; guard
        ...
        for proj in expanded_ast.expressions:
            if isinstance(proj, exp.Alias):
                ...
                # Bare column with no alias wrapper. quill-core normally emits an
    ...
    def _build_model_ctes(
        ...
        """Generate one CTE per model via quill-core transform_sql."""
        ctes: list[exp.CTE] = []
        for model_name, columns in used_columns.items():
            if columns is None:
                # SELECT * — let quill-core handle column visibility (CLAC)
            ...
            expanded_ast = parse_one(expanded, dialect="quill")
            # quill-core emits ``SELECT "<m>".col FROM (...) AS "<m>"`` using
            ...
            # ``quill_src_<m>`` (no leading underscore — Oracle ORA-00911) so
    ...
    def _build_view_ctes(self, view_refs: dict[str, tuple[str, bool]]) -> list[exp.CTE]:
        ...
        injected before it. quill-core is never asked to expand the view.
        """
        ctes: list[exp.CTE] = []
    ...
    def _rename_outer_alias(ast: exp.Expression, model_name: str) -> None:
        ...
        # the quoting.
        new_alias = f"quill_src_{model_name}"
        source.set(
            "alias",
