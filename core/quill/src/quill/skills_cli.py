"""``quill skills`` — serve bundled Quill agent skill guides."""
...
from quill import skills_delivery

skills_app = typer.Typer(
    name="skills",
    help="Serve Quill agent skill guides (run `quill skills list`).",
)
...
def list_cmd() -> None:
    ...
    if not skills:
        ...
        return
    typer.echo("Available skills (run `quill skills get <name>`):")
    for skill in skills:
...
@skills_app.command()
def get(
    name: str = typer.Argument(..., help="Skill name (see `quill skills list`)."),
    ...
    try:
        ...
        typer.echo(
            ...
            "Run `quill skills list` for available names.",
            err=True,
        )
        raise typer.Exit(1)
        ...
        typer.echo(
            ...
            "Run `quill skills list` to see available scripts.",
            err=True,
        )
        raise typer.Exit(1)
    typer.echo(content)
