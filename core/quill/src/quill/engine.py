"""QuillEngine — SQL transform + execute against a data source.
...
from quill.connector.factory import get_connector
from quill.mdl import get_manifest_extractor, get_session_context, to_json_base64
from quill.mdl.cte_rewriter import CTERewriter, get_sqlglot_dialect
from quill.model.data_source import DataSource
from quill.model.error import DIALECT_SQL, ErrorCode, ErrorPhase, QuillError
from quill.policy import resolve_model_name, validate_sql_policy


class QuillEngine:
    """Thin facade over quill-core MDL processing and connector execution.

    Parameters
    ----------
    manifest_str:
        Base64-encoded MDL JSON string (as produced by ``quill_core.to_json_base64``).
    data_source:
    ...
    function_path:
        ...
        Passed through to quill-core SessionContext.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._config = config or QuillConfig()
    ...
    def dry_plan(self, sql: str, properties: dict | None = None) -> str:
        ...
        Transformation flow::
            ...
            User SQL (target dialect, e.g. Postgres)
              ...
              → per-model: quill-core transform_sql → Quill dialect SQL
              → per-model: sqlglot parse (Quill dialect) → inject as CTE
