...
def _build_connect_kwargs(connection_info) -> dict[str, Any]:
    ...
    if web_identity_token and role_arn:
        session_name = (
            getattr(connection_info, "role_session_name", None) or "quill-oidc-session"
        )
        sts = boto3.client(
...
class AthenaConnector(ConnectorABC):
    ...
    def query(self, sql: str, limit: int | None = None) -> pa.Table:
        ...
        if limit is not None:
            ...
            # `) AS _quill_sub LIMIT n`. (Single-line sibling connectors don't
            # guard this.)
            executed = (
                "SELECT * FROM (\n"
                f"{strip_trailing_semicolon(sql)}\n"
                f") AS _quill_sub LIMIT {int(limit)}"
            )
        try:
