...
class DuckDBConnector(ConnectorABC):
    ...
    def _attach_database(self, connection_info) -> None:
        ...
        Raises ``QuillError`` if no files are found or an attach fails.
        """
        db_files = self._list_duckdb_files(connection_info)
        if not db_files:
            raise QuillError(ErrorCode.DUCKDB_FILE_NOT_FOUND, "No DuckDB files found.")
    ...
    def _list_duckdb_files(self, connection_info) -> list[str]:
        ...
        discovered. Raises ``QuillError`` if the directory cannot be listed.
        """
