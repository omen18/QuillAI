"""Quill Enterprise connector using the Postgres wire protocol via psycopg.

Quill Enterprise exposes a Postgres-compatible endpoint, so we connect with
``psycopg`` directly and build Arrow tables from the cursor description instead
of going through ibis. The OID map below covers the types quill emits for
...
from quill.connector.base import ConnectorABC, strip_trailing_semicolon
from quill.model.error import DIALECT_SQL, ErrorCode, ErrorPhase, QuillError

# Postgres OID → Arrow type. Quill publishes Trino-style values over the
...
class QuillConnector(ConnectorABC):
    def __init__(self, connection_info):
        ...
        if (
            ...
            raise QuillError(
                ...
                "connection_url is not supported for quill; "
                "use QuillConnectionInfo instead",
            )
    ...
    def query(self, sql: str, limit: int | None = None) -> pa.Table:
        ...
        try:
            ...
            raise
        except (QuillError, TimeoutError):
            raise
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Error closing Quill connection: {e}")
        finally:
            self._closed = True
            self.connection = None


def create_connector(connection_info) -> QuillConnector:
    return QuillConnector(connection_info)
