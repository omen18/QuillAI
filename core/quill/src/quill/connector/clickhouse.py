...
class ClickHouseConnector(ConnectorABC):
    ...
    def query(self, sql: str, limit: int | None = None) -> pa.Table:
        ...
        # ``SELECT * FROM (SELECT 1;) AS _quill_sub LIMIT N`` is invalid SQL.
        # Semicolons inside string literals are preserved.
        stripped = strip_trailing_semicolon(sql)
        statement = stripped
        if limit is not None:
            statement = f"SELECT * FROM ({stripped}) AS _quill_sub LIMIT {limit}"
        try:
    ...
    def dry_run(self, sql: str) -> None:
        stripped = strip_trailing_semicolon(sql)
        try:
            self.connection.query(f"SELECT * FROM ({stripped}) AS _quill_sub LIMIT 0")
        except _ClickHouseDbError as e:
            if "TIMEOUT_EXCEEDED" in str(e):
                raise DatabaseTimeoutError(str(e)) from e
            raise QuillError(
