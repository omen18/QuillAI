"""Compose the `quill genbi build` instruction.
...
``quill context instructions``: plain markdown on stdout.
"""
...
# Version of the quill-core-wasm npm package this instruction targets.
# Keep in sync with core/quill-core-wasm/package.json.
QUILL_CORE_WASM_VERSION = "0.4.1"
...
2. HARD RULE: warehouse credentials MUST NEVER be inlined into the app.
   ...
   `quill genbi verify` scans for inlined credentials as best-effort
...
2. Point the engine profile's `source` at those static assets so the browser
   queries them client-side via quill-core-wasm. No backend is involved.
"""
...
def _format_model_inventory(models: list[dict]) -> str:
    ...
    if not models:
        return "- (no models found — run `quill context build` first)"
    lines = []
    for model in models:
...
`quill-core-wasm`. The app runs the Quill engine in the browser and answers the
user's request below using this project's context layer.

## Wiring quill-core-wasm (pinned: {QUILL_CORE_WASM_VERSION})
...
```html
<script type="module">
  import {{ QuillEngine }} from
    "https://unpkg.com/@quill/quill-core-wasm@{QUILL_CORE_WASM_VERSION}/dist/index.js";
...
1. `quill genbi register {app_name} --data-mode {data_mode}`
2. `quill genbi verify {app_name}`
