...
Owns ``<project>/.quill/apps.yml``. Machine-written (via ``quill genbi
register``), never hand-rolled. Mirrors the ``~/.quill/profiles.yml``
...
_INDEX_RELPATH = Path(".quill") / "apps.yml"
...
class MalformedIndexError(Exception):
    """Raised when ``.quill/apps.yml`` exists but can't be parsed as an index."""
