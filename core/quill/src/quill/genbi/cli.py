"""Typer sub-app for ``quill genbi`` commands."""
...
def build(
    ...
    from quill.genbi.composer import compose_build_instruction  # noqa: PLC0415
    ...
    from quill.genbi.composer import DATA_MODES  # noqa: PLC0415
    ...
    if not mdl_path.exists():
        ...
        from quill.context import build_json, save_target  # noqa: PLC0415
...
def register(
    ...
    """Record an agent-authored app in the project index (.quill/apps.yml)."""
    from quill.genbi.composer import DATA_MODES  # noqa: PLC0415
    from quill.genbi.index import register_app  # noqa: PLC0415
    ...
    if not app_dir.is_dir():
        typer.echo(
            ...
            "  Write the app there first (see `quill genbi build`).",
            err=True,
        )
        raise typer.Exit(1)
...
def list_apps(path: ProjectPathOpt = None) -> None:
    ...
    from quill.genbi.index import load_index  # noqa: PLC0415

    project_path = _discover(path)
    apps = load_index(project_path)["apps"]
    if not apps:
        typer.echo("No apps registered. See `quill genbi build` to create one.")
        return
...
def remove(
    ...
    from quill.genbi.index import remove_app  # noqa: PLC0415
...
def _require_registered(project_path, name: str) -> dict:
    from quill.genbi.index import get_app  # noqa: PLC0415

    entry = get_app(project_path, name)
    if entry is None:
        typer.echo(
            f"Error: app {name!r} is not registered.\n"
            f"  Run `quill genbi register {name}` first.",
            err=True,
        )
        raise typer.Exit(1)
    return entry
...
def verify(
    ...
    from quill.genbi.index import update_app  # noqa: PLC0415
    from quill.genbi.verify import verify_app  # noqa: PLC0415
...
def deploy(
    ...
    from quill.genbi.index import update_app  # noqa: PLC0415
    from quill.genbi.providers import get_provider  # noqa: PLC0415
    from quill.genbi.providers.base import DeployError  # noqa: PLC0415
    from quill.genbi.tokens import resolve_token  # noqa: PLC0415
    from quill.genbi.verify import verify_app  # noqa: PLC0415
