"""Quill CLI — SQL transform and execution via the Quill semantic layer."""
...
from quill.context_cli import context_app

app = typer.Typer(name="quill", help="Quill Engine CLI", no_args_is_help=False)

_QUILL_HOME = Path(os.environ.get("QUILL_HOME", str(Path.home() / ".quill"))).expanduser()
_DEFAULT_CONN = _QUILL_HOME / "connection_info.json"
...
def _require_mdl(mdl: str | None) -> str:
    ...
    if mdl is not None:
        return mdl
    try:
        from quill.context import discover_project_path  # noqa: PLC0415
        ...
        typer.echo(
            ...
            "  Hint: run `quill context build` first.",
            err=True,
        )
...
def _load_manifest(mdl: str) -> str:
    ...
    if path.exists():
        ...
        if path.suffix.lower() == ".json":
            # Raw JSON file — base64-encode it for QuillEngine
...
def _load_conn(
    ...
    *,
    ...
    """Load connection dict from inline JSON or file, with ~/.quill auto-discovery.

    If neither --connection-info nor --connection-file is given, looks for
    connection_info.json in ~/.quill.  Raises typer.Exit(1) if required=True and nothing
    is found.
    """
    if connection_info:
        try:
...
def _resolve_engine_profile(mdl: str | None) -> tuple[str | None, dict]:
    ...
    """
    from quill.profile import (  # noqa: PLC0415
        get_active_profile,
...
def _discover_project_for_engine(mdl: str | None) -> Path | None:
    ...
    Resolution order:
      1. If ``--mdl`` is a real file, walk up its directory tree looking
         for ``quill_project.yml``. ``<project>/target/mdl.json`` is a build
    ...
    if mdl is not None:
        ...
        if mdl_path.exists() and mdl_path.is_file():
            for parent in mdl_path.resolve().parents:
                if (parent / "quill_project.yml").exists():
                    return parent
...
def _build_engine(
    ...
    *,
    ...
    from quill.config import load_config  # noqa: PLC0415
    from quill.engine import QuillEngine  # noqa: PLC0415
    from quill.model.data_source import DataSource  # noqa: PLC0415
    from quill.model.error import QuillError  # noqa: PLC0415
    ...
    if not connection_info and not connection_file:
        ...
        if prof_dict:
            ...
            try:
                config = load_config(_QUILL_HOME)
            except (QuillError, OSError) as e:
                typer.echo(f"Error: {e}", err=True)
...
def _print_store_tip(sql: str) -> None:
    ...
    typer.echo(
        f"\n# To save this query:\n"
        f"# quill memory store --nl '<natural language question>' "
        f"--sql '{escaped}'",
        err=True,
    )
...
def _maybe_print_store_tip(sql: str, quiet: bool) -> None:
    if quiet:
        return
    from quill.sql_classify import is_exploratory  # noqa: PLC0415
...
def _version_callback(value: bool) -> None:
    """Handle ``quill --version`` the idiomatic Typer way.

    ``quill version`` remains for scripts that already use it; ``--version``
...
def main(
    ...
    version: Annotated[
        ...
        typer.Option(
            ...
            is_eager=True,
            help="Print the quill version and exit.",
        ),
    ] = None,
) -> None:
    """Quill Engine CLI.
    ...
    ~/.quill.  Use a subcommand (query / dry-run / dry-plan)
...
def query(
    ...
    """Execute a SQL query through the Quill semantic layer."""
    with _build_engine(mdl, connection_info, connection_file) as engine:
        try:
            result = engine.query(sql, limit=limit)
        except Exception as e:
...
from quill.docs_cli import docs_app  # noqa: E402, PLC0415

app.add_typer(docs_app)

from quill.ask_cli import ask as _ask_command  # noqa: E402, PLC0415
from quill.cube_cli import cube_app  # noqa: E402, PLC0415
from quill.skills_cli import skills_app  # noqa: E402, PLC0415
from quill.utils_cli import utils_app  # noqa: E402, PLC0415
...
# The `memory` subcommand group is always registered: `quill memory store`
# writes knowledge/sql/*.md without the optional `memory` extra, and the
# lancedb-backed commands degrade with a clear "install quill[memory]" message.
...
from quill.memory.cli import memory_app  # noqa: E402

app.add_typer(memory_app)

from quill.genbi.cli import genbi_app  # noqa: PLC0415, E402
from quill.profile_cli import profile_app  # noqa: PLC0415, E402

app.add_typer(genbi_app)
app.add_typer(profile_app)

from quill.serve_cli import serve_app  # noqa: PLC0415, E402
