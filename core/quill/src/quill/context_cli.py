"""Typer sub-app for ``quill context`` commands."""
...
ProjectPathOpt = Annotated[
    ...
    typer.Option(
        "--path",
        "-p",
        help="Project directory. Auto-detected via QUILL_PROJECT_HOME, cwd walk, or ~/.quill/config.yml.",
    ),
]
...
FromOsiOpt = Annotated[
    ...
    typer.Option(
        ...
        help=(
            ...
            "of truth instead of a quill YAML project. The OSI file is read "
...
def import_cmd(
    ...
    force: Annotated[
        bool,
        typer.Option("--force", help="Overwrite generated Quill project files."),
    ] = False,
) -> None:
    """Import a Quill project from an external source."""
    if source != "dbt":
        typer.echo(
    ...
    from quill.context import write_project_files  # noqa: PLC0415
    from quill.dbt import (  # noqa: PLC0415
        DbtLoadError,
        convert_dbt_project_to_quill_project,
    )
    ...
    try:
        imported = convert_dbt_project_to_quill_project(
            dbt_project_dir,
    ...
    if dry_run:
        ...
        try:
            ...
            raise typer.Exit(1)
        typer.echo(f"Imported dbt project to Quill project at {output_path}/")
    ...
    typer.echo("\nNext steps:")
    typer.echo(f"  quill context validate --path {output_path}")
    typer.echo(f"  quill context build --path {output_path}")
...
def init(
    ...
    from_osi: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "One-way conversion: produces a native quill project; the "
    ...
    """Initialize a new Quill project.
    ...
    keeping OSI as the source of truth, use ``quill context build --from-osi``
    instead.
    """
    ...
    if from_mdl:
        ...
        from quill.context import (  # noqa: PLC0415
        ...
        typer.echo("\nNext steps:")
        typer.echo(f"  quill context validate --path {project_path}")
        typer.echo(f"  quill context build --path {project_path}")
        return

    # ── Scaffold empty project (existing behavior) ────────────
    from quill.context import PROJECT_FILE  # noqa: PLC0415
    ...
    project_yml = (
        ...
        'version: "1.0"\n'
        "\n"
        "# Quill Engine namespace (NOT your database's catalog/schema).\n"
    ...
    # NL→SQL pairs live in knowledge/sql/ (written by `quill memory store`),
    # so no queries.yml is scaffolded.

    typer.echo(f"Quill project initialized: {project_path}")
    typer.echo("  quill_project.yml            — project metadata (edit data_source)")
    if not empty:
    ...
    typer.echo(
        "  knowledge/sql/              — confirmed NL-SQL pairs (quill memory store)"
    )
    typer.echo("  AGENTS.md                   — AI agent workflow guidance")
    # A pre-existing legacy queries.yml is still auto-loaded by `quill memory
    ...
    if (project_path / "queries.yml").exists():
        typer.echo(
            ...
            "`quill memory index`, but is deprecated — migrate its pairs into "
...
def validate(
    ...
    if isinstance(profile_pin, str) and profile_pin.strip():
        ...
        try:
            from quill.profile import list_profiles  # noqa: PLC0415
            ...
            if profile_pin.strip() not in registered:
                sem_warnings.append(
                    ...
                    "exist in ~/.quill/profiles.yml. "
                    "Run `quill context set-profile <name>` to rebind."
                )
    ...
    if no_pin and not has_hard_error:
        typer.echo(
            ...
            "back to the\n"
            "  globally active profile in ~/.quill/profiles.yml.\n"
            "  Run `quill context set-profile <name>` to pin one explicitly."
        )
...
def build(
    ...
    Default mode: reads quill_project.yml, models/*/metadata.yml (+ref_sql.sql),
    ...
    intermediate quill project is created. Requires --data-source.
    """
    if from_osi:
        _build_from_osi(
            from_osi=from_osi,
    ...
    typer.echo("")
    typer.echo("Next: quill --sql 'SELECT ...' to query your data.")
...
def show(
    ...
    if output == "json":
        ...
        if not models and not views:
            typer.echo("Empty project. Run `quill context init` to get started.")
...
def instructions(
    ...
    from quill.context import discover_project_path, load_rules  # noqa: PLC0415
...
def set_profile(
    ...
    ``quill_project.yml``. Future CLI commands and the SDK use the bound
    ...
    if not (project_path / PROJECT_FILE).exists():
        typer.echo(
            ...
            "  Run `quill context init` to scaffold a project first.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        typer.echo(
            f"Error: could not read ~/.quill/profiles.yml: {exc}",
            err=True,
        )
        raise typer.Exit(1)
    ...
    if name not in profiles:
        ...
        typer.echo(
            f"Error: profile '{name}' not found in ~/.quill/profiles.yml.\n"
            f"  Available profiles: {avail}\n"
            f"  Run `quill profile add {name} --datasource <ds>` to create it.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    if not new_ds:
        typer.echo(
            ...
            "Edit ~/.quill/profiles.yml or recreate it via `quill profile add`.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    if old_ds and old_ds != new_ds and (project_path / "target" / "mdl.json").exists():
        typer.echo(
            f"\n⚠ MDL was built for {old_ds}. Run `quill context build` "
...
def upgrade(
    ...
    typer.echo("\nUpgrade complete. Run `quill context validate` to check the result.")
...
def _init_from_osi(
    *,
    ...
    """Migrate an OSI file to a native quill project (one-way).

    Reuses ``build_json_from_osi`` to produce the MDL, then
    ``convert_mdl_to_project`` + ``write_project_files`` to scaffold the
    full quill YAML layout. The OSI file is read once and never referenced
    again after this command — the quill project becomes the source of truth.
    """
    from quill.context import (  # noqa: PLC0415
        convert_mdl_to_project,
        write_project_files,
    )
    from quill.osi import build_json_from_osi  # noqa: PLC0415
    ...
    typer.echo(f"Migrated OSI → quill project at {project_path}/")
...
def _validate_from_osi(
    *,
    ...
    """Lint an OSI file against the QUILL conversion rules."""
    from quill.osi import lint_osi_file  # noqa: PLC0415
...
def _show_from_osi(
    *,
    ...
    from quill.osi import build_manifest_from_osi  # noqa: PLC0415
    ...
    if output == "json":
        from quill.context import _convert_keys  # noqa: PLC0415
