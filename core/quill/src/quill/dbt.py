...
DBT_ADAPTER_TO_QUILL_DATASOURCE = {
    "athena": "athena",
    "bigquery": "bigquery",
...
def default_quill_profile_name(target: DbtTarget) -> str:
    """Return a stable default Quill profile name for a dbt target."""
    return f"{target.profile_name}-{target.target_name}".replace("_", "-")


def map_dbt_adapter_to_quill(adapter_type: str) -> str:
    """Map a dbt adapter name to a Quill datasource name."""
    normalized = adapter_type.strip().lower()
    try:
        return DBT_ADAPTER_TO_QUILL_DATASOURCE[normalized]
...
def resolve_dbt_target(
    ...
    *,
    ...
    return DbtTarget(
        ...
        datasource=map_dbt_adapter_to_quill(adapter_type),
        project=project,
        profile=profile,
...
def load_dbt_artifacts(
    ...
    *,
    ...
    """Load the dbt artifacts needed for Quill import."""
    root = Path(project_dir).expanduser().resolve()
    project = load_dbt_project(root)
...
def convert_dbt_project_to_quill_project(
    project_dir: str | Path,
    *,
    ...
    """Convert dbt artifacts into Quill project files."""
    ...
    from quill.memory.markdown import render_query_markdown, slugify  # noqa: PLC0415
...
def convert_dbt_target_to_quill_profile(target: DbtTarget) -> dict[str, Any]:
    """Convert a resolved dbt target into a Quill profile payload."""
    ...
    if datasource == "duckdb":
        ...
        return _build_quill_profile("duckdb", {"url": str(url), "format": "duckdb"})

    if datasource == "postgres":
        return _build_quill_profile(
            "postgres",
            {
...
def _build_quill_profile(datasource: str, payload: dict[str, Any]) -> dict[str, Any]:
    """Validate a profile payload with Quill's connection-info model."""
    ...
    try:
        ...
        raise DbtLoadError(
            f"Invalid dbt target for Quill datasource {datasource!r}: {exc}"
        ) from exc
...
def _bigquery_credentials_base64(output: dict[str, Any]) -> str:
    """Encode BigQuery credentials from dbt output into Quill's expected format."""
...
def _build_imported_models(
    ...
    for unique_id, node in sorted(manifest.get("nodes", {}).items()):
        ...
        model = _build_model_metadata(
            ...
            quill_name=str(node.get("alias") or node.get("name") or ""),
        ...
        if column is not None:
            ...
            if test_name == "not_null":
                ...
                if target_model is not None:
                    ...
                    # column causes the Quill engine to treat the FK as a join
...
def _ensure_unique_model_name(name: str, used_names: set[str]) -> None:
    if name in used_names:
        raise DbtLoadError(
            f"Duplicate Quill model name '{name}' generated from dbt artifacts. "
...
def _build_dbt_query_pairs(
    ...
    *,
    ...
    from quill.memory.seed_queries import generate_seed_queries  # noqa: PLC0415
