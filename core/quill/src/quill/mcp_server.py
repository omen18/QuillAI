"""FastMCP server exposing QuillEngine query + context/knowledge tools.
...
class ServeContext:
    ...
    project: Path
    engine: Any  # quill.engine.QuillEngine
    allow_write: bool
    no_connect: bool
...
def _memory_path(ctx: ServeContext) -> str:
    ...
    return str(ctx.project / ".quill" / "memory")
...
def _register_query_tools(mcp: FastMCP, ctx: ServeContext) -> None:
    if not ctx.no_connect:
        ...
        def run_sql(sql: str, limit: int | None = None) -> dict:
            """Execute a SQL query through the Quill semantic layer and return rows.
        ...
        def query_cube(
            ...
            Mirrors ``quill cube query``. ``time_dimension`` uses the CLI spec
            ...
            """
            from quill_core import cube_query_to_sql  # noqa: PLC0415

            from quill import context  # noqa: PLC0415
            from quill.cube_cli import _build_cube_query  # noqa: PLC0415
...
def _register_context_tools(mcp: FastMCP, ctx: ServeContext) -> None:
    ...
    def get_mdl() -> dict:
        ...
        from quill.context import build_json  # noqa: PLC0415
    ...
    def list_models() -> dict:
        ...
        Returns ``{"models": [...]}``.
        """
        from quill.context import load_models  # noqa: PLC0415
    ...
    def describe_model(name: str) -> dict:
        ...
        from quill.context import load_models, load_relationships  # noqa: PLC0415
    ...
    def get_data_source() -> dict:
        ...
        from quill.context import load_project_config  # noqa: PLC0415
    ...
    def list_cubes() -> dict:
        ...
        Returns ``{"cubes": [...]}``.
        """
        from quill.context import load_cubes  # noqa: PLC0415
    ...
    def list_functions() -> dict:
        ...
        ``{"functions": [...]}``.
        """
        from quill.mdl import get_session_context  # noqa: PLC0415
...
def _register_knowledge_tools(mcp: FastMCP, ctx: ServeContext) -> None:
    ...
    def get_instructions() -> dict:
        ...
        from quill.context import load_rules  # noqa: PLC0415
    ...
    def recall_queries(question: str, limit: int = 3) -> dict:
        ...
        ``{"matches": [...]}``.
        """
        from quill.memory.index_backend import get_index  # noqa: PLC0415
    ...
    def get_context(
        ...
        schema has been indexed (``quill memory index``); otherwise falls back
        to the full plain-text schema description (same content as
        ``describe_schema``).
        """
        from quill.context import build_json  # noqa: PLC0415
        from quill.memory import schema_indexer  # noqa: PLC0415

        manifest = build_json(ctx.project)
        try:
            from quill.memory.store import MemoryStore  # noqa: PLC0415
            ...
            return {
                ...
                "note": (
                    "Install quill[memory] and run `quill memory index` for "
    ...
    def list_stored_queries(
        ...
        try:
            ...
            from quill.memory.markdown import load_query_pairs  # noqa: PLC0415
    ...
    def list_knowledge() -> dict:
        """List knowledge files readable via the quill://knowledge/{path} resource.
        ...
        if knowledge_dir.is_dir():
            # Only list what the quill://knowledge/{name} and
            # quill://knowledge/{subdir}/{name} templates can serve: top-level
...
def _register_write_tools(mcp: FastMCP, ctx: ServeContext) -> None:
    ...
    def store_query(
        ...
        """
        from quill.memory.markdown import write_query_markdown  # noqa: PLC0415
...
def _register_resources(mcp: FastMCP, ctx: ServeContext) -> None:
    @mcp.resource("quill://mdl", mime_type="application/json")
    ...
    @mcp.resource("quill://instructions", mime_type="text/markdown")
    ...
    @mcp.resource("quill://project", mime_type="application/json")
    def project_resource() -> str:
        """Summary of quill_project.yml (name, catalog, schema, data source)."""
    ...
    @mcp.resource("quill://agents", mime_type="text/markdown")
    ...
    @mcp.resource("quill://knowledge/{name}")
    ...
    @mcp.resource("quill://knowledge/{subdir}/{name}")
...
def _workflow_text(ctx: ServeContext, question: str | None = None) -> str:
    """Build the `quill_workflow` prompt body, reflecting the registered tool set.
    ...
    steps = [
        "Read the `quill://mdl` resource and use `list_models` / "
        "`describe_model` to understand the schema; for schema, read "
        "`quill://mdl` or use `get_context` / `describe_schema`; browse "
        "project knowledge via `list_knowledge` + the "
        "`quill://knowledge/{path}` resource.",
...
def _register_prompts(mcp: FastMCP, ctx: ServeContext) -> None:
    @mcp.prompt()
    def quill_workflow(question: str | None = None) -> str:
        """SOP for answering a data question with the quill MCP tools."""
...
def run_server(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        logger.info(f"Serving quill MCP over streamable-http at {host}:{port}")
