"""Quill CLI configuration loaded from ~/.quill/config.json."""
...
class QuillConfig:
    """Immutable configuration for the Quill CLI.
...
def load_config(quill_home: Path) -> QuillConfig:
    """Load configuration from ``quill_home/config.json``.

    Returns default ``QuillConfig`` when the file does not exist.
    Raises ``QuillError`` when the file exists but contains invalid JSON.
    """
    config_path = quill_home / "config.json"
