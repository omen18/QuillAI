...
_DATA_READER_NAMES: frozenset[str] = frozenset(
    {
        ...
        "parquet_scan",
        "glob",
        # duckdb extension squills (lakehouse / external db)
        "iceberg_scan",
        "delta_scan",
        "postgres_scan",
        "postgres_query",
        ...
        "load_file",
        # duckdb spatial / sniffing / metadata squills over arbitrary paths
        "sniff_csv",
        "st_read",
        "st_readosm",
        "st_read_meta",
...
def resolve_model_name(
    ...
    Follows the SQL convention used across Quill's CTE rewriter, policy check,
...
def validate_sql_policy(
    ...
    """Raise ``QuillError`` if the SQL violates strict-mode policies.
    ...
    config:
        Quill configuration with strict_mode and denied_functions settings.
    """
