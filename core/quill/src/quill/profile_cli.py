"""Typer sub-app for ``quill profile`` commands."""
...
profile_app = typer.Typer(
    name="profile",
    help="Manage connection profiles (~/.quill/profiles.yml).",
)
...
def list_cmd() -> None:
    ...
    from quill.profile import get_active_name, list_profiles  # noqa: PLC0415
    ...
    try:
        ...
        raise typer.Exit(1)
    if not profiles:
        typer.echo("No profiles configured. Run `quill profile add` to create one.")
        return
...
def add(
    ...
    profile, so the user can fix it with ``quill profile add --ui`` or
    by editing ``~/.quill/profiles.yml``.
    """
    from quill.profile import add_profile  # noqa: PLC0415
    ...
    if from_file:
        ...
        typer.echo(
            ...
            "Edit ~/.quill/profiles.yml to add connection fields."
        )
        minimal = True
...
def import_cmd(
    ...
    name: Annotated[
        Optional[str],
        typer.Option("--name", help="Destination Quill profile name."),
    ] = None,
    activate: Annotated[
        bool,
    ...
    from quill.dbt import (  # noqa: PLC0415
        DbtLoadError,
        convert_dbt_target_to_quill_profile,
        default_quill_profile_name,
        resolve_dbt_target,
    )
    ...
    try:
        ...
        profile_data = convert_dbt_target_to_quill_profile(dbt_target)
        ...
        dest_name = name or default_quill_profile_name(dbt_target)
    ...
    typer.echo(
        ...
        f"into Quill profile '{dest_name}' ({profile_data['datasource']})."
    )
    if activate:
...
def _post_add(name: str, *, validate: bool, minimal: bool) -> None:
    ...
    user into running ``quill context init`` against a broken profile.
    """
    ok = True
    ...
    if ok:
        typer.echo("")
        typer.echo("Next: quill context init")
...
def _retry_hint(name: str) -> str:
    ...
    return (
        ...
        f"    quill profile add {name} --from-file <path>   # dotenv-driven\n"
        f"    quill profile add {name} --ui                 # browser form\n"
        f"    quill profile add {name} --interactive        # prompt-driven"
    )
...
def _validate_connection(name: str) -> bool:
    ...
    from quill.connector.factory import get_connector  # noqa: PLC0415
    from quill.model.data_source import DataSource  # noqa: PLC0415
    ...
    # instead.  QuillEngine does the same conversion when initialised from
    ...
    try:
        ...
        typer.echo(
            ...
            "    quill profile debug                 # show resolved config\n"
            f"    quill profile add {name} --ui       # edit and re-validate",
            err=True,
        )
        return False
...
def _interactive_add(default_ds: str | None) -> dict:
    ...
    from quill.model.field_registry import (  # noqa: PLC0415
...
def rm(
    ...
    """Remove a profile."""
    from quill.profile import remove_profile  # noqa: PLC0415
...
def switch(
    ...
    from quill.profile import switch_profile  # noqa: PLC0415
...
def debug(
    ...
    from quill.profile import debug_profile  # noqa: PLC0415
