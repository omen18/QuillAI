...
X_QUILL_DB_STATEMENT_TIMEOUT = "x-quill-db-statement_timeout"
...
class DataSource(StrEnum):
        ...
        match self:
            case DataSource.postgres:
                ...
                if "statement_timeout" not in options:
                    if options:
                        options += " "
                    options += f"-c statement_timeout={headers.get(X_QUILL_DB_STATEMENT_TIMEOUT, 180)}s"
                    kwargs["options"] = options
                info.kwargs = kwargs
            case DataSource.clickhouse:
                session_timeout = headers.get(X_QUILL_DB_STATEMENT_TIMEOUT, 180)
    ...
    def _build_connection_info(self, data: dict) -> ConnectionInfo:
        ...
        match self:
            ...
            case DataSource.quill:
                return QuillConnectionInfo.model_validate(data)
            case DataSource.clickhouse:
