...
Skill content ships inside the wheel under ``quill/skills_content/<name>/``.
``quill skills get <name>`` returns the skill's ``SKILL.md`` main guide. Deeper
``references/`` and bundled ``scripts/`` are surfaced by ``quill skills list``
...
def _content_root():
    """Traversable for ``quill/skills_content/`` (anchored on the ``quill`` package)."""
    return resources.files("quill") / _CONTENT_DIR
