...
_DEFAULT_PROJECT = _QUILL_HOME / "project"
PROJECT_FILE = "quill_project.yml"
_TARGET_DIR = "target"
...
def load_global_config() -> dict:
    """Load ~/.quill/config.yml (global preferences)."""
    config_file = _QUILL_HOME / "config.yml"
...
def discover_project_path(explicit: str | None = None) -> Path:
    ...
    2. QUILL_PROJECT_HOME env var
    3. Walk up from cwd looking for quill_project.yml
    4. default_project in ~/.quill/config.yml
    ...
    # 2. QUILL_PROJECT_HOME env var
    env = os.environ.get("QUILL_PROJECT_HOME")
    if env:
...
# Field order preferred when writing quill_project.yml back from a dict —
...
def save_project_config(project_path: Path, config: dict) -> None:
    """Write ``quill_project.yml`` with a stable field ordering.
...
# knowledge.yml, decoupled from the MDL schema_version in quill_project.yml.
...
def get_schema_version(project_path: Path) -> int:
    """Return the schema_version from quill_project.yml (default 1)."""
...
def build_manifest(project_path: Path) -> dict:
    ...
    manifest: dict = {
        "catalog": project_config.get("catalog", "quill"),
...
def validate_project(project_path: Path) -> list[ValidationError]:
    ...
    Checks:
    0. quill_project.yml exists and has required fields
        ...
        if sv not in _SUPPORTED_SCHEMA_VERSIONS:
            errors.append(
                ValidationError(
                    "error",
                    PROJECT_FILE,
                    f"unsupported schema_version {sv} — please upgrade quill CLI",
                )
            )
            ...
            if kv not in _SUPPORTED_KNOWLEDGE_VERSIONS:
                errors.append(
                    ValidationError(
                        "error",
                        _KNOWLEDGE_CONFIG_FILE,
                        f"unsupported knowledge schema_version {kv} — please upgrade quill CLI",
                    )
                )
    ...
    # AnalyzedQuillMDL::analyze (see quill-core lineage::validate_cubes).
...
def plan_upgrade(
    ...
    for version in range(current, target):
        ...
        # changes needed — only quill_project.yml is restamped.
...
def apply_upgrade(project_path: Path, result: UpgradeResult) -> None:
    ...
    for version in range(result.from_version, result.to_version):
        ...
        # v2→v3, v3→v4: only the quill_project.yml stamp changes (handled below)

    # Update quill_project.yml
    config = load_project_config(project_path)
