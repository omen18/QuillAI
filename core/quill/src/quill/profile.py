...
The stored YAML keeps the placeholders so ``quill profile debug`` never
...
_PROFILES_FILE = _QUILL_HOME / "profiles.yml"
...
def _ensure_env_loaded() -> None:
    ...
    2. Walk up from ``$CWD`` to find the project root (``quill_project.yml``),
       load its ``.env`` if different from CWD
    3. ``~/.quill/.env`` — user-global fallback for operators with many
    ...
    try:
        ...
        # python-dotenv ships with core quill; this branch is defensive
    ...
    # Find project root by looking for quill_project.yml; if different from
...
def _save_raw(data: dict) -> None:
    ...
    _QUILL_HOME.mkdir(parents=True, exist_ok=True)
    payload = yaml.dump(
        data, default_flow_style=False, sort_keys=False, allow_unicode=True
    )
    # Write to a temp file in the same directory then atomically replace
    fd, tmp_path = tempfile.mkstemp(dir=_QUILL_HOME, suffix=".yml.tmp")
    try:
...
def resolve_profile_for_project(project_path: Path) -> tuple[str | None, dict]:
    ...
    Resolution order:
      1. ``profile:`` field in ``<project>/quill_project.yml`` (if non-empty)
      2. Global active profile in ``~/.quill/profiles.yml``
    ...
    """
    project_yml = project_path / "quill_project.yml"
    ...
    if project_yml.exists():
        try:
            ...
            raise SystemExit(
                ...
                "  Fix the file or run `quill context init --force` to "
                "rescaffold."
            ) from exc
    ...
    if pinned_name not in profiles:
        ...
        raise SystemExit(
            ...
            "Run `quill context set-profile <name>` to rebind, or "
            f"`quill profile add {pinned_name}` to recreate the missing profile."
        )
...
def resolve_connection(
    ...
    Legacy ~/.quill/connection_info.json fallback is handled separately by
