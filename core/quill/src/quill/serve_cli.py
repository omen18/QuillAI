"""Typer sub-app for ``quill serve`` commands."""
...
serve_app = typer.Typer(
    name="serve",
    help="Serve quill capabilities to external clients (MCP, ...).",
)

_SOURCE_FILES = ("quill_project.yml", "relationships.yml")
...
def _print_connection_help(
    *,
    ...
    """
    quill_cmd = shutil.which("quill") or sys.argv[0] or "quill"
    quill_home = os.environ.get("QUILL_HOME")
    line = "─" * 68
    ...
    if transport == "http":
        ...
        echo(" quill MCP server — Streamable HTTP")
        echo(f"   URL: {url}")
        echo("")
        echo(" Register with a client:")
        echo(f"   Claude Code   claude mcp add --transport http quill {url}")
        echo(f"   Codex         codex mcp add quill --url {url}")
        echo(
        ...
        cmd = " ".join(shlex.quote(t) for t in [quill_cmd, *args])
        env_flag = f" -e {shlex.quote('QUILL_HOME=' + quill_home)}" if quill_home else ""
        echo(" quill MCP server — stdio (your MCP client spawns this process)")
        echo("")
        echo(" Register with a client:")
        echo(f"   Claude Code   claude mcp add quill{env_flag} -- {cmd}")
        echo(f"   Codex         codex mcp add quill{env_flag} -- {cmd}")
        echo("")
        echo(" Or add to an IDE MCP config (Claude Desktop / Cursor / VS Code):")
        server: dict = {"command": quill_cmd, "args": args}
        if quill_home:
            server["env"] = {"QUILL_HOME": quill_home}
        config = {"mcpServers": {"quill": server}}
...
def serve_mcp(
    ...
    """Serve quill's query + context/knowledge tools as an MCP server.

    Backed by an in-process QuillEngine — no ibis-server, no HTTP engine.
    """
    ...
    try:
        ...
        typer.echo(
            "Install the MCP extra: pip install 'quill[mcp]'",
            err=True,
        )
        raise typer.Exit(1)
    ...
    from quill.cli import _build_engine  # noqa: PLC0415
    ...
    if _mdl_is_stale(project_path, mdl_path):
        logger.warning(
            "MDL may be stale — re-run `quill context build`",
        )
    ...
    from quill.mcp_server import ServeContext, run_server  # noqa: PLC0415
