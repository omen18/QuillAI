"""Typer sub-app for ``quill memory`` commands."""
...
PathOpt = Annotated[
    ...
    typer.Option(
        "--path",
        "-p",
        help="LanceDB storage directory. Defaults to <project>/.quill/memory/.",
    ),
]
MdlOpt = Annotated[
    Optional[str],
...
def _default_memory_path() -> Path:
    """Return the project-local memory path, or ~/.quill/memory/ as fallback."""
    try:
        from quill.context import discover_project_path  # noqa: PLC0415

        return discover_project_path() / ".quill" / "memory"
    except (SystemExit, Exception):
        return _QUILL_HOME / "memory"
...
def _load_manifest(mdl: str | None) -> dict:
    ...
    if mdl:
        ...
        try:
            ...
            typer.echo(
                "Error: no quill project found and --mdl not specified.\n"
                "  Run this command from a directory containing quill_project.yml,\n"
                "  set QUILL_PROJECT_HOME to the project path, or pass --mdl explicitly.",
                err=True,
            )
            raise typer.Exit(1)
...
def _get_store(path: str | None):
    ...
    try:
        ...
        if (e.name or "").split(".")[0] not in {
            ...
            raise
        typer.echo(
            "Error: quill[memory] extras not installed. "
            "Run: pip install 'quill[memory]'",
            err=True,
        )
        raise typer.Exit(1)
...
def index(
    ...
    """
    from quill.memory.index_backend import resolve_backend  # noqa: PLC0415
    ...
    if resolve_backend() == "grep":
        ...
        typer.echo(
            "`quill memory recall` works over grep; semantic schema search "
            "(`quill memory fetch`) needs `quill[memory]`.",
            err=True,
        )
        return
    ...
    if include_instructions and mdl is None:
        try:
            from quill.context import (  # noqa: I001, PLC0415
...
def describe(
    ...
    from quill.memory.schema_indexer import describe_schema  # noqa: PLC0415
...
def store(
    ...
    try:
        ...
        if (e.name or "").split(".")[0] not in {
            ...
            raise
        # memory extra not installed — markdown-only; run `quill memory index` later.
...
def reset(
    ...
    The LanceDB index is a derived artifact — after reset, run `quill memory
    ...
    idx.reset()
    typer.echo(
        "Memory index reset. Run `quill memory index` to rebuild from knowledge/sql/."
    )
...
def check(
    ...
    if not missing and not stale:
        ...
        return
    if missing:
        typer.echo(f"  {len(missing)} not indexed — run `quill memory index`.")
    if stale:
        typer.echo(
            f"  {len(stale)} user pair(s) indexed without markdown — "
            "stale index, run `quill memory index`."
        )
...
def watch(
    ...
    ``quill memory index`` so semantic recall never serves a stale schema while
    ...
    from quill.context import discover_project_path  # noqa: PLC0415
    from quill.memory.index_backend import resolve_backend  # noqa: PLC0415
    from quill.memory.watch import watch_loop  # noqa: PLC0415

    if resolve_backend() == "grep":
        typer.echo(
            "grep backend: knowledge/sql/ IS the index — nothing to watch. "
            "Install `quill[memory]` for a derived index to keep fresh.",
            err=True,
        )
        raise typer.Exit(1)
...
def export(
    ...
    run `quill memory index` to rebuild, then `quill memory reset` once verified.
    """
    from quill.context import discover_project_path  # noqa: PLC0415
    from quill.memory.markdown import write_query_markdown  # noqa: PLC0415
...
def _interactive_forget(mem_store, source: str | None, limit: int) -> None:
    ...
    try:
        ...
        typer.echo(
            ...
            "Install with: pip install 'quill[interactive]'\n"
            "Or use: quill memory forget --id <ID> [--id <ID> ...]",
            err=True,
        )
        raise typer.Exit(1)
...
def _discover_project_queries_path() -> Path | None:
    """Return ``<project>/queries.yml`` if inside a quill project, else None."""
    try:
        from quill.context import discover_project_path  # noqa: PLC0415
