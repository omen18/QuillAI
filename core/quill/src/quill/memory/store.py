...
from quill.memory.embeddings import (
    _DEFAULT_DIM,
    _DEFAULT_MODEL,
...
_QUILL_MEMORY_DIR = Path.home() / ".quill" / "memory"
...
class MemoryStore:
    ...
    ----------
    path:
        Directory for LanceDB storage.  Defaults to ``~/.quill/memory/``.
    model_name:
    ...
    def __init__(
        ...
        resolved = Path(path).expanduser() if path else _QUILL_MEMORY_DIR
    ...
    def _upsert_seed_queries(self, manifest: dict) -> int:
        ...
        from quill.memory.seed_queries import (  # noqa: PLC0415
            SEED_TAG,
