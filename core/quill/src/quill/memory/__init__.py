"""Quill Memory — LanceDB-backed schema and query memory.

Public API for programmatic use::

    from quill.memory import QuillMemory
...
class QuillMemory:
    """High-level memory API for Quill Engine.

    Parameters
    ----------
    path:
        LanceDB storage directory.  Defaults to ``~/.quill/memory/``.
    """
    ...
    def get_context(
        ...
        *,
        ...
        See :data:`~quill.memory.schema_indexer.SCHEMA_DESCRIBE_THRESHOLD`.
        """
        kwargs: dict = {
            "limit": limit,
            "item_type": item_type,
