...
Backend selection: ``QUILL_MEMORY_BACKEND=grep|lancedb`` forces a choice;
...
from quill.memory.markdown import load_query_pairs
...
def resolve_backend(env: str | None = None) -> str:
    ...
    Honors an explicit ``QUILL_MEMORY_BACKEND=grep|lancedb`` (or *env*) override,
    ...
    """
    choice = (
        (env if env is not None else os.environ.get("QUILL_MEMORY_BACKEND", ""))
        .strip()
        .lower()
    )
    if choice == "grep":
