...
The semantic memory index (``quill memory index``) is a derived artifact built
...
returns wrong schema context to the LLM. ``quill memory watch`` closes that loop:
...
library. The CLI (:mod:`quill.memory.cli`) supplies the reindex callback.
"""
