"""Prompt-shaping helpers for ``quill ask``.

``quill ask`` wraps a user's natural-language prompt in one of two bundled
...
def render(mode: str, user_prompt: str) -> str:
    ...
    tpl = (resources.files("quill") / _TEMPLATES_DIR / f"{mode}.md.tmpl").read_text(
        encoding="utf-8"
    )
