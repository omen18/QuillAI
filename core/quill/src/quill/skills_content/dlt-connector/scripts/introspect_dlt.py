#!/usr/bin/env python3
"""Introspect a dlt-produced DuckDB file and generate a Quill v2 YAML project.
...
5. Normalizes column types using quill's type_mapping.parse_type (sqlglot)
6. Generates a complete Quill v2 YAML project
...
class Column:
    ...
    quill_type: str  # normalized via parse_type
    is_nullable: bool
...
# ---------------------------------------------------------------------------
# Type normalization — delegates to quill SDK's parse_type when available
# ---------------------------------------------------------------------------


def _normalize_type(raw_type: str) -> str:
    """Normalize a DuckDB column type using quill's type_mapping.parse_type.

    Quill SDK uses sqlglot to parse database-specific types into canonical
    SQL forms (e.g. "character varying" → "VARCHAR", "INT8" → "BIGINT").
    Falls back to uppercase raw type if quill SDK is not importable.
    """
    try:
        from quill.type_mapping import parse_type  # noqa: PLC0415

        return parse_type(raw_type, "duckdb")
    except ImportError:
        # Fallback if quill SDK is not installed in the environment
...
def _resolve_catalog(duckdb_path: Path) -> str:
    ...
    When quill engine connects to a DuckDB file, it runs:
        ATTACH DATABASE 'path/to/file.duckdb' AS "<stem>" (READ_ONLY)
    where <stem> is the filename without extension. This means the catalog
    in table_reference MUST match the filename stem, otherwise quill engine
...
def discover_tables(
    ...
    *,
    ...
    Args:
        ...
        catalog_name: The catalog name that quill engine will use when
...
def generate_project_files(
    ...
    *,
    ...
    files["quill_project.yml"] = yaml.dump(
    ...
    rules = (
        "# Project rules\n\n"
        f"This Quill project was auto-generated from a dlt DuckDB pipeline.\n\n"
...
def write_project(files: dict[str, str], output_dir: Path, *, force: bool = False):
    ...
    project_file = output_dir / "quill_project.yml"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a Quill project from a dlt DuckDB file."
    )
    parser.add_argument("--duckdb-path", required=True, help="Path to the .duckdb file")
    parser.add_argument(
        "--output-dir",
        default=".",
        help="Directory to write the Quill project (default: current dir)",
    )
    parser.add_argument(
        "--project-name",
        default=None,
    ...
    # The catalog must match the filename stem — this is how quill engine's
    ...
    try:
        ...
        print(f"\nQuill project written to {output_dir}/")
        print(f"  {len(tables)} models, {len(relationships)} relationships")
        print("\nNext steps:")
        print(f"  cd {output_dir}")
        print("  quill context validate")
        print("  quill context build")
