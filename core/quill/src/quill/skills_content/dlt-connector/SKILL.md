---
name: dlt-connector
description: "Connect SaaS data (HubSpot, Stripe, Salesforce, GitHub, Slack, etc.) to Quill Engine for SQL analysis. Guides the user through the full flow: install dlt, pick a SaaS source, set up credentials, run the data pipeline into DuckDB, then auto-generate a Quill semantic project from the loaded data. Use this skill whenever the user mentions: connecting SaaS data, importing data from an API, dlt pipelines, loading HubSpot/Stripe/Salesforce/GitHub/Slack data, querying SaaS data with SQL, or setting up a new data source from a REST API. Also trigger when the user already has a dlt-produced DuckDB file and wants to create a Quill project from it."
license: Apache-2.0
metadata:
  author: quill
---
...
> Reference docs (`dlt_sources`) and the `introspect_dlt` script are bundled. Pull references with `quill skills get dlt-connector --full`; fetch a script with `quill skills get dlt-connector --script <name>`.

Connect SaaS data to Quill Engine for SQL analysis — from zero to a verified, queryable project in one conversation.

## Who this is for

Data analysts who know SQL and some Python, but may not have used dlt or Quill before. Explain concepts briefly when they first appear, but don't over-explain things a SQL-literate person would already know.
...
2. **Model** — Introspect the DuckDB schema and auto-generate a Quill semantic project (YAML models, relationships, profile)
...
When quill engine connects to a DuckDB file, it ATTACHes it using the filename (without `.duckdb` extension) as the catalog alias:
...
Column types must be normalized using quill SDK's `type_mapping.parse_type()` function, which uses sqlglot to convert database-specific types (like DuckDB's `HUGEINT`, `TIMESTAMP WITH TIME ZONE`) into canonical SQL types that quill-core understands. Do not hardcode type mappings — always delegate to `parse_type(raw_type, "duckdb")`.

The `introspect_dlt.py` script does this automatically when quill SDK is installed.
...
## Phase 2: Model — Generate Quill Project

Run the introspection script to auto-generate a complete Quill project from the DuckDB file:

```bash
# first fetch the script: quill skills get dlt-connector --script introspect_dlt > introspect_dlt.py
...
- **Sets `table_reference.catalog` to the DuckDB filename stem** (matching quill engine's ATTACH behavior)
...
- **Normalizes column types using `quill.type_mapping.parse_type()`** (sqlglot-based)
- Generates a complete v5 YAML project (quill_project.yml, models/, relationships.yml, knowledge/rules/)

After running, show the user what was generated:

```bash
# Show project summary
cat <project-directory>/quill_project.yml
echo "---"
...
Create a Quill profile so the user can query without specifying connection details every time. The `url` must point to the **directory containing** the `.duckdb` file (not the file itself):

```python
import yaml
from pathlib import Path

quill_home = Path.home() / ".quill"
quill_home.mkdir(exist_ok=True)
profiles_file = quill_home / "profiles.yml"
...
quill --sql 'SELECT COUNT(*) as total FROM "<table_name>"'
```
...
- Missing profile → check `quill profile list`
...
```bash
# Preview data
quill --sql 'SELECT * FROM "<table_name>" LIMIT 5'

# If there's a relationship, verify both models are queryable
quill --sql 'SELECT * FROM "<parent>" LIMIT 5'
quill --sql 'SELECT * FROM "<child>" LIMIT 5'
```

Show the results to the user and explain what they're seeing. This is their first look at the data through Quill — make it count.
...
`quill skills get genbi`. Its snapshot source is the very `.duckdb` file this
pipeline produced.

## Troubleshooting

If `quill context build` fails:
- Check that `data_source: duckdb` is set in `quill_project.yml`
- Verify the DuckDB file path in the profile is correct
- Run `quill context validate` for detailed error messages
...
- Re-run `introspect_dlt.py` with quill SDK installed to get proper type normalization

General:
- Check that the profile is active: `quill profile list`
- The DuckDB file might be locked if a dlt pipeline is running — wait for it to finish

## Important notes

- dlt's `_dlt_parent_id` / `_dlt_id` columns are kept in the actual DuckDB tables but hidden from Quill model definitions. They're only used in relationship conditions.
...
- Column types are normalized using quill SDK's `parse_type()` with sqlglot's DuckDB dialect. If a type looks wrong, the user can edit the model's `metadata.yml` directly.
