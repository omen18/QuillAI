---
name: generate-mdl
description: "Generate a Quill MDL project by exploring a database with available tools (SQLAlchemy, database drivers, MCP connectors, or raw SQL). Guides agents through schema discovery, type normalization, and MDL YAML generation using the quill CLI. Use when: user wants to create or set up a new MDL, onboard a new data source, or scaffold a project from an existing database."
license: Apache-2.0
metadata:
  author: quill
---

# Generate Quill MDL — CLI Agent Workflow

Builds an MDL project by discovering database schema and converting it
into Quill's YAML project format. The agent uses whatever database tools
are available in its environment for introspection; the quill CLI handles
type normalization, validation, and build.

For memory and query workflows after setup, run `quill skills get usage`.

## Prerequisites

- `quill` CLI installed (`pip install "quill[<datasource>]"`)
- A working database connection (credentials available to the agent)
- A connection profile (set up via `quill profile add`) or connection info ready

## Phase 0 — Detect existing project

**Goal:** If the current directory is already inside a quill project, let the user decide how to proceed.

Check whether `quill_project.yml` exists in the current working directory
(or any parent up to the repository root). If found:

1. Tell the user that an existing quill project was detected and show its path.
2. Ask:
   - **Reset** — wipe the existing project (`models/`, `views/`,
     `relationships.yml`, `knowledge/`, and rebuild `quill_project.yml`)
   ...
   - **New path** — keep the existing project untouched and choose a
     ...
     then `quill context init --path <new_path>` and continue from Phase 1
     using that path.
...
1. Verify connectivity using whichever tool is available:
   ...
   - If quill profile exists: `quill profile debug` to check config
   - If raw SQL via quill: `quill --sql "SELECT 1"` (requires profile or connection file)

2. Ask the user:
   - Which **schema(s)** or **dataset(s)** to include (skip if only one exists)
   - Whether to include **all tables** or a subset
   - The **datasource type** for quill (e.g., `postgres` (including Aurora), `mysql` (including Aurora), `bigquery`, `snowflake`) — needed for type normalization dialect
...
### Option C: Raw SQL via quill

If no driver is available but a quill profile is configured, query
`information_schema` through quill itself:

```bash
quill --sql "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'" -o json
quill --sql "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'orders'" -o json
```
...
**Goal:** Convert raw database types to quill-core-compatible types.

### Python import (recommended for batch processing)

```python
from quill.type_mapping import parse_type, parse_types
...
echo '[{"column":"id","raw_type":"int8"},{"column":"name","raw_type":"character varying"}]' \
  | quill utils parse-types --dialect postgres
```
...
```bash
quill context init --path /path/to/project
```
...
>
> These are Quill Engine's internal namespace — they are NOT the database's
> native catalog or schema. Keep the defaults (`catalog: quill`, `schema: public`)
...
> (see Step 2). Do not copy database catalog/schema values into `quill_project.yml`.
...
These descriptions are indexed by `quill memory index` and significantly
...
quill context validate --path /path/to/project

# If strict mode is desired:
quill context validate --path /path/to/project --strict

# Build JSON manifest
quill context build --path /path/to/project

# Verify against database
quill --sql "SELECT * FROM <model_name> LIMIT 1"
```
...
After this step, `quill memory fetch` and `quill memory recall` are
operational. See `quill skills get usage` for query workflows.
...
Each change follows: edit YAML → `quill context validate` →
`quill context build` → `quill memory index`.
...
| Normalize types (Python) | `from quill.type_mapping import parse_type` |
| Normalize types (CLI) | `quill utils parse-type --type T --dialect D` |
| Normalize types (batch) | `quill utils parse-types --dialect D < columns.json` |
| Scaffold project | `quill context init` |
| Write models | Create `models/<name>/metadata.yml` |
| Write relationships | Edit `relationships.yml` |
| Validate | `quill context validate` |
| Build manifest | `quill context build` |
| Test query | `quill --sql "SELECT * FROM <model> LIMIT 1"` |
| Index memory | `quill memory index` |
...
- Do not skip `quill memory index` after build — stale indexes degrade recall quality
