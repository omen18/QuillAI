...
2. **Every MDL edit must validate.** Right after any MDL YAML change, run `quill context validate`. If it fails, **revert that single change** and feed the error back. Never leave a project in an invalid state.
...
### Step 1 — Choose the Quill project

**Always ask the user which project to enrich before doing anything else** — never assume cwd. A user can have several Quill projects and an ambient `~/.quill` profile that doesn't match the one they want to augment.

Offer concrete hints in the question so the user can answer in one round-trip:

```bash
# Hint 1 — does cwd look like a project?
test -f quill_project.yml && pwd

# Hint 2 — does ~/.quill/config.yml point at a default project?
grep -E '^project_path:' ~/.quill/config.yml 2>/dev/null
```

Then ask:

> Which Quill project do you want me to augment?
> a) `$PWD` (current directory)         ← if Hint 1 matched
> b) `<path from ~/.quill/config.yml>`   ← if Hint 2 matched
...
```bash
cd <chosen-path>
test -f quill_project.yml || {
  echo "Error: <chosen-path> is not a Quill project (no quill_project.yml)."
  exit 1
}
quill context show >/dev/null 2>&1 || {
  echo "Error: quill context show failed — manifest may be invalid."
  exit 1
}
```

If either check fails, stop and tell the user — suggest `quill skills get onboarding` if it's not a project, or `quill context validate` if the manifest is broken.
...
```bash
quill memory status 2>/dev/null | grep -q "Backend: lancedb"
```

Writing NL→SQL pairs (`quill memory store` → `knowledge/sql/*.md`) works **regardless** — that
...
- Match (`Backend: lancedb`) → set `MEMORY_AVAILABLE = true`. Semantic recall and `quill memory fetch` are usable, and `quill memory index` builds an embedding index in Step 8.
- No match (grep backend) → set `MEMORY_AVAILABLE = false`. Skip the semantic read/index paths below; pair writeback still happens via `quill memory store`, and grep recall still works.
...
Read both sides — the raw material and the current Quill context — before forming any opinion.
...
### Current Quill context

| Source | Command |
|---|---|
| MDL (full) | `quill context show --output json` |
| Business rules | `quill context instructions` (reads `knowledge/rules/` + any legacy `instructions.md`) |
| Existing cubes (names) | `quill cube list` |
| Existing cubes (measures + dimensions) | `quill cube describe <cube>` for each name above |
| NL→SQL pairs | read `knowledge/sql/*.md` directly |
| (Memory) stored pairs | `quill memory list -n 200 --output json` |
| (Memory) schema as text | `quill memory describe` |
...
```bash
quill --sql "SELECT DISTINCT <col> FROM <model> LIMIT 30" --output json
# For magic sentinels (catalog #5), also fetch min/max:
quill --sql "SELECT MIN(<col>) AS lo, MAX(<col>) AS hi FROM <model>" --output json
```
...
For each raw file, internally extract 5–15 **atomic claims** — single statements that could be true or false, e.g. "an order has exactly one customer", "user means type=default by default", "ARR equals MRR × 12 minus refunds". Then for each claim, classify against the current Quill context:
...
3. Write back.
4. Run `quill context validate` immediately after any MDL edit. On failure: revert the single change and log the revert with the error.
...
| Aggregation metric / named measure (with measures + dimensions) | **`cubes/<name>/metadata.yml`** | New file per cube. Default sink for any `SUM` / `COUNT` / `AVG` / ratio metric raw defines or Lane 3 infers. See `cube_proposals` for the YAML template, naming policy, duplication guard, and validation flow. Run `quill context validate` + `quill cube query --cube <name> --sql-only` after writing; revert on either failure. **Always escalates to grill in auto-pilot** (Universal Rule 7b). |
...
| NL→SQL example (canonical or ad-hoc) | **`knowledge/sql/`** | `quill memory store --nl "..." --sql "..." --tags "source:enrich"` — writes `knowledge/sql/<slug>.md` (committable) and indexes it when the extra is present. Works whether or not `MEMORY_AVAILABLE`. |
...
- NL→SQL pairs are written by `quill memory store`; each becomes a `knowledge/sql/<slug>.md` with YAML frontmatter (`nl`, `sql`, `source`, optional `datasource`/`tags`). Don't hand-write the files — use `quill memory store --tags "source:enrich"`.
- MDL YAML uses snake_case keys (e.g. `primary_key`, `is_calculated`, `not_null`). `quill context build` converts to camelCase for `target/mdl.json`.
...
rules in `knowledge/rules/` are read by `quill context instructions`, not embedded by `index`.)
...
```text
Quill Enrich Context — session summary  (mode: <grill|autopilot>)
...
Added:
  ...
  knowledge/sql/   : N new NL→SQL pairs                                       via quill memory store
...
```text
Skipped this session: N gaps (re-run /quill-enrich-context to revisit)
```
...
Validation:
  K successful applies, M reverted after quill context validate failed:
...
- `quill memory store` works whether or not `MEMORY_AVAILABLE` — it always writes the `knowledge/sql/*.md` pair (and indexes it only when the extra is present). No need to fall back to another sink.
...
- Do not run `quill context build` after every single MDL edit — once at the end is enough. Do run `quill context validate` after every edit.
- Do not assume `raw/` was created by `quill context init` — it isn't. This skill creates it.
...
- Do not skip `quill cube query --cube <name> --sql-only` after creating a cube. Structural `quill context validate` doesn't catch unresolvable measure / dimension expressions.
