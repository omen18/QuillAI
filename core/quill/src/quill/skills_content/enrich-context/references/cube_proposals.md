...
**Why cube is the default**: the Quill docs call cubes the "highest-leverage correctness primitive" for smaller models. Agents pick wrong joins, double-count, and mis-truncate dates when forced to write aggregation SQL by hand. Cubes pre-declare those decisions once.
...
```bash
quill cube list                       # all existing cube names
quill cube describe <cube_name>       # measures + expressions per cube
```

For each measure you're about to propose:

- **Same expression already exists in another cube** (e.g. `SUM(amount)` for the same `base_object`) → do **not** propose a new cube. Store a `knowledge/sql/` example (via `quill memory store`) pointing at the existing cube instead, so the agent learns to reach for it.
- **Same name in `quill cube list`** but different `base_object` → name collision. Either fall back to `<name>_v2` (auto-pilot) or grill the user for a better name (grill mode).
- **Old MDL `metrics:` already defines this** (visible in `quill context show --output json` under each model's `metrics:` array) → do not propose. Surface on the Step 9 "please fix manually" list with the note "old metrics: entry — consider migrating to a cube".
...
base_object: <model_or_view>  # MUST already exist in the project — verify with quill context show
measures:
  - name: total
...
The `base_object` must already exist as a model name or view name. Check with `quill context show --output summary`. If raw's metric crosses multiple tables (e.g. "revenue per customer segment" needs `orders ↔ customers`), the correct path is:

1. If a relationship already exists → cube can still use one base model and the related column is reachable via the relationship's calculated column on that model. Verify by inspecting `quill context show --output json`.
...
quill cube query --cube <name> --measures <first_measure> --sql-only
```

**On failure:**

- `quill context validate` error → revert the cube YAML file (delete it), log the error to the audit, move on.
- `quill cube query --sql-only` error → revert, surface the specific measure / dimension that won't compile, and either grill (grill mode) or skip (auto-pilot).

Never leave a project with a cube YAML that doesn't pass both checks. A broken cube poisons `quill cube list` for every future agent session.

## Auto-pilot escalation

Cubes are **always** high-blast-radius — a new cube YAML becomes a public name in `quill cube list` that every future agent sees. In auto-pilot, treat every cube proposal as a Universal Rule 7(b) escalation: drop into grill, ask the user, then either apply or skip. This holds even when the cube comes from a Lane 2 NEW claim (raw explicitly defined the metric) — the artifact's blast radius doesn't depend on inference confidence.
...
```bash
quill memory store --tags "source:enrich" \
  --nl "daily active users for last week" \
  --sql "SELECT day, dau FROM (<result of: quill cube query --cube daily_engagement --measures dau --time-dimension 'day:day:2024-01-01,2024-01-08'>)"
```

(Or simpler — log it in the Step 9 audit and let the agent reach for `quill cube query` directly at usage time.)
...
- Do not write a cube whose `base_object` doesn't exist — `quill context validate` will fail and you'll revert anyway.
