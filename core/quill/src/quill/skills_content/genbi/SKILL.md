---
name: genbi
description: "Turn a Quill project's context layer into a shareable, browser-side GenBI web app and deploy it to the user's Vercel or Cloudflare account. Orchestrates the full flow: `quill genbi build` returns a project-hydrated build instruction, the agent authors the app from scratch into apps/<name>/, then register → verify → deploy produce a shareable URL. Use this skill whenever the user wants to: build a dashboard from their Quill project, make a shareable analytics app, deploy their context layer as a web app, host a GenBI app on Vercel or Cloudflare Pages, or asks for a 'genbi app'."
license: Apache-2.0
metadata:
  author: quill
---

# Quill GenBI App — Agent Workflow Guide

> This guide is served by the `quill` CLI (`quill skills get genbi`), so it
> always matches your installed quill version.

Turn a Quill context layer into a shareable GenBI app — from a natural-language
request to a public URL in one conversation.

**Division of labor:** the CLI owns the authoritative build instruction (it
knows the live project facts and the pinned `quill-core-wasm` version) and all
deterministic state (index, verify, deploy). You — the agent — author the app
code by following the instruction. Never hand-write `.quill/apps.yml`.

## Preconditions

1. A Quill project is discoverable (`quill_project.yml` in cwd/ancestors, or ask
   for the path and pass `-p`).
2. The context layer exists. If `target/mdl.json` is missing, `quill genbi
   build` compiles it implicitly — no separate step needed.
3. `quill` CLI ≥ the version that ships the `genbi` command group
   (`quill genbi --help` works).
...
```bash
quill genbi build <name> --prompt "<the user's request, verbatim>" --data-mode <mode>
```
...
- Load `quill-core-wasm` from the CDN given in the instruction; never bundle
  the ~68MB binary.
...
- **DuckDB-backed project** (incl. anything loaded by the `dlt-connector`
  ...
  first: `quill skills get dlt-connector`. Then come back here to ship it.
...
```bash
# from the project root; <db> is the project's DuckDB file (see quill_project.yml)
python - <<'PY'
import duckdb
...
```bash
quill genbi register <name> --data-mode <mode>
quill genbi verify <name>
```

If verify fails: fix the reported problems and re-run verify. Do NOT proceed
to deploy on a failed verify. Offer `quill genbi open <name>` for a local
preview before shipping.

### 5. Deploy (only if the user asked for it)

```bash
quill genbi deploy <name> --provider vercel      # or cloudflare
```
...
- To make the app publicly shareable, the user disables it in the Vercel
  ...
  is not controllable from `quill genbi deploy`; it's a one-time toggle per
  project in Vercel.
...
- All index state goes through `quill genbi register/remove` — never edit
  `.quill/apps.yml` by hand.

## Quick reference

| Step | Command |
| --- | --- |
| Get build instruction | `quill genbi build <name> --prompt "…" [--data-mode snapshot\|live]` |
| Record the app | `quill genbi register <name> --data-mode <mode>` |
| Preflight | `quill genbi verify <name>` |
| Local preview | `quill genbi open <name>` |
| Ship | `quill genbi deploy <name> --provider vercel\|cloudflare [--prod]` |
| Inventory | `quill genbi list` / `quill genbi remove <name>` |
