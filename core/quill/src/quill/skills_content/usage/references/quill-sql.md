# Quill SQL — How CTE-Based Modeling Works

Quill Engine rewrites your SQL by injecting CTEs (Common Table Expressions) that expand each MDL model into its underlying database query. Understanding this mechanism helps you diagnose errors and write correct SQL.
...
Your SQL (target dialect, e.g. Postgres)
  ...
  → per model: quill-core expands the model definition → CTE
...
| Calculated fields / metrics | Yes — quill-core expands them inside the model CTE |
...
```bash
quill dry-plan --sql "SELECT o_custkey, SUM(o_totalprice) FROM orders GROUP BY 1"
```
...
| **Fails** with "No model references found" | Your FROM clause doesn't match any MDL model name | Check model names: `quill memory fetch -q "<name>" --type model --threshold 0` |
| **Fails** with column error | A column you referenced doesn't exist in the model | Check columns: `quill memory fetch -q "<col>" --model <name> --threshold 0` |
...
quill dry-plan --sql "SELECT ..." 2>&1

# Run against DB and capture the error
quill --sql "SELECT ..." 2>&1
```
...
If the rewriter detects no model references in your SQL (e.g. `SELECT 1` or queries against raw database tables), it falls back to passing the entire query through quill-core's `transform_sql()` directly. This means:
...
`quill cube query` doesn't execute SQL directly — it produces SQL from a
...
- **`FROM` is the cube's `baseObject`** — quill-core then resolves it to the
...
1. `quill cube query --sql-only ...` to inspect the generated SQL.
2. If the SQL looks reasonable, run `quill cube query ...` (drop `--sql-only`).
