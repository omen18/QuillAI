# Quill Memory — When to index, context, store, and recall
...
| Command | When to use |
|---------|-------------|
| `quill memory fetch -q "..."` | Default. Auto-selects full text (small schema) or embedding search (large schema) based on a 30K-char threshold. |
| `quill memory fetch -q "..." --type T --model M` | When you need filtering (forces search strategy on large schemas). |
| `quill memory describe` | When you want the full schema text and know it is small. |
...
```bash
quill memory fetch -q "revenue" --threshold 50000   # raise for larger context windows
```

### Cube schema items

When the MDL defines cubes, `quill memory index` emits these additional schema items:
...
These items are reachable via `quill memory fetch "<question>"`. For aggregation
...
the agent should follow up with `quill cube describe <cube>` and `quill cube query`
rather than hand-writing `GROUP BY` SQL.

`quill memory describe` also adds a cube section that lists each cube's measures,
dimensions, time dimensions, and hierarchies in markdown.

---

## Indexing: `quill memory index`

**When to index:**
- After updating model YAML files and rebuilding (`quill context build`)
- When `quill memory status` shows `schema_items: 0 rows`
- When `quill memory fetch` returns stale results (references deleted models)
...
sources change, run `quill memory watch` — it polls `target/mdl.json` and
`knowledge/sql/*.md` and reindexes automatically on change, so `fetch` never serves a
stale schema.

```bash
quill memory watch   # poll + auto-reindex until Ctrl+C
```

---

## Storing queries: `quill memory store`
...
## Recalling queries: `quill memory recall`
...
```bash
quill memory recall -q "monthly revenue by category" --limit 3
```
...
```
Session start:
  1. quill memory status → if schema_items is 0: quill memory index

User asks a question:
  2. quill memory recall -q "<question>" --limit 3
  3. quill memory fetch -q "<question>"
  4. Write SQL using recalled examples + schema context
  5. quill --sql "..."

After execution:
  6. Show results to user
  7. Store by default → quill memory store --nl "..." --sql "..."
...
```bash
quill memory status              # path, table names, row counts
quill memory reset --force       # drop everything, start fresh
```

All memory commands accept `--path DIR` to override the default storage directory (`<project>/.quill/memory/`, falling back to `~/.quill/memory/` outside a project).
