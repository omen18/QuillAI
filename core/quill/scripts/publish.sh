#!/usr/bin/env bash
#
# Build and publish quill to PyPI or TestPyPI.
#
# Usage:
...
VERSION=$(sed -nE 's/^__version__\s*=\s*"([^"]+)".*/\1/p' src/quill/__init__.py | head -n1)
if [[ -z "$VERSION" ]]; then
    echo "Error: failed to parse __version__ from src/quill/__init__.py" >&2
    exit 1
fi
echo "==> Building quill v${VERSION}"
...
if [[ "$REPOSITORY" == "testpypi" ]]; then
    ...
    echo "    pip install --index-url https://test.pypi.org/simple/ quill"
else
...
echo ""
echo "==> Done! Published quill v${VERSION} to ${REPOSITORY}"
