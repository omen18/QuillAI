"""Unit tests for quill.profile — profile CRUD and resolution logic."""
...
def _write_project(project_path: Path, **fields) -> None:
    """Write a minimal quill_project.yml with the given fields."""
    ...
    (project_path / "quill_project.yml").write_text(yaml.safe_dump(config))
...
def test_resolve_profile_for_project_raises_on_malformed_yaml(tmp_path: Path):
    """A broken quill_project.yml should fail loudly, not silently fall back to
    the global active profile — the latter risks targeting the wrong DB."""
    proj = tmp_path / "myproj"
    proj.mkdir()
    (proj / "quill_project.yml").write_text("schema_version: 3\nname: [unclosed\n")

    with pytest.raises(SystemExit) as exc:
        profile_mod.resolve_profile_for_project(proj)
    msg = str(exc.value).lower()
    assert "quill_project.yml" in msg or "yaml" in msg
