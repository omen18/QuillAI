"""`quill.cli` must detect the optional `memory` extra without
...
def _fresh_import_modules() -> dict[str, bool]:
    """Import quill.cli in a clean subprocess and report loaded heavy modules."""
    code = (
        "import sys, importlib; importlib.import_module('quill.cli'); "
...
def test_memory_subcommand_registered_when_extra_present():
    ...
    cli = importlib.import_module("quill.cli")
...
def test_memory_subcommand_always_registered():
    """`memory` is always registered — `quill memory store` writes knowledge/sql/*.md
