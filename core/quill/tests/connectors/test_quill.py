"""Quill connector tests.

Quill Enterprise speaks the Postgres wire protocol, so we point a
``PostgresContainer`` at the connector and stand up tables with the data
types Quill publishes (Trino-style VARCHAR/DECIMAL/ARRAY/ROW/MAP plus the
...
from quill.connector.quill import (
    QuillConnector,
    _arrow_type,
    _build_column
)
from quill.connector.base import strip_trailing_semicolon as _strip_trailing_semicolon
from quill.model import QuillConnectionInfo

psycopg = pytest.importorskip("psycopg")
testcontainers_postgres = pytest.importorskip("testcontainers.postgres")
PostgresContainer = testcontainers_postgres.PostgresContainer

pytestmark = pytest.mark.quill


_FIXTURE_DDL = """
    CREATE TABLE quill_demo (
...
def test_arrow_type_maps_quill_scalars() -> None:
...
def test_dry_run_returns_none_contract() -> None:
    ...
    connector = QuillConnector.__new__(QuillConnector)
...
def quill_connector():
    with PostgresContainer("postgres:16") as pg:
        ...
        # QuillConnectionInfo treats `workspace` like a postgres database and
        # `pat` like a password — pass them through directly.
        connection_info = QuillConnectionInfo(
        ...
        connector = QuillConnector(connection_info)
        try:
            yield connector
        finally:
            connector.close()
...
def test_quill_connector_query_returns_arrow_table(quill_connector) -> None:
    table = quill_connector.query("SELECT * FROM quill_demo ORDER BY id")
...
def test_quill_connector_query_applies_limit(quill_connector) -> None:
    table = quill_connector.query("SELECT * FROM quill_demo ORDER BY id", limit=1)
    assert table.num_rows == 1


def test_quill_connector_query_preserves_duplicate_column_names(
    quill_connector,
) -> None:
    ...
    table = quill_connector.query(
        "SELECT a.id, b.id FROM quill_demo a, quill_demo b ORDER BY a.id, b.id LIMIT 1"
    )
    assert table.num_columns == 2
    assert [field.name for field in table.schema] == ["id", "id"]


def test_quill_connector_dry_run_succeeds(quill_connector) -> None:
    # Returns None and must not raise on a valid statement.
    assert quill_connector.dry_run("SELECT 1 AS x") is None


def test_quill_connector_dry_run_raises_for_invalid_sql(quill_connector) -> None:
    from quill.model.error import QuillError  # noqa: PLC0415

    with pytest.raises(QuillError):
        quill_connector.dry_run("SELECT * FROM no_such_table")


def test_quill_connector_preserves_unconstrained_numeric_precision(
    quill_connector,
) -> None:
    ...
    table = quill_connector.query(f"SELECT '{literal}'::numeric AS n")
    assert table.schema.field("n").type == pa.string()
    assert table.to_pylist() == [{"n": literal}]


def test_quill_connector_query_wraps_sql_with_trailing_semicolon(
    quill_connector,
) -> None:
    ...
    table = quill_connector.query("SELECT 1 AS x;", limit=1)
    assert table.num_rows == 1
    assert table.to_pylist() == [{"x": 1}]


def test_quill_connector_dry_run_wraps_sql_with_trailing_semicolon(
    quill_connector,
) -> None:
