"""Integration tests for the ``quill profile`` CLI sub-app."""
...
import quill.profile as profile_mod
from quill.profile_cli import profile_app
...
def isolated_profiles(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(profile_mod, "_QUILL_HOME", tmp_path)
...
def test_add_minimal_skips_validation():
    ...
    assert "Next: quill context init" in result.output
...
def test_add_from_file_runs_validation_by_default(tmp_path, monkeypatch):
    ...
    import quill.profile_cli as profile_cli  # noqa: PLC0415
...
def test_add_from_file_suppresses_next_hint_on_validation_failure(
    ...
    """When validation fails, the misleading ``Next: quill context init``
    ...
    result = runner.invoke(profile_app, ["add", "duck", "--from-file", str(conn_file)])
    assert result.exit_code == 0
    assert "Next: quill context init" not in result.output
...
def test_validate_success(tmp_path, monkeypatch):
    ...
    from quill import profile_cli  # noqa: PLC0415
    ...
    monkeypatch.setattr(
        "quill.connector.factory.get_connector", lambda ds, info: FakeConnector()
    )
...
def test_validate_failure_prints_warning(monkeypatch):
    ...
    monkeypatch.setattr("quill.connector.factory.get_connector", failing_get_connector)

    buf = _capture_typer_echo(monkeypatch)
    profile_cli._validate_connection("pg")
    output = buf.getvalue()
    assert "Connection failed: connection refused" in output
    assert "quill profile debug" in output
    assert "quill profile add pg --ui" in output
...
def test_validate_invalid_connection_info(monkeypatch):
    ...
    monkeypatch.setattr("quill.connector.factory.get_connector", should_not_be_called)
