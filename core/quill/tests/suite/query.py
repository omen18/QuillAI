"""Shared query test suite for QuillEngine connector tests.
    ...
    class TestClickHouse(QuillQueryTestSuite):
        ...
        def engine(self):                        # (4) engine fixture — class-scoped
            with ClickHouseContainer() as ch:
                ...
                with QuillEngine(manifest_str, DataSource.clickhouse, conn_info, fallback=False) as e:
                    yield e
...
All ``QuillQueryTestSuite`` tests run automatically.  Any ``test_*`` methods you
...
define a separate mix-in class and include it only where relevant::
    ...
    class TestPostgres(QuillQueryTestSuite, TimezoneTestSuite):
        ...   # gets core tests + timezone tests

    # tests/connectors/test_duckdb.py
    class TestDuckDB(QuillQueryTestSuite):
...
class QuillQueryTestSuite:
    ...
    def test_basic_select(self, engine: QuillEngine) -> None:
        result = engine.query(
    ...
    def test_count(self, engine: QuillEngine) -> None:
        result = engine.query('SELECT COUNT(*) AS cnt FROM "orders"')
        assert result["cnt"][0].as_py() == self.order_count

    def test_query_with_limit(self, engine: QuillEngine) -> None:
    ...
    def test_calculated_field(self, engine: QuillEngine) -> None:
        result = engine.query(
    ...
    def test_explicit_join(self, engine: QuillEngine) -> None:
        result = engine.query(
            """
    ...
    def test_order_id_dtype(self, engine: QuillEngine) -> None:
    ...
    def test_dry_run_valid(self, engine: QuillEngine) -> None:
        # Should not raise
        engine.dry_run('SELECT * FROM "orders" LIMIT 1')

    def test_dry_run_invalid_table(self, engine: QuillEngine) -> None:
    ...
    def test_dry_plan_returns_sql(self, engine: QuillEngine) -> None:
