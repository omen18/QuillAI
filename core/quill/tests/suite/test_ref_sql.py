...
QuillEngine pipeline: dry_plan → query → result.
"""
...
def engine(tmp_path_factory):
    """QuillEngine backed by DuckDB with TPCH sf=0.01 data."""
    ...
    with QuillEngine(manifest_str, DataSource.duckdb, conn_info, fallback=False) as e:
        yield e
...
class TestRefSqlDryPlan:
    ...
    def test_dry_plan_ref_sql_model(self, engine: QuillEngine) -> None:
    ...
    def test_dry_plan_ref_sql_aggregate_model(self, engine: QuillEngine) -> None:
    ...
    def test_dry_plan_table_ref_model_still_works(self, engine: QuillEngine) -> None:
...
class TestRefSqlQuery:
    ...
    def test_query_ref_sql_returns_rows(self, engine: QuillEngine) -> None:
        result = engine.query(
    ...
    def test_query_ref_sql_aggregate(self, engine: QuillEngine) -> None:
        result = engine.query(
    ...
    def test_query_ref_sql_with_where(self, engine: QuillEngine) -> None:
    ...
    def test_query_ref_sql_count(self, engine: QuillEngine) -> None:
    ...
    def test_query_table_ref_model_coexists(self, engine: QuillEngine) -> None:
    ...
    def test_query_ref_sql_with_order_by(self, engine: QuillEngine) -> None:
        result = engine.query(
            'SELECT o_orderkey, o_totalprice FROM "high_value_orders" '
