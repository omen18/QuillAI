"""Unit tests for quill.context — load/validate/build YAML→JSON."""
...
def test_get_schema_version_default(tmp_path):
    (tmp_path / "quill_project.yml").write_text("name: test\ndata_source: postgres\n")
...
def _make_v2_project(tmp_path: Path, schema_version: int = 2) -> Path:
    """Write quill_project.yml with the given schema_version."""
    (tmp_path / "quill_project.yml").write_text(
...
def test_load_knowledge_rules_concatenates_sorted(tmp_path):
    from quill.context import load_knowledge_rules  # noqa: PLC0415
...
def test_get_knowledge_schema_version(tmp_path):
    from quill.context import create_knowledge_skeleton, get_knowledge_schema_version  # noqa: PLC0415
...
def test_save_target_creates_dir(tmp_path):
    _make_v2_project(tmp_path)
    manifest = {"catalog": "quill", "schema": "public", "models": []}
    out = save_target(manifest, tmp_path)
    assert out.exists()
    assert out.name == "mdl.json"
    loaded = json.loads(out.read_text())
    assert loaded["catalog"] == "quill"
...
def test_discover_no_project_raises(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("QUILL_PROJECT_HOME", raising=False)
    # Use a non-existent QUILL_HOME so no config.yml can be found
    monkeypatch.setenv("QUILL_HOME", str(tmp_path / "empty_quill_home"))
    import importlib  # noqa: PLC0415

    import quill.context as ctx  # noqa: PLC0415

    importlib.reload(ctx)
    with pytest.raises(SystemExit, match="no quill project found"):
        ctx.discover_project_path()


def test_discover_via_env_var(tmp_path, monkeypatch):
    """QUILL_PROJECT_HOME env var overrides cwd walk."""
    project_dir = tmp_path / "my_project"
    project_dir.mkdir()
    (project_dir / "quill_project.yml").write_text("name: test\ndata_source: pg\n")
    monkeypatch.setenv("QUILL_PROJECT_HOME", str(project_dir))
    result = discover_project_path()
    assert result == project_dir


def test_discover_via_config(tmp_path, monkeypatch):
    """~/.quill/config.yml default_project used as last fallback."""
    project_dir = tmp_path / "my_project"
    project_dir.mkdir()
    (project_dir / "quill_project.yml").write_text("name: test\ndata_source: pg\n")
    quill_home = tmp_path / "quill_home"
    quill_home.mkdir()
    (quill_home / "config.yml").write_text(f"default_project: {project_dir}\n")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("QUILL_PROJECT_HOME", raising=False)
...
def test_schema_version_5_recognized(tmp_path):
    """v5 quill_project.yml is supported, not reported as unsupported."""
    (tmp_path / "quill_project.yml").write_text(
        "schema_version: 5\nname: test\ndata_source: postgres\ncatalog: quill\nschema: public\n"
    )
...
def test_convert_mdl_preserves_dialect(tmp_path):
    ...
    project = yaml.safe_load(file_map["quill_project.yml"])
...
def test_plan_upgrade_v1_to_v2_rejects_duplicate_cube_targets(tmp_path):
    ...
    from quill.context import UpgradeError as _UE  # noqa: PLC0415
...
def test_plan_upgrade_v4_to_v5_lists_knowledge(tmp_path):
    ...
    assert "quill_project.yml" in result.files_modified
...
_PROJECT_FILE = "quill_project.yml"
...
from quill.context import validate_manifest
