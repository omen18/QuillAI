"""Tests for `quill skills` content delivery (tracer bullet: infra + usage)."""
...
def test_skills_list_includes_usage_with_references():
    ...
    assert "quill-sql" in result.output
...
def test_skills_get_usage_returns_guide():
    ...
    assert "Quill Engine CLI" in result.output
