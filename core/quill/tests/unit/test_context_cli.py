"""Integration tests for the `quill context` CLI sub-app."""
...
def _make_valid_project(tmp_path: Path) -> Path:
    ...
    (tmp_path / "quill_project.yml").write_text(
        'schema_version: 2\nname: test_proj\nversion: "1.0"\n'
    ...
    (view_dir / "sql.yml").write_text(
        "statement: SELECT id, total FROM quill.public.orders\n"
    )
...
def test_init_creates_scaffold(tmp_path):
    ...
    assert (tmp_path / "quill_project.yml").exists()
    ...
    # Verify quill_project.yml contains namespace clarification comments and defaults
    project_yml = (tmp_path / "quill_project.yml").read_text()
    assert "catalog: quill" in project_yml
...
def test_init_refuses_existing(tmp_path):
    (tmp_path / "quill_project.yml").write_text("name: existing\n")
    result = runner.invoke(app, ["context", "init", "--path", str(tmp_path)])
...
def test_print_warnings_collapses_beyond_threshold(capsys):
    ...
    from quill import context_cli  # noqa: PLC0415
...
def test_init_does_not_clobber_existing_rules(tmp_path):
    ...
    general.write_text("My real rules.\n")
    (tmp_path / "quill_project.yml").unlink()  # avoid the conflict-guard early exit
    runner.invoke(app, ["context", "init", "--empty", "--path", str(tmp_path)])
...
def test_init_writes_v5(tmp_path):
    """`quill context init` stamps the latest layout, schema_version 5."""
    result = runner.invoke(app, ["context", "init", "--path", str(tmp_path)])
    assert result.exit_code == 0, result.output
    config = yaml.safe_load((tmp_path / "quill_project.yml").read_text())
...
def test_v5_uses_v2_reader(tmp_path):
    ...
    from quill.context import load_models, load_views  # noqa: PLC0415

    def _populate(root: Path, sv: int) -> Path:
        root.mkdir(parents=True)
        (root / "quill_project.yml").write_text(
            f"schema_version: {sv}\nname: t\ndata_source: postgres\n"
            "catalog: quill\nschema: public\n"
        )
...
def test_validate_fail(tmp_path):
    ...
    (tmp_path / "quill_project.yml").write_text("schema_version: 2\nname: broken\n")
    result = runner.invoke(app, ["context", "validate", "--path", str(tmp_path)])
    assert result.exit_code == 1
...
def test_upgrade_cli_default_to_latest(tmp_path):
    from quill.context import _LATEST_SCHEMA_VERSION  # noqa: PLC0415
...
def test_import_dbt_dry_run(tmp_path):
    ...
    output_dir = tmp_path / "quill_project"
    ...
    assert result.exit_code == 0, result.output
    assert "Dry run" in result.output
    assert "quill_project.yml" in result.output
...
def test_import_dbt_writes_project_and_builds(tmp_path):
    ...
    assert result.exit_code == 0, result.output
    assert (output_dir / "quill_project.yml").exists()
    ...
    config = yaml.safe_load((output_dir / "quill_project.yml").read_text())
    assert config["schema_version"] == 5
...
def test_import_dbt_force_overwrites_managed_files(tmp_path):
    ...
    output_dir = tmp_path / "quill_project"
    output_dir.mkdir()
    (output_dir / "quill_project.yml").write_text("name: old\n")
    ...
    assert "jaffle_shop" in (output_dir / "quill_project.yml").read_text()
...
def test_write_project_files_force_preserves_queries_without_replacement(tmp_path):
    from quill.context import ProjectFile, write_project_files  # noqa: PLC0415

    (tmp_path / "queries.yml").write_text("version: 1\npairs:\n  - nl: keep\n")
    files = [
        ProjectFile(
            relative_path="quill_project.yml",
...
# ── quill context set-profile ──────────────────────────────────────────────


def _isolate_profiles(home_dir: Path, monkeypatch) -> None:
    """Redirect ~/.quill profile I/O to ``home_dir`` for the duration of a test."""
    import quill.profile as profile_mod  # noqa: PLC0415

    home_dir.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(profile_mod, "_QUILL_HOME", home_dir)
...
def test_set_profile_writes_profile_field(tmp_path, monkeypatch):
    ...
    _isolate_profiles(tmp_path / "quill-home", monkeypatch)
    ...
    config = yaml.safe_load((proj / "quill_project.yml").read_text())
...
def test_set_profile_errors_cleanly_when_save_fails(tmp_path, monkeypatch):
    ...
    import quill.context as context_mod  # noqa: PLC0415
...
def test_set_profile_preserves_other_fields(tmp_path, monkeypatch):
    ...
    assert config["catalog"] == "quill"
    assert config["schema"] == "public"
    assert config["schema_version"] == 5


def test_set_profile_preserves_custom_fields(tmp_path, monkeypatch):
    """Unknown / custom fields in quill_project.yml must survive set-profile.
    ...
    (proj / "quill_project.yml").write_text(yaml.safe_dump(config))

    result = runner.invoke(app, ["context", "set-profile", "duck", "--path", str(proj)])
    assert result.exit_code == 0, result.output

    # Round-trip: custom keys survive the rewrite
    config_after = yaml.safe_load((proj / "quill_project.yml").read_text())
...
def test_set_profile_warns_about_stale_mdl_when_datasource_changes(
    ...
    assert result.exit_code == 0, result.output
    assert "quill context build" in result.output
...
def test_set_profile_no_stale_mdl_warning_when_datasource_unchanged(
    ...
    assert result.exit_code == 0, result.output
    assert "quill context build" not in result.output
...
# ── quill context validate — profile binding hint ──────────────────────────
...
def test_validate_warns_when_pinned_profile_missing(tmp_path, monkeypatch):
    ...
    (tmp_path / "quill_project.yml").write_text(yaml.safe_dump(config))
...
def test_validate_hint_shown_even_when_warnings_present(tmp_path, monkeypatch):
    ...
    from quill import context as context_mod  # noqa: PLC0415
