"""Tests for MDL JSON → YAML project conversion (quill context init --from-mdl)."""
...
def test_convert_mdl_to_project():
    ...
    assert "quill_project.yml" in file_map
...
def test_write_project_files_refuses_overwrite(tmp_path: Path):
    (tmp_path / "quill_project.yml").write_text("existing")
    files = convert_mdl_to_project(SAMPLE_MDL)
...
def test_write_project_files_force_overwrites(tmp_path: Path):
    ...
    project = yaml.safe_load((tmp_path / "quill_project.yml").read_text())
    assert project["schema_version"] == 2
...
def test_empty_mdl():
    ...
    mdl = {"catalog": "quill", "schema": "public"}
    files = convert_mdl_to_project(mdl)
    paths = {f.relative_path for f in files}
    assert paths == {"quill_project.yml", "AGENTS.md", "knowledge/knowledge.yml"}
    assert "instructions.md" not in paths


def test_no_data_source():
    """Missing dataSource — quill_project.yml omits data_source field."""
    mdl = {"catalog": "quill", "schema": "public", "models": [], "views": []}
    files = convert_mdl_to_project(mdl)
    project = yaml.safe_load(next(f for f in files if f.relative_path == "quill_project.yml").content)
...
def test_empty_relationships_not_written():
    ...
    mdl = {"catalog": "quill", "schema": "public", "relationships": []}
    files = convert_mdl_to_project(mdl)
...
def test_cli_init_from_mdl(tmp_path: Path, sample_mdl_file: Path):
    ...
    assert (project_dir / "quill_project.yml").exists()
