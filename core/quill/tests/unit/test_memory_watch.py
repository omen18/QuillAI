"""Unit tests for the dependency-free memory watch loop (`quill memory watch`).

These run in the unit CI job: they exercise the change-detection + reindex
loop logic in :mod:`quill.memory.watch` using only the standard library, plus
...
from quill.memory.markdown import write_query_markdown
from quill.memory.watch import (
...
def test_cli_watch_grep_backend_exits(tmp_path, monkeypatch):
    monkeypatch.setenv("QUILL_MEMORY_BACKEND", "grep")
    monkeypatch.setenv("QUILL_PROJECT_HOME", str(tmp_path))
    (tmp_path / "quill_project.yml").write_text("name: t\n", encoding="utf-8")
