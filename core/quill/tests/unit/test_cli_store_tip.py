...
Uses typer's CliRunner with a mocked QuillEngine so no real database is needed.
"""
...
def _mock_engine(result=None):
    ...
    with patch("quill.cli._build_engine", return_value=mock_engine):
        yield mock_engine
...
def test_main_tip_appears_on_stderr_for_analytical_query():
    ...
    assert "quill memory store" in result.stderr
    assert "quill memory store" not in result.stdout
...
def test_main_tip_suppressed_with_quiet_flag():
    ...
    assert result.exit_code == 0
    assert "quill memory store" not in result.stderr
