"""Behavior tests for `quill genbi deploy` — token discovery + providers."""
...
import quill.genbi.providers.cloudflare as cloudflare
import quill.genbi.providers.vercel as vercel
from quill.cli import app
from quill.genbi.tokens import resolve_token
...
def _isolate_env_loading(monkeypatch):
    ...
    ``resolve_token``'s tier-2 calls ``quill.profile._ensure_env_loaded()``,
    which merges ``~/.quill/.env`` (and any cwd ``.env``) into ``os.environ``.
    Without neutralizing it, these tests depend on the developer's machine —
    a real ``VERCEL_TOKEN`` in ``~/.quill/.env`` makes the missing-token cases
    ...
    (tier 3).
    """
    import quill.profile  # noqa: PLC0415

    monkeypatch.setattr(quill.profile, "_ensure_env_loaded", lambda: None)
...
def test_deploy_vercel_uploads_and_persists_state(tmp_path: Path, monkeypatch) -> None:
    ...
    index = yaml.safe_load((project / ".quill" / "apps.yml").read_text())
    ...
    assert "tok-123" not in (project / ".quill" / "apps.yml").read_text()
...
def test_deploy_cloudflare_invokes_wrangler_and_persists_state(
    ...
    assert "cf-tok" not in (project / ".quill" / "apps.yml").read_text()
