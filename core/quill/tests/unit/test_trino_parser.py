...
from quill.connector.trino import (
    _build_trino_column,
...
def test_trino_connector_import_error_has_install_hint(monkeypatch) -> None:
    """If ``import trino`` fails, the connector should raise a QuillError with
    a clear ``pip install 'quill[trino]'`` hint rather than a raw
    ImportError.
    """
    import builtins  # noqa: PLC0415
    import sys  # noqa: PLC0415

    from quill.connector import trino as trino_module  # noqa: PLC0415
    ...
    assert exc.value.error_code == ErrorCode.INVALID_CONNECTION_INFO
    msg = str(exc.value)
    assert "pip install 'quill[trino]'" in msg
...
def test_native_connector_does_not_import_ibis(monkeypatch) -> None:
    ...
    monkeypatch.delitem(sys.modules, "quill.connector.trino", raising=False)
    import quill.connector.trino  # noqa: F401, PLC0415
