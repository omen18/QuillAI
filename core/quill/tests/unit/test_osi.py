"""Tests for OSI (Open Semantic Interchange) → Quill MDL conversion."""
...
from quill.osi import (
    _extract_quill_block,
    _is_calculated_expression,
    _parse_source,
    _pick_expression,
    build_json_from_osi,
    build_manifest_from_osi,
    extract_quill_config,
    lint_osi_file,
    parse_osi,
...
def test_select_semantic_model_ambiguous_errors_with_snippet():
    ...
    assert "vendor_name: QUILL" in errs[0].message
...
# ── Unit: QUILL block extraction ───────────────────────────────────────────


def test_extract_quill_block_parses_json_string():
    ext = [
        {"vendor_name": "QUILL", "data": '{"dialect": "SNOWFLAKE"}'},
        {"vendor_name": "DBT", "data": '{"foo": "bar"}'},
    ]
    out = _extract_quill_block(ext)
    assert out == {"dialect": "SNOWFLAKE"}


def test_extract_quill_block_tolerates_raw_dict():
    """Spec says `data` is a JSON string, but tolerate dict for hand-authored YAML."""
    ext = [{"vendor_name": "QUILL", "data": {"dialect": "ANSI_SQL"}}]
    assert _extract_quill_block(ext) == {"dialect": "ANSI_SQL"}


def test_extract_quill_block_no_match_returns_empty():
    ext = [{"vendor_name": "DBT", "data": "{}"}]
    assert _extract_quill_block(ext) == {}


def test_extract_quill_block_last_wins_when_duplicated():
    ext = [
        {"vendor_name": "QUILL", "data": '{"dialect": "ANSI_SQL"}'},
        {"vendor_name": "QUILL", "data": '{"dialect": "SNOWFLAKE"}'},
    ]
    assert _extract_quill_block(ext)["dialect"] == "SNOWFLAKE"


def test_extract_quill_block_ignores_malformed_json():
    ext = [{"vendor_name": "QUILL", "data": "{this is not json}"}]
    assert _extract_quill_block(ext) == {}


# ── Unit: extract_quill_config — precedence ────────────────────────────────


def test_extract_quill_config_sm_overrides_root():
    osi = {
        "custom_extensions": [
            {
                "vendor_name": "QUILL",
    ...
    cfg, errs = extract_quill_config(osi, sm)
    assert cfg.dialect == "SNOWFLAKE"
    assert cfg.metrics_mode == "note"  # inherited from root
    assert errs == []


def test_extract_quill_config_cli_overrides_sm():
    osi = {}
    sm = {"custom_extensions": [{"vendor_name": "QUILL", "data": '{"metrics": "note"}'}]}
    cfg, _ = extract_quill_config(osi, sm, cli_overrides={"metrics": "skip"})
    assert cfg.metrics_mode == "skip"


def test_extract_quill_config_invalid_metrics_falls_back_with_warning():
    osi = {}
    sm = {
        "custom_extensions": [{"vendor_name": "QUILL", "data": '{"metrics": "bogus"}'}]
    }
    cfg, errs = extract_quill_config(osi, sm)
...
def test_build_minimal_clean_no_warnings():
    """Minimal fixture has QUILL column_types for every dataset — no warnings."""
...
def test_build_minimal_structure():
    ...
    # Column types pulled from QUILL block
...
def test_build_minimal_calculated_field():
    """customer.full_name has a SQL expression and per-field QUILL type override."""
    manifest, _ = build_manifest_from_osi(
        _fixture("minimal.yaml"), data_source="postgres"
    )
    ...
    assert full_name["type"] == "VARCHAR"  # from field-level QUILL block
...
def test_build_tpcds_composite_pk_preserved():
    """store_sales has composite PK [ss_item_sk, ss_ticket_number] — quill MDL now
...
def test_build_tpcds_untyped_field_warnings_include_snippets():
    """No QUILL column_types provided → every dataset triggers a typed warning
    ...
    for w in untyped:
        ...
        assert "vendor_name: QUILL" in w.message
...
def test_cli_validate_from_osi_surfaces_warnings():
    """TPC-DS fixture has no QUILL column_types → warnings printed."""
    result = runner.invoke(
        app,
        [
            "context",
...
def test_cli_init_from_osi_scaffolds_project(tmp_path: Path):
    """OSI → quill project layout, ready for the standard build flow."""
    ...
    # Standard quill project layout
    assert (proj / "quill_project.yml").exists()
    ...
    # The OSI semantic_model.name flowed into quill_project.yml
    import yaml as _yaml  # noqa: PLC0415

    cfg = _yaml.safe_load((proj / "quill_project.yml").read_text())
...
def test_cli_init_from_osi_refuses_overwrite_without_force(tmp_path: Path):
    """Without --force, an existing quill_project.yml blocks the migration."""
    proj = tmp_path / "existing"
    proj.mkdir()
    (proj / "quill_project.yml").write_text("name: prior\n")
    result = runner.invoke(
        app,
        [
            "context",
