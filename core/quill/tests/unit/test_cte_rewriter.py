...
from quill.mdl import get_session_context
...
class TestSingleModel:
    ...
    def test_star_not_expanded(self):
        """SELECT * should pass star to quill-core so all columns are included."""
        rw = _make_rewriter(_SINGLE_MODEL_MANIFEST)
        result = rw.rewrite('SELECT * FROM "orders"')
        assert _has_cte(result, "orders")
        cte_body = _cte_body_sql(result, "orders")
        assert cte_body is not None
        # quill-core expands SELECT * into all model columns — verify every
...
class TestMultiModel:
    ...
    def test_qualified_star_not_expanded(self):
        """SELECT table.* should pass star to quill-core so all columns appear."""
        rw = _make_rewriter(_MULTI_MODEL_MANIFEST)
...
class TestCTEEdgeCases:
    ...
    def test_user_cte_shadows_model(self):
        ...
        # the whole query (quill-core handles user CTEs natively).
        assert isinstance(result, str)
        # No model-generated CTE named "orders" should be present
        # (quill-core may restructure the query but won't add a model CTE)
...
class TestView:
    ...
    def test_view_body_kept_verbatim(self):
        """The view CTE body is the native-SQL statement, not a quill-core
...
class TestDialectMapping:
    ...
    def test_quill_maps_to_trino(self):
        assert get_sqlglot_dialect(DataSource.quill) == "trino"
