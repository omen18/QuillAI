...
dependency-free and `quill memory store` must work without LanceDB.
"""
...
from quill.memory.markdown import (
    load_query_pairs,
...
def test_parse_source_handles_null_tags():
    ...
    from quill.memory.cli import _parse_source  # noqa: PLC0415
...
def test_recall_path_annotation_handles_collisions(tmp_path, monkeypatch):
    ...
    from quill.memory.cli import _annotate_markdown_paths  # noqa: PLC0415
...
def test_cli_store_writes_markdown_without_extra(tmp_path, monkeypatch):
    """`quill memory store` works without the memory extra — markdown only."""
    monkeypatch.setenv("QUILL_PROJECT_HOME", str(tmp_path))
