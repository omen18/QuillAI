"""Unit tests for QuillEngine — no database required.

transpile() and dry_plan() exercise the quill-core MDL planning + sqlglot
...
def duckdb_engine(tmp_path_factory):
    """A QuillEngine pointed at a temporary DuckDB file (not queried by unit tests)."""
    db_dir = tmp_path_factory.mktemp("unit_duckdb")
    conn_info = {"url": str(db_dir), "format": "duckdb"}
    with QuillEngine(_MANIFEST_STR, DataSource.duckdb, conn_info, fallback=False) as e:
        yield e


@pytest.fixture(scope="module")
def pg_engine():
    """A QuillEngine configured for Postgres (no real connection opened for planning)."""
    conn_info = {
        "host": "localhost",
        "port": 5432,
        "database": "test",
    ...
    with QuillEngine(_MANIFEST_STR, DataSource.postgres, conn_info, fallback=False) as e:
        yield e
...
def test_dry_plan_returns_string(duckdb_engine: QuillEngine) -> None:
    sql = duckdb_engine.dry_plan('SELECT o_orderkey FROM "orders" LIMIT 1')
    assert isinstance(sql, str)
    assert len(sql) > 0


def test_dry_plan_postgres_dialect(pg_engine: QuillEngine) -> None:
...
def test_dry_plan_calculated_field(duckdb_engine: QuillEngine) -> None:
...
def test_dry_plan_invalid_sql_raises(duckdb_engine: QuillEngine) -> None:
...
_STRICT_CONFIG = QuillConfig(strict_mode=True)
_BLACKLIST_CONFIG = QuillConfig(denied_functions=frozenset(["pg_read_file"]))
...
def test_strict_mode_blocks_unknown_table():
    ...
    with QuillEngine(
        ...
        with pytest.raises(QuillError) as exc_info:
...
def test_non_strict_mode_allows_unknown_table(duckdb_engine: QuillEngine):
