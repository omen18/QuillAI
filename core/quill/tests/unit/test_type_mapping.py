"""Tests for quill.type_mapping — parse_type() and parse_types()."""
...
from quill.type_mapping import (
    parse_type,
    parse_types,
    translate_type,
    translate_types,
...
def _run_quill(*args: str, stdin: str | None = None) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-m", "quill.cli", *args],
        input=stdin,
        capture_output=True,
        text=True,
    )
...
def test_cli_parse_type_single() -> None:
    result = _run_quill("utils", "parse-type", "--type", "int8", "--dialect", "postgres")
...
def test_cli_parse_types_stdin() -> None:
    ...
    result = _run_quill("utils", "parse-types", "--dialect", "postgres", stdin=payload)
    _assert_success(result)
