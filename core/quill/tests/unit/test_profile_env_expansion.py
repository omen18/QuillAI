"""Unit tests for ``quill.profile.expand_profile_secrets`` and ``.env`` discovery.
...
from quill import profile as profile_mod
from quill.profile import (
    MissingSecretError,
...
def test_user_global_env_is_used(tmp_path, monkeypatch):
    """``~/.quill/.env`` provides a fallback for operators sharing secrets
    across multiple projects."""
    monkeypatch.delenv("GLOBAL_VAR", raising=False)
    quill_home = tmp_path / ".quill"
    quill_home.mkdir()
    (quill_home / ".env").write_text("GLOBAL_VAR=global\n")
    monkeypatch.setattr(profile_mod, "_QUILL_HOME", quill_home)
...
def test_project_root_env_loaded_when_cwd_is_subdir(tmp_path, monkeypatch):
    ...
    (mirrors the typical ``cd subdir && quill --sql`` usage)."""
...
def test_debug_profile_returns_literal_placeholder(tmp_path, monkeypatch):
    """``quill profile debug`` shows the ``${VAR}`` text, never the real value."""
...
def test_debug_masks_registry_sensitive_fields(tmp_path, monkeypatch):
    ...
    cases = [
        ("quill_p", {"datasource": "quill", "host": "h", "pat": "SECRET"}, "pat"),
        (
            "url_p",
