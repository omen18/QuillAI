"""Behavior tests for `quill genbi build` — the instruction composer."""
...
def _make_project(tmp_path: Path) -> Path:
    ...
    (target / "mdl.json").write_text(
        '{"catalog": "quill", "schema": "public", "models": []}'
    )
    return tmp_path
...
def test_build_includes_model_inventory(tmp_path: Path) -> None:
    ...
    # Data source type from quill_project.yml
...
def test_build_includes_wasm_wiring_and_final_steps(tmp_path: Path) -> None:
    ...
    assert "quill-core-wasm" in result.output
    ...
    assert "quill genbi register myapp --data-mode snapshot" in result.output
    assert "quill genbi verify myapp" in result.output
