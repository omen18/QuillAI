...
from quill.mcp_server import ServeContext, _workflow_text, build_server  # noqa: E402
...
def test_recall_queries_uses_ctx_project(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("quill.memory.index_backend.get_index", fake_get_index)

    recall_queries(question="revenue")

    assert captured["project"] == proj_b
    assert captured["mem_path"] == str(proj_b / ".quill" / "memory")
    assert captured["mem_path"] != str(proj_a / ".quill" / "memory")
...
def test_get_context_uses_ctx_project(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("quill.context.build_json", lambda project: {})
    ...
    monkeypatch.setattr("quill.memory.store.MemoryStore", FakeStore)

    get_context(question="revenue")

    assert captured["path"] == str(proj_b / ".quill" / "memory")
    assert captured["path"] != str(proj_a / ".quill" / "memory")
