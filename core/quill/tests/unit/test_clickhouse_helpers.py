"""Unit tests for ``quill.connector.clickhouse`` URL parsing and query wrapping.
...
from quill.connector.clickhouse import (
    ClickHouseConnector,
...
def test_clickhouse_query_strips_trailing_semicolon_before_subquery_wrap() -> None:
    ...
    assert sent == "SELECT * FROM (SELECT 1) AS _quill_sub LIMIT 5"
...
def test_clickhouse_query_strips_multiple_trailing_semicolons_and_whitespace() -> None:
    ...
    (sent,), _ = mock_conn.query.call_args
    assert sent == "SELECT * FROM (SELECT 1) AS _quill_sub LIMIT 3"
...
def test_clickhouse_dry_run_strips_trailing_semicolon() -> None:
    ...
    (sent,), _ = mock_conn.query.call_args
    assert sent == "SELECT * FROM (SELECT 1) AS _quill_sub LIMIT 0"
