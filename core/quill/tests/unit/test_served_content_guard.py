"""Guard: every `quill <cmd>` mentioned in bundled skill/docs/template content
...
rollout — e.g., a lifted skill mentioning `quill docs get` before that command
...
# ``quill memory`` is conditionally registered (needs `quill[memory]`
...
# The first token after `quill` must be one of these (commands or top-level
# flags) for the snippet to count as a CLI invocation worth validating. Otherwise
# it's prose ("the quill engine connects to ...", "in the quill project layout").
_TOP_LEVEL_COMMANDS: set[str] = {p.split()[0] for p in COMMANDS if p}

# Flags on `quill memory ...` cannot be introspected here without the memory
...
_SKILLS_CONTENT = _REPO / "core" / "quill" / "src" / "quill" / "skills_content"
_DOCS_CONTENT = _REPO / "core" / "quill" / "src" / "quill" / "docs_content"
_ASK_TEMPLATES = _REPO / "core" / "quill" / "src" / "quill" / "ask_templates"
# The discovery stub ships to users' local skill dirs via `npx skills add`,
# so any `quill <cmd>` it references must resolve to a real CLI command too.
_DISCOVERY_STUB = _REPO / "skills" / "quill" / "SKILL.md"

# Match `quill ` followed by content up to a newline, backtick, single/double
# quote (avoid consuming SQL strings), or closing paren.
_INVOCATION = re.compile(r"\bquill\s+(?P<rest>[^\n`'\"\)]+)")
...
def _iter_content_files() -> list[Path]:
    ...
    the standalone discovery stub at `skills/quill/` that's distributed to
...
def _enumerate_invocations() -> list[tuple[Path, str, list[str]]]:
    """Return (file, raw_snippet, tokens) for every quill invocation in served content."""
...
def _findings() -> list[str]:
    ...
    for path, snippet, tokens in _enumerate_invocations():
        ...
        # command/group OR a top-level flag (e.g. `quill --sql ...`). Otherwise
        # it's prose ("the quill engine", "in the quill project layout") — skip.
        ...
        # subcommand the user typo'd — `quill docs typo`, `quill memory typo`.
        ...
        if (
            ...
            problems.append(
                f"{rel}: unknown subcommand '{leftover[0]}' for 'quill {cmd_path}'"
                + f"  (in `quill {snippet}`)"
            )
            continue
        ...
        for tok in leftover:
            ...
            if flag not in allowed:
                problems.append(
                    f"{rel}: unknown flag '{flag}' for 'quill {cmd_path}'".rstrip()
                    + f"  (in `quill {snippet}`)"
                )
    return problems
...
def test_at_least_one_invocation_was_validated():
    """Sanity: ensure the squill actually finds invocations to validate
    ...
    # squill that silently dropped most matches would weaken the guard
    # without obviously failing — keep the floor tight enough to notice.
    assert len(invocations) >= 200, (
        f"only found {len(invocations)} quill invocations; regex may be broken"
    )


def test_unknown_subcommand_is_flagged(tmp_path, monkeypatch):
    """An unknown subcommand under a known group (`quill docs typo`) must
    be reported. Leaf commands taking positional args (`quill skills get
    ...
    fake_skill.write_text(
        "Run `quill docs typo` to break stuff.\n"
        "Also `quill skills get usage` — this is legit (positional, not typo).\n"
        "And `quill memory typo` — should also be flagged.\n",
        encoding="utf-8",
    )
    ...
    problems = _findings()
    joined = "\n".join(problems)
    assert "unknown subcommand 'typo' for 'quill docs'" in joined, problems
    assert "unknown subcommand 'typo' for 'quill memory'" in joined, problems
