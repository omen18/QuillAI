"""Unit tests for the quill.memory module."""

from __future__ import annotations

import pytest

from quill.memory.schema_indexer import (
    describe_schema,
...
# These require lancedb + sentence-transformers (quill[memory] extra).


@pytest.fixture
def memory_store(tmp_path):
    """Create a MemoryStore backed by a temp directory."""
    pytest.importorskip("lancedb", reason="quill[memory] extras not installed")
    pytest.importorskip(
        "sentence_transformers", reason="quill[memory] extras not installed"
    )
...
# ── QuillMemory public API tests ───────────────────────────────────────────


@pytest.fixture
def quill_memory(tmp_path):
    """Create a QuillMemory instance backed by a temp directory."""
    pytest.importorskip("lancedb", reason="quill[memory] extras not installed")
    pytest.importorskip(
        "sentence_transformers", reason="quill[memory] extras not installed"
    )

    from quill.memory import QuillMemory  # noqa: PLC0415

    return QuillMemory(path=tmp_path)


@pytest.mark.unit
class TestQuillMemory:
    def test_full_lifecycle(self, quill_memory):
        result = quill_memory.index_manifest(_MANIFEST)
        assert result["schema_items"] == 11

        ctx = quill_memory.get_context(_MANIFEST, "customer")
        assert ctx["strategy"] == "full"
        assert "### Model: customer" in ctx["schema"]

        quill_memory.store_query(
            nl_query="find expensive orders",
            sql_query="SELECT * FROM orders WHERE o_totalprice > 1000",
        )
        recalled = quill_memory.recall_queries("costly orders")
        assert len(recalled) >= 1
        assert any(r["nl_query"] == "find expensive orders" for r in recalled)

        assert quill_memory.schema_is_current(_MANIFEST)

        status = quill_memory.status()
        assert status["tables"]["schema_items"] == 11
        # query_history has 1 user query + seed queries
        assert status["tables"]["query_history"] >= 1

        quill_memory.reset()
        assert quill_memory.status()["tables"] == {}
...
class TestMemoryStoreSeedLifecycle:
    def test_index_schema_seeds_query_history(self, memory_store):
        from quill.memory.seed_queries import SEED_TAG  # noqa: PLC0415
    ...
    def test_index_schema_no_seed_flag(self, memory_store):
        ...
        assert result["seed_queries"] == 0
        from quill.memory.store import _table_names  # noqa: PLC0415
...
class TestYamlRoundTrip:
    def test_pairs_to_yaml_and_back(self, memory_store):
        from quill.memory.cli import _pairs_to_yaml  # noqa: PLC0415
...
class TestMarkdownSourcedIndex:
    ...
    def test_load_query_pairs_index_is_convergent(self, memory_store, tmp_path):
        from quill.memory.markdown import (  # noqa: PLC0415
            load_query_pairs,
    ...
    def test_lancedb_backend_via_get_index(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_MEMORY_BACKEND", "lancedb")
        ...
        write_query_markdown(tmp_path, "Total revenue", "SELECT SUM(amount) FROM o")
        idx = get_index(tmp_path, str(tmp_path / ".quill" / "memory"))
    ...
    def test_cli_export_migrates_query_history_to_markdown(self, tmp_path, monkeypatch):
        """`quill memory export` writes existing LanceDB pairs to knowledge/sql/."""
        pytest.importorskip("lancedb", reason="quill[memory] extras not installed")
        pytest.importorskip(
            "sentence_transformers", reason="quill[memory] extras not installed"
        )
        ...
        from quill.cli import app  # noqa: PLC0415
        from quill.memory.markdown import parse_query_markdown  # noqa: PLC0415
        from quill.memory.store import MemoryStore  # noqa: PLC0415

        monkeypatch.setenv("QUILL_PROJECT_HOME", str(tmp_path))
        store = MemoryStore(path=str(tmp_path / ".quill" / "memory"))
