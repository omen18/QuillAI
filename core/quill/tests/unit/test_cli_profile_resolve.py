...
from quill.cli import _resolve_engine_profile
...
def _write_project(project_dir: Path, **fields) -> Path:
    ...
    (project_dir / "quill_project.yml").write_text(yaml.safe_dump(config))
...
def test_resolve_engine_profile_falls_back_when_no_project_at_cwd(
    ...
    monkeypatch.chdir(tmp_path)  # tmp_path has no quill_project.yml
...
def test_resolve_engine_profile_walks_up_from_nonstandard_mdl_layout(
    ...
    (proj / "quill_project.yml").write_text(
        _yaml.safe_dump(
            {
                "schema_version": 3,
                "name": "test_proj",
    ...
    assert name == "via_walk_up", (
        "Walk-up didn't find quill_project.yml; resolver still relies on the "
