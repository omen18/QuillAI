"""Case-sensitivity rules across the quill SDK.
...
from quill.mdl.cte_rewriter import CTERewriter
...
def test_cte_rewriter_renames_outer_alias_to_avoid_bigquery_shadow():
    ...
    quill-core's transform_sql emits ``SELECT "<m>".col FROM (...) AS "<m>"``;
    ...
    outermost alias to ``quill_src_<m>`` to break the shadow chain.
    """
    manifest = {
        "catalog": "quill",
        "schema": "public",
    ...
    assert "quill_src_Cards_Cleaned" in rewritten
...
def test_cte_rewriter_oracle_uppercases_columns():
    ...
    quill-core's schema check then rejected.
    """
    manifest = {
        "catalog": "quill",
        "schema": "public",
