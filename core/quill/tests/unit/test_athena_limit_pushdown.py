...
from quill.connector.athena import AthenaConnector
...
def test_query_pushes_limit_into_sql():
    ...
    with patch(
        "quill.connector.athena._build_athena_arrow_table", return_value=MagicMock()
    ) as builder:
        # contextlib.closing expects .close()
        cursor.close = MagicMock()
        # wrap closing: connection.cursor() is used as CM via closing()
        with patch(
            "quill.connector.athena.contextlib.closing", side_effect=lambda c: _CM(c)
        ):
            connector.query("SELECT 1;", limit=3)

    cursor.execute.assert_called_once_with(
        "SELECT * FROM (\nSELECT 1\n) AS _quill_sub LIMIT 3"
    )
...
def test_query_without_limit_runs_original_sql():
    ...
    with (
        ...
        patch("quill.connector.athena.contextlib.closing", side_effect=lambda c: _CM(c)),
    ):
        connector.connection.cursor.return_value = cursor
