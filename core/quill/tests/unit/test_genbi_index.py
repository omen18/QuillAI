"""Behavior tests for the app index — `quill genbi register/list/remove`."""
...
from quill.genbi.index import MalformedIndexError, index_path, load_index
...
def test_register_requires_app_dir_on_disk(tmp_path: Path) -> None:
    ...
    assert not (project / ".quill" / "apps.yml").exists()
...
def test_load_index_normalises_null_apps(tmp_path: Path) -> None:
    ...
    from quill.genbi.index import get_app
