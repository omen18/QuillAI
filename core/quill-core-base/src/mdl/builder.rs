...
mod test {
    ...
    fn test_model_roundtrip() {
        ...
        let model = ModelBuilder::new("test")
            .table_reference(r#""Quill"."Public"."Source""#)
            .column(ColumnBuilder::new("id", "integer").build())
            .build();
    ...
    fn test_manifest_layout_version_default() {
        let json = r#"{"catalog":"quill","schema":"public"}"#;
    ...
    fn test_manifest_layout_version_explicit() {
        let json = r#"{"layoutVersion":2,"catalog":"quill","schema":"public"}"#;
        let manifest: Manifest = serde_json::from_str(json).unwrap();
