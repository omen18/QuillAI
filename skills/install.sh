#!/usr/bin/env bash
# Install the Quill agent skill discovery stub into your local AI client.
#
# The actual skill content lives inside the `quill` CLI itself
# (`pip install quill`). This script installs the discovery stub (`quill`)
# that points an AI client at the CLI; from then on the agent fetches
# everything else via `quill skills get` / `quill ask`.
#
# Usage:
...
if [ -n "$SCRIPT_DIR" ] && [ -d "$SCRIPT_DIR/$SKILL" ]; then
  ...
  curl -fsSL "https://github.com/$REPO/archive/refs/heads/$BRANCH.tar.gz" \
    | tar -xz -C "$tmpdir" --strip-components=2 "Quill-${BRANCH}/skills/${SKILL}"
