---
sidebar_label: How does Quill keep agents from hallucinating?
---

# How does Quill keep agents from hallucinating?
...
Quill's architecture is designed to **give the agent the context it needs at every step**, and to **fail loudly when the agent guesses**. Here is how the pieces fit.
...
If `email` is not in the `customers` model, the agent cannot query it. It does not exist in Quill's view of your data. If the canonical join between `orders` and `customers` is declared once in MDL, the agent does not have to invent it.
...
`quill dry-plan` takes the agent's modeled SQL and expands it into the exact SQL that will run against the database. The agent, and you, see:
...
```text
1. quill memory recall  -q "top customers by revenue"
   → returns 2 similar past NL-SQL pairs

2. quill memory fetch    -q "customer lifetime value"
