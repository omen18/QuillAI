...
> An agent that ships in production does not memorize your business. It reads structured context, recalls proven examples, and writes them back as it works. Quill is designed around that loop, and it is what lets the BI your agents generate get more correct over time.
...
The difference is that an agent forgets between sessions unless your tooling stores the learning somewhere reviewable. Quill captures that learning in four explicit places (MDL, business rules, confirmed NL→SQL pairs, and the memory index) so the agent picks up where the team left off, every time.

## Two phases: scaffold fast, enrich deep

Quill runs the agent through two phases whenever you set up a new project.
...
|---|---|---|---|
| **MDL** (`models/`, `views/`, `relationships.yml`) | Structural and semantic contract: what data exists, how it relates, which calculations are reusable | `loyalty_v3` is the canonical loyalty table; `revenue = price * qty - refunds` | `quill context build`, manual edits, agent-proposed changes |
...
| **`knowledge/sql/`** | Confirmed natural-language-to-SQL pairs, one markdown file each — the source of truth for recall | "top customers by revenue" → the accepted SQL | `quill memory store`, manual edits |
| **Memory index** (`.quill/memory/`) | Derived retrieval index over MDL + `knowledge/` (optional LanceDB, else grep) | rebuilt any time, gitignored | `quill memory index` |

MDL says what the data *means*. `knowledge/rules/` says how your team wants it *used*. `knowledge/sql/` records what has *worked*. The first three are version-controlled files in your repo; the memory index is a derived artifact rebuilt from them. Together they are the context layer the rest of the docs refer to — see [What does Quill mean by context?](/oss/concepts/what_is_context).
...
User asks a business question
  │
  ├── 1. quill memory recall   → find similar accepted NL-SQL pairs
  ├── 2. quill memory fetch    → retrieve relevant models, columns, relationships
  ├── 3. Write SQL against MDL objects, not raw tables
  ├── 4. quill dry-plan        → see expanded SQL before execution
  ├── 5. quill --sql ...       → execute
  ├── 6. Repair on failure    → diagnose at MDL layer vs DB layer
  └── 7. quill memory store    → persist the confirmed pair
```
...
Quill lets the system compound:
