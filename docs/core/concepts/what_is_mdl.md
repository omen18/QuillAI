...
Modeling Definition Language (MDL) is the semantic contract at the center of Quill.
...
Quill is the open context layer for AI agents. MDL is the core layer where structural, semantic, and business meaning become machine-readable.
...
MDL models the business-facing shape of your data. A Quill project stores these definitions as readable YAML and compiles them into an engine-ready `target/mdl.json` manifest.
...
When the contract changes, you can review the diff. When a query runs, Quill can plan against the contract. When another agent joins the workflow, it does not need to learn the business from scratch.
...
A Quill project separates the parts that should move with the project from the parts that belong to an environment:
...
This is the same philosophy as the broader Quill workflow: **scaffold fast, then enrich deep**.

## MDL and execution

MDL is not only metadata for prompts. Quill uses MDL during SQL planning.

When a query references modeled objects, Quill expands those models, relationships, calculated fields, and views into executable SQL for the target data source. The Rust semantic engine is the source of truth for how MDL semantics map to SQL.
...
MDL is the durable semantic contract for Quill: readable by humans, usable by agents, enforceable by the engine, and portable across every app that needs trusted business data.
