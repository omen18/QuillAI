...
Memory is the behavioral layer of Quill's context system.
...
That is what Quill memory provides.
...
On top of that markdown, Quill builds a **derived index** for retrieval. With the optional
`memory` extra it's a [LanceDB](https://lancedb.com/) embedding index under `.quill/memory/`
...
| Persistent index | LanceDB under `.quill/memory/` (built by `index`/`store`) | None — the markdown *is* the index, so `index` is a no-op |
...
`QUILL_MEMORY_BACKEND=lancedb`) for semantic recall and schema search; nothing about your
`knowledge/` files changes when you switch.

## How memory is used

When an agent answers a question through Quill, memory usually participates before SQL is written:

```text
User question
  |
  |-- quill memory recall -q "..."  -> find similar accepted questions and SQL
  |-- quill memory fetch -q "..."   -> find relevant models, columns, relationships, and instructions
  |-- Agent writes SQL against MDL objects
  |-- Quill plans and executes the query
  |-- quill memory store            -> save confirmed NL-SQL pair
```
...
`quill memory store` writes the confirmed NL→SQL pair to `knowledge/sql/` and indexes it. The
...
See the [Refine answer quality](/oss/guides/refine) recipe and [CLI reference](/oss/reference/cli#quill-memory--schema--query-memory) for command details.
...
`quill memory index` rebuilds the local index from them. The derived index under
`.quill/memory/` stays gitignored, because it's reproducible from the markdown rather than
the collaboration surface itself.

(Have an older project whose history is still in a LanceDB index? `quill memory export`
...
Memory is how Quill gets better with use while keeping the source of truth inspectable and versionable.
