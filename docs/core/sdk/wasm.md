...
Browser-native semantic SQL engine. The Rust quill-core engine compiled to
...
[`quill-langchain`](./langchain.md) or [`quill-pydantic`](./pydantic.md)
...
| | `quill-core-wasm` | `quill-langchain` / `quill-pydantic` |
|---|---|---|
...
| **Connection profile** | None — data is fetched / registered client-side | Required (built via `quill profile add`) |
| **MDL source** | Passed as a JS object per page load | Read from `target/mdl.json` per tool call |
| **Memory / agent tools** | Not in the WASM SDK | Built-in tools (`quill_query`, `quill_recall_queries`, …) |
| **Query path** | DataFusion executes the SQL in-browser | Engine plans SQL, target database executes |

There is no `QuillToolkit` here — the WASM SDK exposes the engine primitives
...
```bash
npm install @quill/quill-core-wasm
```

```javascript
import { QuillEngine } from '@quill/quill-core-wasm';
```
...
```html
<script type="module">
  import { QuillEngine } from 'https://unpkg.com/@quill/quill-core-wasm@0.3.0/dist/index.js';
</script>
```
...
The same `QuillEngine` instance handles three data-loading modes. Pick the one
that fits your data:
...
```javascript
const engine = await QuillEngine.init();
...
### `QuillEngine.init(options?)`

Initialize the engine and load the WASM binary. Call once per page lifecycle.

```typescript
static async init(options?: QuillEngineOptions): Promise<QuillEngine>
```

| Option | Type | Description |
|---|---|---|
| `wasmUrl` | `string \| URL \| BufferSource` | WASM binary source. Defaults to the sibling `quill_core_wasm_bg.wasm` resolved via `import.meta.url`. |

### `engine.loadMDL(mdl, profile)`

Load an MDL manifest and reconfigure the session with Quill analyzer rules.

```typescript
async loadMDL(mdl: object, profile: QuillProfile): Promise<void>
```
...
```javascript
import wasmUrl from '@quill/quill-core-wasm/dist/quill_core_wasm_bg.wasm?url';
const engine = await QuillEngine.init({ wasmUrl });
```
...
`QuillEngine.init()` is safe to call more than once — each call returns an
...
| `@quill/quill-core-wasm` | quill-core | Browser |
|---|---|---|
...
- **Upstream DataFusion (not the upstream fork).** WASM doesn't need the
  unparser fixes; the trade-off is that some advanced quill-core analyzer
...
- **No memory module.** Semantic-search memory (LanceDB) is a server-side
  feature exposed by `quill-langchain` / `quill-pydantic` — not available in
  the WASM SDK.
