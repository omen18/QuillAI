...
This page documents every YAML artifact in a Quill project — `quill_project.yml`, models, relationships, views, cubes, and the `knowledge/` files — with the full field surface for each.

> For the conceptual framing of MDL, see [What does MDL do for the agent?](/oss/concepts/what_is_mdl). For the project lifecycle commands, see [Manage project](/oss/guides/manage_project). For the canonical YAML compilation flow, run `quill context build` after editing.
...
├── .quill/                         # runtime state (gitignored)
│   └── memory/                    # derived LanceDB index (optional; rebuilt from knowledge/sql/)
└── target/
    └── mdl.json                   # build output (gitignored)
```

`quill_project.yml` carries a `schema_version`; **version 5** is the current layout. To
...
|---|---|---|---|
| `schema_version` | int | yes | Project layout version (current: `5` — adds first-class `knowledge/`). `2` = folder-per-entity, `3` = `dialect` support, `4` = composite primary keys, `5` = `knowledge/`. Owned by the CLI — bump with `quill context upgrade` (see [Migration](./migration.md)). |
| `name` | string | yes | Project identifier. |
| `version` | string | no | User-defined project version (free-form, no parsing effect). |
| `catalog` | string | no | **Quill namespace** — not your database catalog. Defaults to `quill`. |
| `schema` | string | no | **Quill namespace** — not your database schema. Defaults to `public`. |
| `data_source` | string | no | Data source type (`postgres`, `bigquery`, `snowflake`, ...). Set by `quill context set-profile`. |
| `profile` | string | no | The bound connection profile name. Set by `quill context set-profile`. |

> **Two levels of `catalog` and `schema`.** The same field names appear in two places with completely different meanings. The project-level fields are Quill's internal namespace; the model-level `table_reference.catalog` and `table_reference.schema` point at the underlying database location.
>
> | Location | Refers to | Example |
> |---|---|---|
> | `quill_project.yml` → `catalog`, `schema` | Quill namespace | `quill`, `public` |
...
This matters for AI agents: any column you omit from the model cannot be retrieved through Quill, regardless of what the agent asks.
...
| `time_dimensions[]` | no | Time-based group-bys. Granularity is applied at query time via `--time-dimension name:granularity` (see [CLI reference](/oss/reference/cli#quill-cube--pre-aggregation-queries)). |
...
Cubes are queried structurally via `quill cube query`, not by writing raw `GROUP BY` SQL. See [Pre-aggregate with cubes](/oss/guides/cubes) for the agent-facing recipe.
...
- `quill context instructions` — full text, run once at session start
- `quill memory fetch -q "..."` — relevant chunks per query
...
`quill memory store` writes these files; `quill memory index` (re)builds the index from them.
...
`quill context build` converts YAML field names to camelCase in `target/mdl.json`:
...
`layoutVersion` is stamped automatically by `quill context build` based on the project's `schema_version` — you do not set it manually in YAML.
...
Valid dialect values: `athena`, `bigquery`, `quill`, `clickhouse`, `databricks`, `datafusion`, `doris`, `duckdb`, `gcs_file`, `local_file`, `minio_file`, `mssql`, `mysql`, `oracle`, `postgres`, `redshift`, `s3_file`, `snowflake`, `spark`, `trino`.
...
Use `quill context upgrade` to bump. The upgrade is dry-runnable with `--dry-run`.
...
Source YAML and the `knowledge/` base (rules, glossary, metrics, caveats, and the NL→SQL pairs under `knowledge/sql/`) are committed. `target/mdl.json` is derived; `.quill/memory/` is a binary LanceDB index rebuilt from `knowledge/sql/` — commit the `knowledge/sql/*.md` pairs, not the index.

## See also

- [Manage project](/oss/guides/manage_project) — lifecycle commands, profile binding, multi-env workflow
- [CLI reference](./cli.md) — every `quill` command
