...
Quill ships **skills** — reusable AI agent workflow guides that teach
Claude Code (or other AI coding agents) how to use the Quill CLI for
...
Earlier versions of Quill shipped each skill as a separate folder of
...
Since Quill `0.8`, skill content **lives inside the `quill` CLI** and is
served on demand:

- One ~50-line **discovery stub** is installed into your agent
  (`skills/quill/SKILL.md`). It teaches the agent that workflow guides and
  shaped prompts are fetched from the CLI.
- The actual workflow guides live in the `quill` Python package and are
...
|-------|-----------|---------|
| **onboarding** | `quill skills get onboarding` | Entry point: environment checks, project scaffolding, profile setup, first query |
| **usage** | `quill skills get usage` | Day-to-day workflow: gather schema context, recall past queries, write SQL, execute, store results |
| **generate-mdl** | `quill skills get generate-mdl` | One-time setup: explore database schema, normalize types, scaffold MDL YAML project |
| **enrich-context** | `quill skills get enrich-context` | Deepen business context the schema can't carry: enum/unit/null semantics, default filters, synonyms, currency rules, and named aggregation metrics as cubes — via grill or auto-pilot mode |
| **dlt-connector** | `quill skills get dlt-connector` | Connect SaaS APIs (HubSpot, Stripe, Salesforce, GitHub, Slack, …) into DuckDB via dlt, then auto-generate a Quill project |
| **genbi** | `quill skills get genbi` | Turn a project's context layer into a shareable, browser-side GenBI web app and deploy it to Vercel or Cloudflare Pages |

List them with `quill skills list`.
...
The agent fetches guides using `quill skills get <name>`. The first call
returns the SKILL.md body — a focused workflow guide. When the agent
needs more depth it can ask for:

```bash
quill skills get <name> --full             # include the skill's references/ inline
quill skills get <name> --script <stem>    # print a bundled script's source
```

Shaped prompts are served the same way:

```bash
quill ask "<question>" --guided            # for weaker LLMs
quill ask "<question>" --direct            # for stronger LLMs
```
...
```text
User says "install quill" / "set up quill"
  │
  ├── Preflight (read-only)
  │     Python 3.11+, virtualenv, quill CLI, working dir
  │
  ...
  │     mkdir, pip install, quill context init,
  ...
  │
  ├── Step 5. Generate MDL
  │     quill skills get generate-mdl
  │
  └── Step 6. First query
        quill --sql "SELECT 1" (sanity)
...
| **Never invent connection field names** | Always run `quill docs connection-info <ds>` to introspect real fields |
...
- "I want to start a Quill project"
...
User asks a question
  │
  ├── 1. Gather context
  │     quill memory fetch -q "..."
  │     quill context instructions      (first query only)
  │
  ├── 2. Recall past queries
  │     quill memory recall -q "..." --limit 3
  │
  ├── 3. Assess complexity
  ...
  │     Simple: quill --sql "..."
  │     Complex: quill dry-plan first, then execute
  │
  └── 5. Store result
        quill memory store --nl "..." --sql "..."
```
...
|-------|------|-----------|
| **MDL-level** | `quill dry-plan` fails | Wrong model/column names, missing relationships |
| **DB-level** | `quill dry-plan` succeeds but execution fails | Type mismatch, permissions, dialect issues |
...
| Workflow | When |
|----------|------|
| **Connect new data source** | `quill profile add` → `quill context init` → build → index |
| **After MDL changes** | `quill context validate` → `quill context build` → `quill memory index` |

### Reference files

`quill skills get usage --full` inlines two reference documents:
...
- **quill-sql.md** — how the CTE-based rewrite pipeline works. Explains
...
| `queries.yml` / `quill memory store` | canonical and ad-hoc NL→SQL pairs |
...
`quill skills get enrich-context --full` inlines:
...
|-------|------|-------------|
| **1. Connect** | Confirm database access | Test connection via SQLAlchemy, driver, or `quill profile debug` |
| **2. Discover** | Collect schema metadata | Introspect tables, columns, types, foreign keys |
| **3. Normalize** | Convert types | `quill utils parse-type` or Python `parse_type()` |
| **4. Scaffold** | Write YAML project | `quill context init`, create model files, relationships |
| **5. Validate** | Check integrity | `quill context validate` → `quill context build` |
| **6. Index** | Initialize memory | `quill memory index` |
...
| **Raw SQL via quill** | Bootstrapping when no Python driver is installed |
...
```bash
# Single type
quill utils parse-type --type "character varying(255)" --dialect postgres
# → VARCHAR(255)

# Batch (stdin JSON)
echo '[{"column":"id","raw_type":"int8"}]' | quill utils parse-types --dialect postgres
```

Or via Python:

```python
from quill.type_mapping import parse_type
...
auto-generation of a Quill project on top.
...
| **2. Model** | Auto-generate a Quill project | Run the bundled `introspect_dlt` script to scan DuckDB, normalize types via `quill.type_mapping.parse_type()`, write models, relationships, profile |
| **3. Build & Verify** | Confirm queries work end-to-end | `quill context build`, `quill memory index`, run sample SQL through the engine — not just file generation |
...
1. **DuckDB catalog naming** — when Quill `ATTACH`es a `.duckdb` file,
...
2. **Type normalization through quill SDK** — column types must go through
   `quill.type_mapping.parse_type()` (sqlglot-based). Don't hardcode
...
- "I have a `.duckdb` file from dlt — make a Quill project from it"
...
app (powered by `quill-core-wasm`) and deploys it to Vercel or Cloudflare
Pages — from a natural-language request to a public URL in one conversation.
Full command reference: [`quill genbi`](cli.md#quill-genbi--build--deploy-genbi-apps).
...
the instruction. `.quill/apps.yml` is only ever written by the CLI.

### Workflow

| Step | Goal | Key actions |
|------|------|-------------|
| **1. Build** | Get the authoritative build instruction | `quill genbi build <name> --prompt "…" --data-mode snapshot\|live` — prints wasm wiring (pinned version), the model/column inventory, acceptance criteria, target folder |
...
| **3. Register & verify** | Record + preflight | `quill genbi register <name> --data-mode <mode>`, then `quill genbi verify <name>` (files, parseable MDL, snapshot asset, default-deny secret scan) |
| **4. Preview** | Local check | `quill genbi open <name>` serves the app for local review |
| **5. Deploy** | Ship a URL | `quill genbi deploy <name> --provider vercel\|cloudflare [--prod]` — preview by default; confirm before `--prod` |
...
- "build a dashboard from my Quill project"
...
`quill skills get <name>` prints `SKILL.md`; `--full` appends every
`references/*.md` in sorted order; `--script <stem>` prints a single file
from `scripts/`. The skill bundle is shipped inside the `quill` wheel
via Hatchling's `[tool.hatch.build.targets.wheel] artifacts` glob, so a
fresh `pip install quill` always carries matching skill content.
