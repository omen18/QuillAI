...
in place until you've verified the result. Run `quill context upgrade` to go to the latest
...
> New projects from `quill context init` are already v5 — these steps are only for projects
> created before v5.
...
quill context upgrade             # restamp to schema_version 5 and create knowledge/
```
...
`instructions.md` still works — `quill context build` and `quill memory index` read it
...
If you have an existing LanceDB memory at `~/.quill/memory`, export it (requires the
`memory` extra to read LanceDB):

```bash
quill memory export                 # query_history → knowledge/sql/*.md
quill memory index                  # rebuild the derived index from knowledge/sql/
quill memory recall -q "revenue"    # verify recall still works
quill memory reset                  # once verified — drops the derived index only
```
...
is still loaded on `index` for the transition, but new pairs from `quill memory store` now
land in `knowledge/sql/`.

`store`, `index`, and `recall` all work **without** `quill[memory]` — pairs are written to
and searched over `knowledge/sql/` directly. Install `quill[memory]` only for semantic
(embedding) recall and schema search. See the
[CLI reference](./cli.md#quill-memory--schema--query-memory).
