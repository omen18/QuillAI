# Quill project layout (v5)

> **Internal engineering doc.** Lives in `docs/core/internals/` and is not published to the
> docs site. It describes the on-disk structure of a Quill project at `schema_version: 5`
> for contributors working on the project reader/writer (`core/quill/src/quill/context.py`)
> and the GenBI app tooling (`core/quill/src/quill/genbi/`).
>
> For the user-facing field surface of each YAML artifact, see the
> [MDL schema reference](../reference/mdl.md).

A Quill project is a directory of YAML/SQL source plus a few generated artifacts. v5 is the
...
|------|------|------------------|
| `schema_version` | `quill_project.yml` | the **project layout shape** — file paths, directory structure, required fields |
...
```text
<project>/
├── quill_project.yml                # schema_version: 5 + name / catalog / schema / data_source
│
...
│   ├── apps.yml                    #   GenBI app index — machine-written by `quill genbi register`
│   └── memory/                     #   LanceDB semantic-memory index
└── target/
    └── mdl.json                    # build artifact (gitignored) — `quill context build` output
```
...
structurally via `quill cube query`.

All four compile into `target/mdl.json` via `quill context build`. The reader dispatches on
...
## GenBI apps — `apps/` and `.quill/apps.yml`

`quill genbi` scaffolds, builds, and deploys small front-end apps generated from the
...
The app **index** lives at `.quill/apps.yml` — a machine-written registry (status state
machine: `scaffolded → built → deployed`), mirroring how `~/.quill/profiles.yml` indexes
connection profiles. It is never hand-edited and lives in the gitignored `.quill/` runtime
directory.
...
| `.quill/` (`apps.yml`, `memory/`) | runtime state — gitignored |
| `target/mdl.json` | build artifact — gitignored |

## Constants (`core/quill/src/quill/context.py`)
