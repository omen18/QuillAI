...
- A Quill project directory with `quill_project.yml`, `models/`, `views/`, `relationships.yml`, and `knowledge/`
...
2. **Open your agent in a fresh project directory and ask:**
   > Use the `/quill` skill to scaffold an MDL project for this database.
...
5. **Run a first modeled query.**
   ```bash
   quill --sql "SELECT * FROM customers LIMIT 5"
   ```
...
- **Type normalization** through `quill utils parse-type` so the manifest types are canonical
