...
dashboard, powered by [`quill-core-wasm`](../sdk/wasm.md), and ships it to your
own Vercel or Cloudflare Pages account. You never type the `quill genbi`
...
flag, see the [`quill genbi` CLI reference](../reference/cli.md#quill-genbi--build--deploy-genbi-apps).

## Before you start

You need two things:

- **A Quill project**: the context layer the dashboard reads from. If you don't
  have one yet, [connect a data source](connect.md) first.
- **An agent with the `quill` CLI**: any coding agent that can run shell
  commands. The agent loads its own GenBI playbook on demand via
  `quill skills get genbi`, so you don't have to teach it the workflow.
...
[CLI reference](../reference/cli.md#quill-genbi--build--deploy-genbi-apps).
...
Agent: I need a VERCEL_TOKEN. Add it to ~/.quill/.env, then tell me.
User:  Done.
...
- [`quill genbi` CLI reference](../reference/cli.md#quill-genbi--build--deploy-genbi-apps): every subcommand and flag
...
- [quill-core-wasm](../sdk/wasm.md): the in-browser engine that powers the app
