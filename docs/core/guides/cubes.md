...
- A queryable cube name in MDL — `quill cube query --cube revenue --measures total --time-dimension "order_date:month"`
...
The `quill cube query` CLI takes a structured input:
...
[CLI reference](/oss/reference/cli#quill-cube--pre-aggregation-queries) for the
...
- [How does Quill keep agents from hallucinating?](/oss/concepts/correctness) — why cubes matter as a correctness primitive
