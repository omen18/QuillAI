...
Quill talks to your database through a **profile** — credentials and connection details stored in `~/.quill/profiles.yml`. Once a profile is bound to a project, every CLI command and SDK call uses it.

The flow is the same regardless of which database you use:

1. **Install the connector extra** for your data source.
2. **Create a profile** with `quill profile add` (or let the `onboarding` guide do it).
3. **Bind the profile** to your project with `quill context set-profile`.
4. **Generate the MDL** with the `generate-mdl` guide (or manually).
5. **Start querying.**

If you have an AI coding agent installed, the `/quill` skill drives the whole thing — see [Installation](/oss/get_started/installation).
...
```bash
pip install "quill[postgres,bigquery,main]"
```
...
```bash
quill docs connection-info <ds>
```

The output is generated directly from the engine's connection schema, so it always matches the version of `quill` you have installed. Examples:
...
```bash
quill profile add my-db --from-file connection.yml
```
...
```bash
cd ~/my-project
quill context set-profile my-db
```

This writes `profile: my-db` and `data_source: <ds>` into `quill_project.yml`. Future commands in this project always use this profile, regardless of which profile is globally active.
