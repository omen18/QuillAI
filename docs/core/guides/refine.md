...
Refinement works out of the box — `quill memory store`, `index`, and `recall` operate over
...
The day-to-day refinement loop runs on the `usage` guide plus a few `quill memory` and
...
User asks a question
  → quill memory recall  (find similar past pairs)
  → quill memory fetch   (retrieve relevant schema)
  → write SQL, dry-plan, execute
  → quill memory store   (write the confirmed pair to knowledge/sql/ + index)
```
...
retrieval (`quill memory fetch -q "..."`) and recall (`quill memory recall -q "..."`) now see
the new context. Run `quill memory check` to see whether the index is in sync with the
markdown.
...
A new environment picks them up automatically on the next `quill memory index` (which
...
| Action | How |
|---|---|
| Browse stored pairs | `quill memory list`, or read `knowledge/sql/*.md` |
| Fix an incorrect pair | edit (or delete) its `knowledge/sql/<slug>.md`, then `quill memory index` |
| Share confirmed pairs | commit `knowledge/sql/` (no export step) |
| Check index vs. markdown | `quill memory check` |
